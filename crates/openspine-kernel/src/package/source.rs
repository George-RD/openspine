...
pub(super) fn capture(directory: &Path) -> Result<BTreeMap<String, Vec<u8>>, Error> {
    let root = OpenOptions::new().read(true)
        ...
        .open(directory).map_err(|_| Error::SourceUnavailable)?;
    let mut capture = Capture {
        files: BTreeMap::new(), aliases: BTreeSet::new(), total_bytes: 0,
    ...
    for name in names(&root, &mut capture.remaining_entries)? {
        if FAMILIES.contains(&name.as_str()) || name == "docs" {
            ...
            for file in names(&child, &mut capture.remaining_entries)? {
                if name == "docs" {
                    if !is_document(&file) { return Err(Error::UnsupportedPayload); }
                } else if !matches!(Path::new(&file).extension().and_then(|e| e.to_str()), Some("yaml" | "yml")) {
...
impl Capture {
    fn file(&mut self, parent: &File, name: &str, path: String) -> Result<(), Error> {
        if self.files.len() >= MAX_FILES { return Err(Error::LimitExceeded); }
        if !self.aliases.insert(path.to_ascii_lowercase()) { return Err(Error::InvalidPath); }
        let file = open_at(parent, name, false)?;
        let metadata = file.metadata().map_err(|_| Error::SourceUnavailable)?;
        if !metadata.is_file() || metadata.nlink() != 1 { return Err(Error::SourceUnavailable); }
        if metadata.mode() & 0o111 != 0 { return Err(Error::UnsupportedPayload); }
        let allowance = MAX_FILE_BYTES.min(MAX_TOTAL_BYTES - self.total_bytes);
        if metadata.len() > allowance as u64 { return Err(Error::LimitExceeded); }
        let mut bytes = Vec::new();
        file.take(allowance as u64 + 1).read_to_end(&mut bytes)
            .map_err(|_| Error::SourceUnavailable)?;
        if bytes.len() > allowance { return Err(Error::LimitExceeded); }
        if bytes.contains(&0) || std::str::from_utf8(&bytes).is_err() {
            return Err(Error::UnsupportedPayload);
        }
        self.total_bytes += bytes.len();
        if self.files.insert(path, bytes).is_some() { return Err(Error::InvalidPath); }
        Ok(())
    }
}
...
fn is_document(name: &str) -> bool {
    Path::new(name).extension().and_then(|e| e.to_str())
        .is_some_and(|extension| extension.eq_ignore_ascii_case("md") || extension.eq_ignore_ascii_case("txt"))
}

fn portable_name(name: &str) -> bool {
    if name.is_empty() || name.len() > 128 || name.ends_with('.')
...
fn open_at(parent: &File, name: &str, directory: bool) -> Result<File, Error> {
    ...
    if fd < 0 { return Err(Error::SourceUnavailable); }
...
fn names(parent: &File, remaining: &mut usize) -> Result<Vec<String>, Error> {
    ...
    if duplicate < 0 { return Err(Error::SourceUnavailable); }
    ...
    loop {
        ...
        if entry.is_null() {
            ...
            if unsafe { *errno_pointer() } != 0 { return Err(Error::SourceUnavailable); }
            break;
        }
        // SAFETY: a successful readdir returns a NUL-terminated d_name.
        let name = unsafe { CStr::from_ptr((*entry).d_name.as_ptr()) }
            .to_str().map_err(|_| Error::InvalidPath)?;
        if matches!(name, "." | "..") { continue; }
        if *remaining == 0 { return Err(Error::LimitExceeded); }
        *remaining -= 1;
        if !portable_name(name) { return Err(Error::InvalidPath); }
