...
use crate::store::{package_install::{CommitResult, PrepareResult}, Store};
use super::install_types::{crash_at, InstallError as Error, InstallationResult, PackageProvenance};
...
pub(crate) fn recover(store: &Store, objects: &PackageObjects) -> Result<(), Error> {
    for id in store.recover_package_attempts().map_err(|_| Error::Ledger)? {
        objects.cleanup(id)?;
        store.finish_package_staging_cleanup(id).map_err(|_| Error::Ledger)?;
    }
    Ok(())
}

pub(crate) fn install(store: &Store, objects: &PackageObjects, snapshot: &PackageSnapshot, provenance: PackageProvenance) -> Result<InstallationResult, Error> {
    recover(store, objects)?;
    let metadata = match store.prepare_package_install(snapshot.identity(), provenance).map_err(|_| Error::Ledger)? {
    ...
    let result = (|| {
        ...
        match store.commit_package_install(&metadata).map_err(|_| Error::Ledger)? {
    ...
    if result.is_err() {
        store.interrupt_package_install(&metadata).map_err(|_| Error::Ledger)?;
    } else {
        crash_at("after-commit");
    }
    let cleanup = objects.cleanup(metadata.installation_id).and_then(|()| {
        store.finish_package_staging_cleanup(metadata.installation_id).map_err(|_| Error::Ledger)
    });
    match result {
        Ok(mut result) => {
...
pub(crate) fn list(store: &Store, objects: &PackageObjects) -> Result<serde_json::Value, Error> {
    ...
    let packages: Vec<_> = receipts.into_iter().map(|receipt| {
        let availability = if objects.verify(&receipt.identity()).is_ok() { "available" } else { "unavailable-or-corrupt" };
        serde_json::json!({
