...
fn package_maintenance_does_not_sweep_authority_but_runtime_open_still_does() {
    ...
    super::standing_rules_tests::install_legacy_allow_dark_window_rule(
        &store, &rule_manifest("package-no-sweep", "email.send", DarkWindowDefault::Allow), now,
    );
    assert!(store.active_standing_rule_for_action(&action, now).unwrap().is_some());
    drop(store);
    let maintenance = Store::open_for_package_management(&path).unwrap();
    assert!(maintenance.active_standing_rule_for_action(&action, now).unwrap().is_some());
    assert_eq!(maintenance.count_audit_events_of_kind("standing_rule.ineligible_allow_retired").unwrap(), 0);
    drop(maintenance);
    let runtime = Store::open(&path).unwrap();
    assert!(runtime.active_standing_rule_for_action(&action, now).unwrap().is_none());
    assert_eq!(runtime.count_audit_events_of_kind("standing_rule.ineligible_allow_retired").unwrap(), 1);
}
