        ...
        self.with_immediate_tx_mapped(
            ...
            |tx| {
                ...
                let advisee_scope: NerveScope = serde_json::from_str(&scope_json)
                    .map_err(|err| NerveError::Storage(err.to_string()))?;
                let advisee_max_tier: ModelTier = serde_json::from_str(&tier_json)
                    .map_err(|err| NerveError::Storage(err.to_string()))?;
                ...
                let window_started_ns = i64::try_from(jiff::Timestamp::now().as_nanosecond())
                    .map_err(|_| {
                        NerveError::Storage("timestamp out of SQLite range".to_string())
                    })?;
                tx.execute(
                ...
                super::event_bus::bind_consumer_conn(
                    tx,
                    &consumer_id,
