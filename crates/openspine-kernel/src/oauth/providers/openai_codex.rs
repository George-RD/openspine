...
pub fn spec() -> OAuthProviderSpec {
    OAuthProviderSpec {
        ...
        default_port: 1455,
        client_id: "openspine-codex-client-id",
    }
}
...
mod tests {
    ...
    async fn oauth_device_code_flow_polls_until_token_granted() {
        ...
        let dev_res = request_device_code(&http, Some(&dev_url))
            .await
            .expect("request dev code");
        assert_eq!(dev_res.device_code, "dev-code-123");
        assert_eq!(dev_res.user_code, "WD-40");

        let token_res = poll_device_token(&http, &dev_res.device_code, Some(&token_url))
            .await
            .expect("poll token");
