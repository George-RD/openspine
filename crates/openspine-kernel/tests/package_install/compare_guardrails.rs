...
fn control_root(fixture: &Fixture) -> PathBuf {
    let roots: Vec<_> = fs::read_dir(fixture.root.path()).unwrap()
...
fn comparison_respects_the_existing_runtime_lifetime_lock() {
    ...
    let lock = fs::OpenOptions::new().read(true).write(true)
        .open(control_root(&fixture).join("lifetime.lock")).unwrap();
    ...
    assert_error(&fixture.run(&["package", "compare", id(&installed), id(&installed), "--json"]), "data-directory-locked");
    drop(lock);
...
fn pending_authenticated_operation_is_not_executed_or_cleared_by_comparison() {
    ...
    let key_hex = environment.lines()
        .find_map(|line| line.strip_prefix("OPENSPINE_ARTIFACT_KEY="))
        .unwrap().trim().trim_matches('"').trim_matches('\'');
    assert_eq!(key_hex.len(), 64);
    let key: Vec<u8> = key_hex.as_bytes().chunks_exact(2)
    ...
    let mac: String = hmac_sha256::HMAC::mac(&message, &key).iter()
    ...
    assert_error(&fixture.run(&["package", "compare", id(&installed), id(&installed), "--json"]), "pending-overlay-operation");
    assert_eq!(fs::read(&pending).unwrap(), before);
    assert!(!control_root(&fixture).join("snapshots/comparison-fixture").exists());
...
fn noncanonical_and_hyphenated_selectors_are_bounded_content_free_errors() {
    ...
    for invalid in [
        ...
        "--FORGED-\u{1b}[31m", oversized.as_str(),
    ] {
...
fn comparison_never_bootstraps_missing_empty_or_unrelated_ledgers() {
    for state in ["missing", "empty", "unrelated"] {
        ...
        if state == "empty" {
            ...
            db.execute_batch("CREATE TABLE unrelated (value TEXT); INSERT INTO unrelated VALUES ('keep');").unwrap();
        }
...
fn database_rows(fixture: &Fixture) -> Vec<(String, Vec<String>)> {
    ...
    let names = db.prepare("SELECT name FROM sqlite_master WHERE type = 'table' ORDER BY name")
        .unwrap().query_map([], |row| row.get::<_, String>(0)).unwrap()
        .collect::<Result<Vec<_>, _>>().unwrap();
    names.into_iter().map(|name| {
        let mut statement = db.prepare(&format!("SELECT * FROM \"{}\"", name.replace('"', "\"\""))).unwrap();
        let columns = statement.column_count();
        let mut rows = statement.query_map([], |row| {
            (0..columns).map(|index| Ok(format!("{:?}", row.get_ref(index)?)))
                .collect::<Result<Vec<_>, rusqlite::Error>>().map(|values| values.join("|"))
