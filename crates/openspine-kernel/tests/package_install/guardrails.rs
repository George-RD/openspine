...
fn corrupt_audit_is_refused_without_another_installation_or_audit_append() {
    ...
    assert!(
        conn.execute(
            "UPDATE audit_log SET event_json = '{}' WHERE seq = (SELECT MIN(seq) FROM audit_log)",
            []
        )
        .unwrap()
            > 0
    ...
    assert_eq!(
        conn.query_row("SELECT COUNT(*) FROM installed_packages", [], |row| row
            .get::<_, i64>(0))
            .unwrap(),
        1
    );
...
fn package_namespace_cannot_overlap_configured_active_artifacts() {
    ...
    assert_eq!(
        text.lines()
            .filter(|line| line.starts_with("lyra_dir:"))
            .count(),
        1
    );
    let text = text
        .lines()
        .map(|line| {
            if line.starts_with("lyra_dir:") {
                "lyra_dir: data/packages/objects"
            } else {
                line
            }
        })
...
fn writable_shared_namespace_is_not_accepted_as_immutable_storage() {
    ...
    assert_eq!(
        error_code(&fixture.run(&["install", "--from", "candidate", "--json"])),
        "destination-invalid"
    );
...
fn different_revisions_coexist_without_selecting_either() {
    ...
    assert_ne!(
        first["receipt"]["installation_id"],
        second["receipt"]["installation_id"]
    );
    assert_ne!(
        first["receipt"]["content_digest"],
        second["receipt"]["content_digest"]
    );
    ...
    for package in packages {
        ...
        assert_eq!(package["active"], false);
    }
}
...
fn concurrent_conflicting_installers_publish_only_one_revision_identity() {
    ...
    copy_tree(
        ...
        &fixture.root.path().join("different"),
    );
    fs::write(
        fixture.root.path().join("different/README.md"),
        "Different bytes.\n",
    )
    .unwrap();
...
fn authority_rows(fixture: &Fixture) -> Vec<(String, Vec<String>)> {
    ...
    let names = conn
        ...
        OR name LIKE 'nerve_%') ORDER BY name",
        )
        .unwrap()
