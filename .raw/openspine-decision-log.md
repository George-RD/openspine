...
- `PipelineStage::SEQUENCE` pins the nine stages in canonical order and `PipelineStage::SYNC_PREFIX` derives the synchronous driver prefix before `Gate`; the driver iterates `SYNC_PREFIX` through one `match`, so the enum is the executable stage plan — tests assert `SEQUENCE` pins order and an instrumented driver run's executed-stage trace equals `SYNC_PREFIX` for both lanes.
