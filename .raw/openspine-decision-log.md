...
# D-153 — "Both admission sources" means approval and the headless approved lane; scoped standing-rule admission stays #128

## Decision

`unify-approved-and-delegated-effect-execution` (#127) lands exactly one kernel-owned Gmail draft executor, addressed by the D-146 `executor_id` `gmail.create_draft` (implementation `gmail.draft.v1`), and reaches it from the two admission sources that actually hold a digest-bound `ActionRequest`: per-instance digest-bound approval (`handle_create_approved_draft`) and the D-117 headless approved lane (`handle_headless_approved`), which previously fell through to the generic dispatcher and received a successful stub. The brief's "both admission sources" is satisfied by those two and by nothing else. Precisely scoped: the headless lane routes **any approved digest-bound `email.create_draft` request** through the executor, which is what makes it a real second caller; the requests today's `run_headless_hook` mints carry `target_ref: None` and the raw webhook body as payload (`pipeline/headless.rs:236-263`), so those refuse pre-effect at the target re-derivation instead of drafting. Resolving a webhook envelope into a draft payload plus thread target is not part of this change. Scope-matched, budget-reserved standing-rule admission invoking the executor is `mine-and-match-reusable-authority-by-scope` (#128): the ordinary shell dispatch path receives `payload: Option<&serde_json::Value>`, not an `&ActionRequest`, so it cannot supply the `payload_ref` / `target_ref` / `target_digest` the executor re-derives against, and constructing that resolved context is #128's scope. That path fails closed with `DispatchError::NoExecutor` instead. Correspondingly, `EffectOutcome` is deliberately not wired to reservation finalize/retain: neither delivered caller holds a standing-rule reservation, so the only reservation decision this change makes is `NoExecutor` → cancel.

## Rationale

A stub-on-miss dispatcher reported success for any catalogued-but-unwired action, so the false success had to die before any scoped delegation could be trusted; that is the whole of #127. But reconstructing a digest-bound context from a bare JSON payload inside the generic dispatcher would have re-created exactly the class of bug being removed — the executor's payload-digest and target-digest re-checks are only meaningful against a context the kernel resolved, not one the shell handed over. Deferring the third caller keeps the executor immutable and its re-derivation load-bearing, and it keeps #127 honest about what it proves. The classification is deny-by-default rather than a known-effectful predicate because the catalog declares 50 ids against the 17 handlers registered before this change (15 after it deletes two placeholder stubs): a positive predicate built from delegation descriptors and counterparty-facing marks would have left roughly two dozen reachable effect ids (`secret.rotate`, `connector.enable`, `policy.modify_direct`, `filesystem.host_write`, `coolify.deploy`, `web.form_submit`, …) still returning success, violating D-004.

## Consequences

- The non-effect stub allowlist is closed at seven catalogued READ ids with no kernel-side implementation and no dedicated production route: `memory.read:owner_preferences_limited`, `memory.read:writing_preferences_scoped`, `email.read_inbox`, `email.read_thread:unselected`, `email.read_attachment`, `filesystem.host_read`, `vault.secret_read`. An eighth may be added only if it meets that same rule, and the addition must be recorded here. `artifact.write:task_scratch`, `model.generate:approved_provider`, and `briefcase.topup` are deliberately excluded and must not be re-added.
- Executor readiness is a two-part catalog-owned query (`AppState::is_execution_backed`): a registered implementation descriptor AND a registered `executor_id`. A descriptor alone is not readiness — D-146's independent-axis promise is now enforced in code, not just in a comment.
- The headless lane appends `headless.approved_dispatched` only for `EffectOutcome::Executed`; the refusal, delivery-unknown, and post-attempt-failure outcomes rely on the executor's own truthful audit, so the ledger never restates a false success.
- #128 inherits one obligation: build the resolved digest-bound context and wire `Executed` → finalize / `DeliveryUnknown` → retain+fence for reserved standing-rule admission.
- Two registered handlers that returned a successful `{"stub": true}` body for effectful ids — `workflow.invoke:approved` and `setup.workflow.start` — are deleted, because the allowlist only governs registry *misses* and a registered no-op would have preserved the same false success behind a different mechanism. Both now fail closed. The kernel's `NoExecutor` stays an opaque `500 {"error": "internal_error"}`, uniform with every other dispatch failure, so a semi-trusted shell cannot enumerate which catalogued actions are unwired. Consequently the shell does NOT diagnose it: `/setup` logs and propagates the error and sends the owner no reply, because inventing "not implemented" from an opaque 500 would mask a genuine connector or resource outage. The honest cost is that `/setup` now answers nothing until the workflow is implemented.
- The executor takes its write permit BEFORE recording the pending-write fence, so a rejected write admission is a true `RefusedPreEffect` that leaves no fence row. Reviewers established that the old ordering labelled a provably pre-write admission rejection as a failed write attempt and recorded reconciliation evidence for a call that never happened. `call_with_connector_write` keeps its behaviour for every other caller and is now composed from `admit_connector_write` + `call_with_admitted_connector_write`. The reordering means a fence-insert store error now drops an admitted permit: `ConnectorProbePermit::Drop` counts an abandoned HalfOpen probe as a connector failure, which re-opens the breaker. That direction is deliberate and fail-closed — the pre-existing Drop contract exists precisely so an abandoned probe never strands the breaker in HalfOpen, and recording success for a probe that never ran would be the unsafe alternative.

## Would change if

The generic dispatch path ever receives a kernel-resolved `ActionRequest` (rather than an opaque payload) before #128 lands — the third caller could then be wired without weakening any re-derivation.
...
Concretely: each classified briefcase item carries an immutable, kernel-minted **provenance label** that records the item's typed-identity **origin** alongside its existing `disclosure_class` sensitivity and its source lineage (producing event + producing exchange digest). Origin is a closed, typed `ProvenanceOrigin` — `Owner(PrincipalId)`, `Counterparty(IdentityRef)`, or `System` — never a string. `Public` is a sensitivity class, not an origin. At **context assembly**, kernel briefcase packing derives each item's origin from the resolved typed identity of its source.

At **external egress**, disclosure coverage and origin closure are separate ordered checks. After coverage passes, every non-public item must either have a genuine `System` origin, match the recipient's typed identity, or be authorized by the grant chain's provenance allowlist. Owner-origin data has no universal exemption: the owner's internal data sent to a stranger is cross-identity data and blocks unless the grant authorizes that origin. A covering disclosure policy alone cannot override a narrowed provenance allowlist. An unresolved origin always fails closed. No LLM or agent judgment is involved.

Grant authorization retains the shipped append-only, narrow-only semantics: no `ProvenanceLabelAllowlist` caveat means unrestricted origins; an empty allowlist authorizes no cross-origin disclosure; multiple caveats intersect. The v1 single-owner owner grant omits this caveat, while narrowed worker grants enforce their effective allowlists. This clarification reconciles the earlier owner-exemption wording with #220 user story 3, CLAIM-44, and the implemented owner-internal-to-stranger block; it does not change runtime authority or lift the Bell fit-review fence.
...
- Legacy schema-1 briefcases that used `System` as a placeholder for the owner's grant, preferences, or skills are refused on load and mutation. Their stored labels are preserved, never guessed into an owner identity. Recovery composes a fresh task from current trusted sources; genuine system provenance remains distinct from these placeholders.
...
| 2026-09-24 | Reconciled D-174's owner-exemption wording with #220 story 3, CLAIM-44 and shipped origin closure; clarified coverage versus grant-chain authorization and fail-closed handling of legacy schema-1 owner sections stamped `System`, preserving immutable provenance. |
