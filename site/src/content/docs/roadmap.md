...
|---|---|---|---|
| Talk to Lyra from the local terminal through the governed task path. | **Wired into Lyra** | [direct-terminal-chat](https://github.com/George-RD/openspine/blob/main/openspec/specs/direct-terminal-chat/spec.md)<br />[terminal_owner_message_composes_local_cli_grant_and_persists_input](https://github.com/George-RD/openspine/blob/main/crates/openspine-kernel/src/pipeline/tests/mod.rs) | The owner can converse locally, but this path does not yet expose recurring account work or responsibility growth. |
