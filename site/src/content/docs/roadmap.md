...
|---|---|---|---|
| Talk to Lyra from the local terminal through the governed task path. | **Wired into Lyra** | [direct-terminal-chat](https://github.com/George-RD/openspine/blob/main/openspec/specs/direct-terminal-chat/spec.md)<br />[terminal_owner_status_reaches_device_through_real_shell](https://github.com/George-RD/openspine/blob/main/crates/openspine-kernel/src/pipeline/tests/terminal_e2e.rs) | The owner can converse locally, but this path does not yet expose recurring account work or responsibility growth. |
