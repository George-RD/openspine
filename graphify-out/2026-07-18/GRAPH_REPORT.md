...
## Corpus Check
- 541 files · ~422,562 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 7481 nodes · 9719 edges · 1649 communities (546 shown, 1103 thin omitted)
- Extraction: 87% EXTRACTED · 13% INFERRED · 0% AMBIGUOUS · INFERRED: 1300 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `5d4364e2`
...
## Communities (1649 total, 1103 thin omitted)
...
### Community 4 - ".default"
Cohesion: 0.10
Nodes (41): approval_required_is_ok_not_err(), counterparty_denial_preserves_only_canonical_deferral(), deny_decision_is_ok_not_err(), generate_sends_bearer_auth(), generate_sends_untrusted_context_in_body(), get_task_deserializes_selection_tokens(), get_task_sends_bearer_auth(), submit_action_sends_bearer_auth() (+33 more)
...
Cohesion: 0.09
Nodes (48): draft_command_composes_email_preview_grant_whose_pending_ref_is_derived_message(), draft_command_for_a_missing_thread_mints_no_grant(), draft_command_for_a_real_thread_composes_a_bound_selection_grant(), draft_command_is_refused_without_the_unsafe_flag_under_process_driver(), draft_command_with_gmail_api_error_audits_no_event_received(), draft_command_without_gmail_configured_is_a_no_op(), artifact_propose_rejects_malformed_yaml(), artifact_propose_rejects_non_proposed_lifecycle() (+40 more)
...
Cohesion: 0.21
Nodes (23): digest_of(), artifact_propose_persists_and_sends_approval_button(), artifact_propose_rejects_duplicate_id_version(), injected_activation_tx_failure_keeps_approved_old_state(), seed_owner_history(), approve_callback_update(), mount_send_message_ok(), orphaned_reconfirm_fixture() (+15 more)
...
Cohesion: 0.14
Nodes (28): GrantMode, bindings_valid(), Caveat, caveat_bytes(), chain_structurally_valid(), ChainStep, compute_mac_hex(), compute_tip() (+20 more)
...
Cohesion: 0.07
Nodes (74): AgentLimits, AgentManifest, main_assistant_agent(), main_assistant_denies_broad_email_access(), MemoryScope, ModelPolicy, OutputChannels, Persistence (+66 more)
...
Cohesion: 0.12
Nodes (27): connector_failure_batches_and_is_audited(), counter_persistence_failure_is_durably_batched_as_resource(), immediate_failure_cannot_enter_digest_lane(), test_state_with_telegram(), digest_detail_corrupt_blob_surfaces_resource_without_leak(), digest_detail_immediate_success_outcome_audit_failure_is_delivery_unknown_and_no_retry(), digest_detail_missing_blob_surfaces_resource_without_leak(), digest_detail_not_found_is_truthful() (+19 more)
...
Cohesion: 0.11
Nodes (11): Fn, built_in_web_egress_endpoints(), Connector, ConnectorRegistry, EgressRegistrationError, GmailConnector, TelegramConnector, Cli (+3 more)
...
Cohesion: 0.16
Nodes (11): commissioned_worker_reports_and_master_relays_through_real_shell(), parent_briefcase(), parent_grant_for_commission(), root_pipeline_shell_does_not_report_worker_result(), worker_shell_binary(), build_state(), build_state_with_store(), repo_lyra_dir() (+3 more)
...
Cohesion: 0.13
Nodes (18): PostApprovalFuture, PostApprovalHandler, handle_draft_approval_callback(), handle_plan_approval_callback(), resolve_approved_plan(), finalize_nomination(), NominationAssertion, reinstate_artifact() (+10 more)
...
Cohesion: 0.15
Nodes (21): String, truncate_for_telegram(), truncate_with_notice(), dispatch_plan_preview(), ActionRequestBody, ActionResponseBody, cleanup_pre_effect_reservations(), dispatch_lyra_preview() (+13 more)
...
Cohesion: 0.15
Nodes (12): Consequences, Consequences, D-043 — `lyra.ui.preview` is extended (not duplicated) to propose the exact reviewed draft and attach the approval button, D-100 — Worker commissioning is an append-only caveat-chain child, Decision, Decision, Decision Index, Lyra PRD Companion — Decisions Log (+4 more)
...
Cohesion: 0.19
Nodes (7): Debug, CircuitBreakerConfig, decode_hex(), signed_at_key_and_payload(), WebhookEnvelope, WebhookRejection, WebhookVerifier
...
## Knowledge Gaps
- **2261 isolated node(s):** `set.sh script`, `autoresearch.sh script`, `TelegramReplyPayload`, `ReadThreadPayload`, `ArtifactNominatePayload` (+2256 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **1103 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `test_state()` connect `config.rs` to `Tasks: Define OpenSpine development process`, `.default`, `tests.rs`, `D-024 — OpenSpec is the development/change-management layer, not the runtime`, `banner`, `policy.rs`, `check-file-sizes.sh`, `Proposal: Define core runtime schemas`, `lib.rs`, `action.rs`, `Proposal: Backfill implemented capability specs`, `SKILL.md`, `Lifecycle`?**
  _High betweenness centrality (0.019) - this node is a cross-community bridge._
- **Why does `handle_owner_update()` connect `config.rs` to `decisions.md`, `D-005 — Private-data shell must be contained`, `.default`, `tests.rs`, `banner`, `telegram.rs`, `openspine-decision-log.md`, `OpenSpine`, `client.rs`, `Error`, `action.rs`, `Proposal: Backfill implemented capability specs`, `SKILL.md`, `Lifecycle`, `ADDED Requirements`?**
  _High betweenness centrality (0.019) - this node is a cross-community bridge._
- **Why does `gate()` connect `D-050 — `max_model_calls` is enforced with an atomic upsert, not a count-then-compare` to `Requirements`, `OpenSpine`, `digest.rs`, `action_catalog.rs`, `D-047 — Task tokens are hashed at rest; expired grants are swept`, `Tasks: Implement artifact lifecycle slice`?**
