...
## Corpus Check
- 532 files · ~415,051 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 7306 nodes · 9511 edges · 1601 communities (530 shown, 1071 thin omitted)
- Extraction: 87% EXTRACTED · 13% INFERRED · 0% AMBIGUOUS · INFERRED: 1276 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `648f6192`
...
1. `handle_owner_update()` - 93 edges
...
## Communities (1601 total, 1071 thin omitted)
...
Cohesion: 0.08
Nodes (60): draft_command_composes_email_preview_grant_whose_pending_ref_is_derived_message(), draft_command_for_a_missing_thread_mints_no_grant(), draft_command_for_a_real_thread_composes_a_bound_selection_grant(), draft_command_is_refused_without_the_unsafe_flag_under_process_driver(), draft_command_with_gmail_api_error_audits_no_event_received(), draft_command_without_gmail_configured_is_a_no_op(), artifact_propose_rejects_malformed_yaml(), artifact_propose_rejects_non_proposed_lifecycle() (+52 more)
...
Cohesion: 0.17
Nodes (27): classified_empty_output_channel_denial(), commission(), commission_is_receipt_idempotent(), commission_receipt_binding_rejects_different_parent_or_request(), commissioning_persists_briefcase_without_board_row(), gate_request(), minimal_briefcase(), narrowed_gate_worker() (+19 more)
...
Cohesion: 0.07
Nodes (55): BuildEnvelopeFn, Future, GrantBindingFn, Output, Pin, PreflightFn, RouteGuardFn, Send (+47 more)
...
Cohesion: 0.11
Nodes (60): Constraints, Policy, ArtifactId, Option, String, Vec, SessionPolicy, a_deny_route_is_never_composed() (+52 more)
...
Cohesion: 0.23
Nodes (9): String, truncate_for_telegram(), truncate_with_notice(), dispatch_plan_preview(), ProposalError, propose_draft_creation(), dispatch_lyra_preview(), PreviewPayload (+1 more)
...
Cohesion: 0.08
Nodes (39): a_failed_token_refresh_surfaces_as_an_error(), a_non_404_api_error_is_not_treated_as_missing(), connector(), fetch_thread_extracts_text_and_skips_attachments(), gmail_token_refresh_only_within_skew_window(), gmail_token_refreshes_near_expiry(), mount_token_endpoint(), rotated_vault_credentials_bypass_live_access_token_cache() (+31 more)
...
Cohesion: 0.05
Nodes (39): PostApprovalFuture, PostApprovalHandler, create_approved_draft(), handle_draft_approval_callback(), handle_plan_approval_callback(), resolve_approved_plan(), finalize_nomination(), NominationAssertion (+31 more)
...
Cohesion: 0.19
Nodes (14): effect_defaults_to_allow_when_omitted(), owner_route(), round_trips_through_serde(), Route, route_can_be_a_deny_route(), RouteActorWhen, RouteEffect, RouteResolution (+6 more)
...
Cohesion: 0.13
Nodes (13): Result, run_benchmarks(), build_selection_token(), format_pending_message(), String, Ulid, Instant, OwnedMutexGuard (+5 more)
...
Cohesion: 0.21
Nodes (7): ActionHandler, Fn, ActionHandlerRegistry, Cli, commit_post_bind_clock(), main(), shutdown_signal()
...
Cohesion: 0.15
Nodes (12): Consequences, Consequences, D-078 — Owner reconfirmation is digest-bound with a durable owner-accepted disposition, D-088 — A workflow transition writes exactly one advancing durable step, Decision, Decision, Decision Index, Lyra PRD Companion — Decisions Log (+4 more)
...
## Knowledge Gaps
- **2227 isolated node(s):** `set.sh script`, `autoresearch.sh script`, `TelegramReplyPayload`, `ReadThreadPayload`, `ArtifactNominatePayload` (+2222 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **1071 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `digest_of_bytes()` connect `digest.rs` to `Proposal: Harden approval and budgets`, `Tasks: Implement Telegram owner control slice`, `GmailConnector`, `D-026 — Shell containment via a `SandboxDriver` trait (Process dev-only / Docker)`, `MODIFIED Requirements`, `telegram.rs`, `.put`, `client.rs`, `Requirements`, `Ok`, `mod.rs`, `Requirements`, `SKILL.md`, `AuditEvent`?**
  _High betweenness centrality (0.025) - this node is a cross-community bridge._
- **Why does `test_state()` connect `config.rs` to `Tasks: Define OpenSpine development process`, `MODIFIED Requirements`, `banner`, `AGENTS.md`, `Lifecycle`, `Proposal: Define core runtime schemas`, `action.rs`, `Proposal: Backfill implemented capability specs`, `SKILL.md`, `Tasks: Implement artifact lifecycle slice`?**
  _High betweenness centrality (0.021) - this node is a cross-community bridge._
- **Why does `handle_owner_update()` connect `config.rs` to `decisions.md`, `D-005 — Private-data shell must be contained`, `Tasks: Implement Telegram owner control slice`, `SKILL.md`, `MODIFIED Requirements`, `banner`, `telegram.rs`, `client.rs`, `Proposal: Define core runtime schemas`, `action.rs`, `Implementation notes`, `Lifecycle`, `ADDED Requirements`?**
  _High betweenness centrality (0.020) - this node is a cross-community bridge._
- **Are the 88 inferred relationships involving `handle_owner_update()` (e.g. with `draft_command_composes_email_preview_grant_whose_pending_ref_is_derived_message()` and `draft_command_for_a_missing_thread_mints_no_grant()`) actually correct?**
  _`handle_owner_update()` has 88 INFERRED edges - model-reasoned connections that need verification._
- **Are the 83 inferred relationships involving `owner_update()` (e.g. with `draft_command_composes_email_preview_grant_whose_pending_ref_is_derived_message()` and `draft_command_for_a_missing_thread_mints_no_grant()`) actually correct?**
