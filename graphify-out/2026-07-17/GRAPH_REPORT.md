# Graph Report - implement-seed-workflows  (2026-07-17)

## Corpus Check
- 436 files · ~320,742 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 5793 nodes · 7291 edges · 1264 communities (431 shown, 833 thin omitted)
- Extraction: 87% EXTRACTED · 13% INFERRED · 0% AMBIGUOUS · INFERRED: 950 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `6efd8a18`
...
## Communities (1264 total, 833 thin omitted)
...
### Community 4 - ".default"
Cohesion: 0.33
Nodes (9): kind_table_excludes_templates(), kind_table_round_trips_all_six_kinds(), loads_every_real_fixture_without_error(), malformed_fixture_fails_to_load(), missing_directory_is_not_an_error(), non_yaml_files_are_ignored(), repo_lyra_dir(), load_registry() (+1 more)
...
Cohesion: 0.06
Nodes (51): Ok, Sha256, CanonicalValue, CanonicalValue<'a>, Digest, digest_from_hash(), digest_of_bytes(), digest_of_bytes_hashes_raw_content_directly() (+43 more)
...
Cohesion: 0.12
Nodes (24): materialize_missing(), parsed(), Error, Path, Result, Store, String, Vec (+16 more)
...
Cohesion: 0.30
Nodes (17): digest_of(), activation_with_mutated_payload_is_denied(), approve_callback_update(), approved_artifact_activates_into_registry_and_overlay(), model_swap_ceremony_switches_real_generate_provider(), mount_send_message_ok(), telegram_stub(), approve_callback_update() (+9 more)
...
Cohesion: 0.18
Nodes (16): BuildEnvelopeFn, GrantBindingFn, PreflightFn, RouteGuardFn, run_pipeline(), email_preview_lane(), LaneSpec, owner_control_lane() (+8 more)
...
Cohesion: 0.08
Nodes (11): activate_approved_artifact(), lifecycle_name(), lineage_from_json(), lineage_to_json(), parse_lifecycle(), ProposedArtifact, Store, ArtifactNamespace (+3 more)
...
Cohesion: 0.23
Nodes (19): dispatch_task_timer_event(), consumed_update_is_dropped_before_pipeline_dispatch(), different_bot_rotation_starts_fresh_namespace(), telegram_offset_reads_namespaced_consumed_offset_for_same_bot(), authority_granted_audit_is_atomic_with_grant_persist(), deadline_timer_reaches_routed_granted_and_worker_gate(), fires_task_timer_and_reaches_worker_gate(), linked_fired_event_after_task_cancelled_ack_skips_without_grant() (+11 more)
...
Cohesion: 0.14
Nodes (20): discard_staged_overlay_files(), load(), OverlayStartup, HashSet, Path, PathBuf, Result, Store (+12 more)
...
Cohesion: 0.20
Nodes (7): ActionHandler, OwnedMutexGuard, RwLock, GrantLimits, ActionHandlerRegistry, AppState, empty_session_policy()
...
Cohesion: 0.22
Nodes (12): PostApprovalHandler, kernel_notify_grant(), notify_owner_best_effort(), notify_owner_required_outcome(), notify_owner_with_digest(), NotifyOutcome, record_notify_skipped(), create_approved_draft() (+4 more)
...
Cohesion: 0.15
Nodes (12): Consequences, Consequences, D-046 — Grant budgets are enforced kernel-dispatch-side; the artifact budget counts only shell-initiated puts, D-050 — `max_model_calls` is enforced with an atomic upsert, not a count-then-compare, Decision, Decision, Decision Index, Lyra PRD Companion — Decisions Log (+4 more)
...
### Community 198 - "head"
Cohesion: 0.25
Nodes (8): Change Log, Consequences, D-091 — Seed workflows ship as overlay artifacts through the standard path, Decision, Open Decision Questions — CLOSED (see linked decisions), Rationale, Research / Reference Backlog, Would change if
...
## Knowledge Gaps
- **1844 isolated node(s):** `autoresearch.sh script`, `TelegramReplyPayload`, `ReadThreadPayload`, `ArtifactNominatePayload`, `ArtifactProposePayload` (+1839 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **833 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `digest_of()` connect `Requirements` to `Design: Authority composition`, `Proposal: Define OpenSpine development process`, `config.rs`, `ADDED Requirements`, `Requirements`, `Proposal: Define core runtime schemas`, `content.d.ts`, `Q: Trace the full runtime call path for a single incoming Telegram message from an authorized owner, all the way through to a TaskGrant being persisted in the kernel's SQLite store. List the modules/files involved, in call order, and identify the 2-3 modules that are most structurally central to this flow.`, `Tasks: Implement selected-thread email preview slice`, `ActionId`, `post_action`?**
  _High betweenness centrality (0.015) - this node is a cross-community bridge._
- **Why does `test_state()` connect `Proposal: Harden approval and budgets` to `D-014 — Bootstrap/setup secrets bypass shell/model context`, `identity.rs`, `ADDED Requirements`, `config.rs`, `D-024 — OpenSpec is the development/change-management layer, not the runtime`, `client.rs`, `policy.rs`, `Proposal: Define core runtime schemas`, `action.rs`, `Tasks: Implement artifact lifecycle slice`?**
  _High betweenness centrality (0.010) - this node is a cross-community bridge._
- **Why does `handle_owner_update()` connect `config.rs` to `Tasks: Implement Telegram owner control slice`, `identity.rs`, `Design: Authority composition`, `client.rs`, `policy.rs`, `digest.rs`, `action.rs`, `Requirements`, `Q: Trace the full runtime call path for a single incoming Telegram message from an authorized owner, all the way through to a TaskGrant being persisted in the kernel's SQLite store. List the modules/files involved, in call order, and identify the 2-3 modules that are most structurally central to this flow.`, `SKILL.md`?**
  _High betweenness centrality (0.010) - this node is a cross-community bridge._
