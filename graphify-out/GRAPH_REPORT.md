...
## Corpus Check
- 468 files · ~346,416 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 6342 nodes · 8168 edges · 1329 communities (484 shown, 845 thin omitted)
- Extraction: 88% EXTRACTED · 12% INFERRED · 0% AMBIGUOUS · INFERRED: 986 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `b4381f27`
...
5. `gate()` - 47 edges
6. `digest_of_bytes()` - 37 edges
7. `request_for()` - 33 edges
8. `grant_with()` - 32 edges
...
## Communities (1329 total, 845 thin omitted)
...
### Community 1 - ".new"
Cohesion: 0.07
Nodes (20): Bot, Update, Url, build_owner_envelope(), CallbackQueryUpdate, parse_digest_detail_command(), parse_digest_namespace(), project_update() (+12 more)
...
Cohesion: 0.06
Nodes (50): apply_compatibility(), dangling_for_parsed(), ensure_reconfirm_request(), exclude_orphans(), find_orphans(), missing_provenance(), OrphanedArtifact, owner_accepted_newly_dangling() (+42 more)
...
Cohesion: 0.06
Nodes (45): String, truncate_for_telegram(), truncate_with_notice(), HandlerFuture, bounded_json_response(), CachedToken, extract_email_address(), GmailConnector (+37 more)
...
Cohesion: 0.14
Nodes (22): kernel_notify_grant(), notify_owner_required(), notify_owner_required_outcome(), notify_owner_with_digest(), NotifyOutcome, record_notify_skipped(), notify_owner_required_errors_and_audits_on_send_failure(), notify_owner_required_succeeds_when_sent() (+14 more)

### Community 31 - "ADDED Requirements"
Cohesion: 0.05
Nodes (45): AgentLimits, AgentManifest, main_assistant_agent(), main_assistant_denies_broad_email_access(), MemoryScope, ModelPolicy, OutputChannels, Persistence (+37 more)
...
Cohesion: 0.14
Nodes (26): batch_failure(), encrypt_digest_detail(), FailureClass, notify_immediate_failure(), record_callback_ack(), record_connector_outcome(), record_connector_outcome_or_batch(), digest_detail_corrupt_blob_surfaces_resource_without_leak() (+18 more)
...
Cohesion: 0.15
Nodes (49): Constraints, Policy, ArtifactId, Option, String, Vec, SessionPolicy, a_deny_route_is_never_composed() (+41 more)
...
Cohesion: 0.22
Nodes (21): digest_of(), injected_activation_tx_failure_keeps_approved_old_state(), activation_with_mutated_payload_is_denied(), approve_callback_update(), approved_artifact_activates_into_registry_and_overlay(), model_swap_ceremony_switches_real_generate_provider(), mount_send_message_ok(), telegram_stub() (+13 more)
...
Cohesion: 0.23
Nodes (16): Future, Output, Pin, Send, GmailFailureClass, email_build_envelope(), email_preflight(), EventInputs (+8 more)
...
Cohesion: 0.15
Nodes (14): I, Iterator, built_in_web_egress_endpoints(), conflicting_egress_rating_is_rejected_without_downgrade(), Connector, connector_registry_enumerates_configured_connectors(), ConnectorRegistry, EgressRegistrationError (+6 more)
...
Cohesion: 0.12
Nodes (19): PostApprovalFuture, PostApprovalHandler, emit_preflight_failure(), notify_owner_best_effort(), create_approved_draft(), handle_draft_approval_callback(), activate_approved_artifact(), finalize_nomination() (+11 more)
...
Cohesion: 0.18
Nodes (24): pre_egress_mac_hex(), pre_egress_root_bytes(), bindings_valid(), Caveat, caveat_bytes(), chain_structurally_valid(), ChainStep, compute_mac_hex() (+16 more)
...
Cohesion: 0.27
Nodes (8): build_selection_token(), format_pending_message(), String, Ulid, OwnedMutexGuard, RwLock, email_grant_binding(), AppState

### Community 138 - "add_column_if_missing"
Cohesion: 0.16
Nodes (10): ActionHandler, Result, run_benchmarks(), Fn, Instant, ActionHandlerRegistry, Cli, commit_post_bind_clock() (+2 more)
...
Cohesion: 0.15
Nodes (12): Consequences, Consequences, D-065 — Provider API-key vault migration belongs to foundation amendment, D-087 — Workflow state machines are declarative with digest-bound approval authorization, Decision, Decision, Decision Index, Lyra PRD Companion — Decisions Log (+4 more)
...
Cohesion: 0.38
Nodes (8): ArtifactNominatePayload, dispatch_artifact_nominate(), approve_callback(), learned_route(), nomination_owner_tap_persists_nominated_status_and_audit(), nomination_rejects_persona_kind_before_authority_path(), nomination_requires_explicit_depersonalized_assertion(), telegram_ok()
...
## Knowledge Gaps
- **2042 isolated node(s):** `set.sh script`, `autoresearch.sh script`, `TelegramReplyPayload`, `ReadThreadPayload`, `ArtifactNominatePayload` (+2037 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **845 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `handle_owner_update()` connect `config.rs` to `Design: Authority composition`, `Lifecycle`, `banner`, `Gmail selected-thread email preview setup (Phase 2)`, `policy.rs`, `digest.rs`, `action.rs`, `Requirements`, `SKILL.md`, `Agent-OS change sequence (2026-07-07, AD canon)`, `SKILL.md`?**
  _High betweenness centrality (0.016) - this node is a cross-community bridge._
- **Why does `Identity` connect `identity.rs` to `D-010 — Model calls with private context go through model gateway`, `Gmail selected-thread email preview setup (Phase 2)`, `Tasks: Harden approval and budgets`, `Design: OpenSpine development process`, `Requirements`, `ActionId`, `Proposal: Implement Telegram owner control slice`?**
  _High betweenness centrality (0.012) - this node is a cross-community bridge._
- **Why does `digest_of_bytes()` connect `Requirements` to `Tasks: Define OpenSpine development process`, `Tasks: Implement Telegram owner control slice`, `GmailConnector`, `MODIFIED Requirements`, `client.rs`, `D-033 — Action identifiers are exact-match dotted strings; unverified senders are audited and ignored`, `D-040 — Pending (pre-approval) `ActionRequest`s are persisted in a new `action_requests` table`, `Requirements`, `Proposal: Backfill implemented capability specs`, `SKILL.md`?**
  _High betweenness centrality (0.011) - this node is a cross-community bridge._
- **Are the 71 inferred relationships involving `handle_owner_update()` (e.g. with `dispatch_polled_updates()` and `draft_command_composes_email_preview_grant_whose_pending_ref_is_derived_message()`) actually correct?**
...
- **Are the 48 inferred relationships involving `test_state_with_telegram()` (e.g. with `lyra_ui_preview_sends_telegram_reply_to_grant_bound_chat()` and `lyra_ui_preview_truncates_long_body_to_utf16_limit()`) actually correct?**
  _`test_state_with_telegram()` has 48 INFERRED edges - model-reasoned connections that need verification._
- **Are the 47 inferred relationships involving `test_state()` (e.g. with `email_counterparty_resolves_to_bound_identity_when_address_is_bound()` and `email_counterparty_stays_unresolved_when_address_is_unbound()`) actually correct?**
