# Graph Report - openspine  (2026-07-19)

## Corpus Check
- 569 files · ~446,764 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 7943 nodes · 10285 edges · 1776 communities (584 shown, 1192 thin omitted)
- Extraction: 87% EXTRACTED · 13% INFERRED · 0% AMBIGUOUS · INFERRED: 1381 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `7e592480`
...
## Communities (1776 total, 1192 thin omitted)
...
### Community 4 - ".default"
Cohesion: 0.09
Nodes (31): approval_required_is_ok_not_err(), counterparty_denial_preserves_only_canonical_deferral(), deny_decision_is_ok_not_err(), generate_sends_bearer_auth(), generate_sends_untrusted_context_in_body(), get_task_deserializes_selection_tokens(), get_task_sends_bearer_auth(), submit_action_sends_bearer_auth() (+23 more)

### Community 5 - "GmailConnector"
Cohesion: 0.20
Nodes (14): converge_owner_accepted_dependencies(), owner_accepted_missing_source_is_fail_closed(), owner_accepted_route_with_no_refs_is_compatible_after_epoch_change(), owner_accepted_route_with_removed_ref_is_newly_dangling(), reconfirm_request_reuses_unchanged_and_rotates_changed_payload(), apply_compatibility(), dangling_for_parsed(), ensure_reconfirm_request() (+6 more)
...
Cohesion: 0.10
Nodes (48): draft_command_composes_email_preview_grant_whose_pending_ref_is_derived_message(), draft_command_for_a_missing_thread_mints_no_grant(), draft_command_for_a_real_thread_composes_a_bound_selection_grant(), draft_command_is_refused_without_the_unsafe_flag_under_process_driver(), draft_command_with_gmail_api_error_audits_no_event_received(), draft_command_without_gmail_configured_is_a_no_op(), plan_preview_records_telegram_failure_counter_on_send_error(), plan_preview_records_telegram_success_counter() (+40 more)
...
Cohesion: 0.09
Nodes (70): Constraints, Policy, ArtifactId, Option, String, Vec, SessionPolicy, class_identity_is_stable_across_composition_timestamps() (+62 more)
...
Cohesion: 0.21
Nodes (22): pending_worker_dispatches(), record_worker_commissioned(), record_worker_result(), surface_stranded_worker(), classified_empty_output_channel_denial(), commission(), commission_is_receipt_idempotent(), commission_receipt_binding_rejects_different_parent_or_request() (+14 more)
...
Cohesion: 0.14
Nodes (16): emit_preflight_failure(), email_preview_lane(), owner_control_lane(), bounded_preview(), empty_session_policy(), email_lane_executed_stage_trace_matches_sync_prefix(), email_lane_marker_is_not_owner_control_screened(), email_lane_preflight_resolves_counterparty_into_persisted_briefcase() (+8 more)
...
Cohesion: 0.13
Nodes (21): RouteWhen, Connector, AccountRole, ActorHint, ChannelTrust, Connector, EventEnvelope, EventType (+13 more)
...
Cohesion: 0.09
Nodes (37): mediate_and_dispatch_action(), long_detail_is_bounded_in_summary_and_full_via_ref(), headless_lane(), HeadlessHookOutcome, HeadlessHookRequest, run_headless_hook(), headless_owner_route_binds_owner_persona_before_grant(), install_headless_route() (+29 more)
...
Cohesion: 0.13
Nodes (20): MutexGuard, ConversationClaimGuard, ConversationClaimGuard<'a>, handle_worker_commission(), claim_conversation_in_flight(), connector_restart_cap_available(), record_worker_failed(), release_conversation_in_flight() (+12 more)
...
Cohesion: 0.34
Nodes (18): artifact_ref(), email_route(), exact_deny_route_wins_over_allow_route(), gmail_connector_authenticated_alone_does_not_match_the_selected_thread_route(), higher_priority_route_wins_over_lower_priority(), matches(), no_matching_route_is_denied_not_ambiguous(), no_relationship_match_denies_the_route() (+10 more)
...
Cohesion: 0.11
Nodes (13): String, truncate_for_telegram(), truncate_with_notice(), dispatch_lyra_preview(), PreviewPayload, dispatch_plan_preview(), ProposalError, propose_draft_creation() (+5 more)
...
Cohesion: 0.35
Nodes (16): accepted_digest_tamper_invalidates(), base_namespace_is_not_treated_as_learned(), dangling_learned_route_is_orphaned_and_excluded(), insert_agent(), insert_pack(), insert_route(), insert_workflow(), learned() (+8 more)

### Community 140 - "Design: Artifact lifecycle slice"
Cohesion: 0.20
Nodes (13): RelationshipKind, effect_defaults_to_allow_when_omitted(), owner_route(), round_trips_through_serde(), Route, route_can_be_a_deny_route(), RouteActorWhen, RouteEffect (+5 more)
...
### Community 365 - "Json"
Cohesion: 0.29
Nodes (9): canonical_catalog(), counterparty_classification_is_kernel_owned_and_fails_closed(), handler_registry_requires_explicit_classification(), test_catalog_effect_paths_are_fully_enumerated_and_classified(), worker_actions_declare_no_egress_and_no_output_channel(), gate_request(), narrowed_gate_worker(), worker_allowed_exact_report_action() (+1 more)
...
## Knowledge Gaps
- **2398 isolated node(s):** `set.sh script`, `autoresearch.sh script`, `TelegramReplyPayload`, `ReadThreadPayload`, `ArtifactNominatePayload` (+2393 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **1192 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `test_state()` connect `Proposal: Define core runtime schemas` to `D-012 — Audit stores private payloads by encrypted/hash reference`, `lib.rs`, `Tasks: Define OpenSpine development process`, `.default`, `config.rs`, `Self`, `Option`, `ProposedArtifact`, `README.md`, `Approach`, `action.rs`, `Requirements`, `Proposal: Backfill implemented capability specs`, `Proposal: Harden approval and budgets`, `Proposal: Implement artifact lifecycle slice`, `Lifecycle`, `D-042 — Reply recipient is kernel-derived, never shell-supplied: newest non-owner sender, matched against a configured mailbox address`?**
  _High betweenness centrality (0.028) - this node is a cross-community bridge._
- **Why does `handle_owner_update()` connect `config.rs` to `decisions.md`, `D-005 — Private-data shell must be contained`, `.default`, `ProposedArtifact`, `client.rs`, `D-032 — Kernel↔shell transport is HTTP/JSON with a per-task bearer token`, `Proposal: Define core runtime schemas`, `Approach`, `action.rs`, `head`, `Lifecycle`, `D-042 — Reply recipient is kernel-derived, never shell-supplied: newest non-owner sender, matched against a configured mailbox address`, `ADDED Requirements`?**
  _High betweenness centrality (0.023) - this node is a cross-community bridge._
- **Why does `digest_of_bytes()` connect `digest.rs` to `D-012 — Audit stores private payloads by encrypted/hash reference`, `artifact_activation_tests.rs`, `.default`, `GmailConnector`, `Design: Authority composition`, `D-015 — Phase 1 should avoid final email send`, `.put`, `Arc`, `Design: Backfill implemented capability specs`, `client.rs`, `ProposedArtifact`, `Requirements`, `content.d.ts`, `Proposal: Define core runtime schemas`, `Proposal: Implement selected-thread email preview slice`, `.default`, `AuditEvent`?**
...
- **Are the 74 inferred relationships involving `test_state_with_telegram()` (e.g. with `lyra_ui_preview_sends_telegram_reply_to_grant_bound_chat()` and `lyra_ui_preview_truncates_long_body_to_utf16_limit()`) actually correct?**
