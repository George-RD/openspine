...
## Corpus Check
- 359 files · ~258,670 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4622 nodes · 6079 edges · 855 communities (371 shown, 484 thin omitted)
- Extraction: 88% EXTRACTED · 12% INFERRED · 0% AMBIGUOUS · INFERRED: 723 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `4dab5be1`
...
1. `handle_owner_update()` - 59 edges
2. `owner_update()` - 56 edges
3. `gate()` - 46 edges
4. `test_state_with_telegram()` - 41 edges
5. `request_for()` - 33 edges
6. `grant_with()` - 32 edges
7. `StoreError` - 31 edges
...
- `notify_owner_with_digest()` --calls--> `gate()`  [INFERRED]
  crates/openspine-kernel/src/pipeline/mod.rs → crates/openspine-gate/src/gate.rs

## Import Cycles
- None detected.

## Communities (855 total, 484 thin omitted)
...
Cohesion: 0.09
Nodes (44): ArtifactRef, bool, digest_inputs(), GatedStepDigest, Outcome, Duration, Error, Into (+36 more)
...
Cohesion: 0.18
Nodes (3): AtomicBool, Store, StoreError
...
Cohesion: 0.20
Nodes (4): From, ActionCatalog, ActionId, EffectPath
...
Cohesion: 0.06
Nodes (51): String, truncate_for_telegram(), truncate_with_notice(), HandlerFuture, ActionRequestBody, ActionResponseBody, dispatch_allowed_action(), dispatch_lyra_preview() (+43 more)
...
Cohesion: 0.30
Nodes (15): lyra_ui_preview_sends_telegram_reply_to_grant_bound_chat(), lyra_ui_preview_truncates_long_body_to_utf16_limit(), truncated_preview_carries_no_approval_button_and_persists_no_action_request(), test_state_with_gmail(), email_read_selected_thread_rejects_expired_token(), email_read_selected_thread_rejects_foreign_grant(), email_read_selected_thread_rejects_malformed_payload(), email_read_selected_thread_rejects_second_use() (+7 more)
...
Cohesion: 0.22
Nodes (9): is_unique_constraint_store_error(), is_unique_constraint_violation(), Error, Option, Result, Ulid, Store, Principal (+1 more)
...
Cohesion: 0.28
Nodes (12): seed_owner_history(), activation_with_mutated_payload_is_denied(), approve_callback_update(), approved_artifact_activates_into_registry_and_overlay(), model_swap_ceremony_switches_real_generate_provider(), mount_send_message_ok(), telegram_stub(), dispatch_artifact_propose() (+4 more)
...
Cohesion: 0.18
Nodes (13): PostApprovalFuture, PostApprovalHandler, notify_owner_best_effort(), create_approved_draft(), handle_draft_approval_callback(), handle_plan_approval_callback(), resolve_approved_plan(), activate_approved_artifact() (+5 more)
...
Cohesion: 0.22
Nodes (17): deny_unknown_fields_rejects_capability_pack_id(), EntityType, Identifier, IdentifierKind, IdentifierVerificationMethod, Identity, identity_json_has_no_authority_field(), IdentityResolution (+9 more)
...
Cohesion: 0.24
Nodes (15): RwLock, AppState, dispatch_polled_updates(), kernel_notify_grant(), notify_owner_with_digest(), NotifyOutcome, poll_telegram_once_for_test(), record_notify_skipped() (+7 more)
...
Cohesion: 0.16
Nodes (18): build_state(), build_state_with_store(), repo_lyra_dir(), test_state_with_gmail_and_telegram(), test_state_with_store(), gated_step_persisted_pending_recovers_without_redispatch(), a_double_tap_on_approve_creates_only_one_gmail_draft(), activate_approved_artifact_audits_failure_when_no_row() (+10 more)
...
Cohesion: 0.06
Nodes (59): BuildEnvelopeFn, build_selection_token(), format_pending_message(), String, Ulid, email_lane_executed_stage_trace_matches_sync_prefix(), owner_lane_executed_stage_trace_matches_sync_prefix(), owner_lane_without_verified_context_fails_closed_before_grant() (+51 more)
...
Cohesion: 0.24
Nodes (6): action_id_qualifier_is_part_of_identity(), action_id_serializes_as_bare_string(), ActionRequest, DenialReason, EffectPathClass, GateDecision
...
Cohesion: 0.25
Nodes (7): Consequences, D-041 — `email.create_draft`'s digest composition: payload = `{subject, body}`, target = `{thread_id, connector, account_role, recipients}`, Decision, Decision Index, Lyra PRD Companion — Decisions Log, Rationale, Would change if
...
## Knowledge Gaps
- **1634 isolated node(s):** `set.sh script`, `autoresearch.sh script`, `TelegramReplyPayload`, `ReadThreadPayload`, `ArtifactProposePayload` (+1629 more)
...
- **Why does `handle_owner_update()` connect `config.rs` to `D-004 — Every effectful action goes through `gate()``, `identity.rs`, `artifact_loader.rs`, `policy.rs`, `Proposal: Define core runtime schemas`, `action.rs`, `Requirements`, `Runtime schema groups`?**
  _High betweenness centrality (0.022) - this node is a cross-community bridge._
- **Why does `Digest` connect `Proposal: Backfill implemented capability specs` to `handle_owner_update`, `tests.rs`, `ADDED Requirements`, `Json`, `Tasks: Implement authority composition`, `Proposal: Implement artifact lifecycle slice`, `ADDED Requirements`?**
  _High betweenness centrality (0.020) - this node is a cross-community bridge._
- **Why does `AppState` connect `identity.rs` to `D-004 — Every effectful action goes through `gate()``, `handle_owner_update`, `Design: Authority composition`, `.default`, `ADDED Requirements`, `config.rs`, `mod.rs`, `OpenSpine`, `Design: Digest-bound draft approval`, `Tasks: Implement selected-thread email preview slice`, `Runtime schema groups`?**
...
- **Are the 39 inferred relationships involving `test_state_with_telegram()` (e.g. with `lyra_ui_preview_sends_telegram_reply_to_grant_bound_chat()` and `lyra_ui_preview_truncates_long_body_to_utf16_limit()`) actually correct?**
  _`test_state_with_telegram()` has 39 INFERRED edges - model-reasoned connections that need verification._