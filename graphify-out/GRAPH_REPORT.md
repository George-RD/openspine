...
## Corpus Check
- 494 files · ~372,567 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 6770 nodes · 8729 edges · 1463 communities (516 shown, 947 thin omitted)
- Extraction: 88% EXTRACTED · 12% INFERRED · 0% AMBIGUOUS · INFERRED: 1071 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `7020b598`
...
3. `test_state_with_telegram()` - 58 edges
4. `gate()` - 54 edges
5. `test_state()` - 50 edges
...
## Communities (1463 total, 947 thin omitted)
...
### Community 1 - ".new"
Cohesion: 0.05
Nodes (34): Bot, counterparty_resolves_identity_but_no_principal(), handle_owner_bind(), IdentityResolver, IdentityResolver<'a>, owner_verified_path_resolves_owner_principal_and_relationship(), Option, Result (+26 more)
...
### Community 4 - ".default"
Cohesion: 0.10
Nodes (11): egress_declarations(), action_id_qualifier_is_part_of_identity(), action_id_serializes_as_bare_string(), ActionCatalog, ActionEgressDeclaration, ActionId, ActionRequest, DenialReason (+3 more)
...
Cohesion: 0.71
Nodes (6): activation_with_mutated_payload_is_denied(), approve_callback_update(), approved_artifact_activates_into_registry_and_overlay(), model_swap_ceremony_switches_real_generate_provider(), mount_send_message_ok(), telegram_stub()
...
Cohesion: 0.17
Nodes (27): bindings_valid(), Caveat, caveat_bytes(), chain_structurally_valid(), ChainStep, compute_mac_hex(), compute_tip(), ct_eq_hex() (+19 more)
...
Cohesion: 0.17
Nodes (16): ActorHint, ChannelTrust, Connector, EventEnvelope, EventType, InteractionMode, legacy_without_thread_id_defaults_to_none(), round_trips_through_serde() (+8 more)

### Community 21 - "post_action"
Cohesion: 0.17
Nodes (8): ApprovalStepInputs, ArtifactRef, Digest, digest_inputs(), StepHandle, StepState, WorkflowCtx<'a>, WorkflowError
...
Cohesion: 0.15
Nodes (18): BuildEnvelopeFn, GrantBindingFn, PreflightFn, RouteGuardFn, gmail_state_with_real_thread(), email_preview_lane(), LaneSpec, owner_control_lane() (+10 more)
...
Cohesion: 0.16
Nodes (19): DeserializeOwned, (), bool, EntryTransitionInputs, GatedDepartureInputs, GatedStepDigest, Outcome, run_timer_driver() (+11 more)

### Community 45 - "digest.rs"
Cohesion: 0.14
Nodes (25): batch_failure(), encrypt_digest_detail(), FailureClass, notify_immediate_failure(), record_callback_ack(), record_connector_outcome(), record_connector_outcome_or_batch(), connector_failure_batches_and_is_audited() (+17 more)
...
Cohesion: 0.16
Nodes (16): RelationshipKind, effect_defaults_to_allow_when_omitted(), owner_route(), round_trips_through_serde(), Route, route_can_be_a_deny_route(), RouteActorWhen, RouteEffect (+8 more)
...
Cohesion: 0.16
Nodes (16): ActionRequestBody, ActionResponseBody, dispatch_lyra_preview(), dispatch_read_selected_thread(), DispatchError, FailureSurface, guard_connector_dispatch(), mediate_and_dispatch_action() (+8 more)
...
Cohesion: 0.07
Nodes (33): deny_unknown_fields_rejects_capability_pack_id(), EntityType, Identifier, IdentifierKind, IdentifierVerificationMethod, Identity, identity_json_has_no_authority_field(), IdentityResolution (+25 more)
...
Cohesion: 0.25
Nodes (4): ActionHandler, GrantLimits, ActionHandlerRegistry, empty_session_policy()
...
Cohesion: 0.16
Nodes (26): complete_task_and_wake(), dispatch_task_timer_event(), dispatch_task_wake(), reattempt_handed_off(), recover_timer_dispatches(), run_task_deadline_consumer(), TimerDispatchOutcome, test_state() (+18 more)
...
Cohesion: 0.18
Nodes (8): String, truncate_for_telegram(), truncate_with_notice(), handle_artifact_nominate(), handle_artifact_propose(), handle_plan_propose(), handle_telegram_reply(), dispatch_plan_preview()
...
Cohesion: 0.52
Nodes (11): digest_of(), seed_owner_history(), approve_callback_update(), mount_send_message_ok(), orphaned_reconfirm_fixture(), reconfirm_legacy_migration_establishes_produced_by_and_proposal(), reconfirm_tap_refuses_base_namespace_collision(), reconfirm_tap_refuses_tampered_yaml() (+3 more)
...
Cohesion: 0.07
Nodes (17): Debug, Drop, BreakerState, CircuitBreaker, CircuitBreakerConfig, ConnectorCallError, ConnectorProbePermit, ConnectorRuntime (+9 more)
...
Cohesion: 0.31
Nodes (7): commissioned_grant_for_receipt(), CommissionReceipt, mark_worker_stranded_notified(), stranded_worker_timeouts(), stranded_worker_timeout_detects_expired(), worker_dispatch_state(), WorkerDispatchState
...
Cohesion: 0.36
Nodes (7): handle_worker_commission(), WorkerBoundParameterPayload, WorkerCommissionPayload, WorkerCommissionResponse, WorkerReportPayload, WorkerRequestPayload, WorkerSlotPayload
...
## Knowledge Gaps
- **2121 isolated node(s):** `set.sh script`, `autoresearch.sh script`, `TelegramReplyPayload`, `ReadThreadPayload`, `ArtifactNominatePayload` (+2116 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **947 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `handle_owner_update()` connect `config.rs` to `decisions.md`, `banner`, `telegram.rs`, `Option`, `D-029 — Gmail OAuth scopes: `readonly` + `compose`, send hard-denied at the gate`, `digest.rs`, `action.rs`, `opsx-apply.md`, `Value`, `Design: Harden approval and budgets`, `Lifecycle`, `Agent-OS change sequence (2026-07-07, AD canon)`?**
  _High betweenness centrality (0.023) - this node is a cross-community bridge._
- **Why does `gate()` connect `D-022 — Agent-owned inbox is distinct from owner mailbox access` to `MODIFIED Requirements`, `banner`, `policy.rs`, `Proposal: Implement selected-thread email preview slice`, `Tasks: Implement artifact lifecycle slice`?**
  _High betweenness centrality (0.020) - this node is a cross-community bridge._
- **Why does `digest_of()` connect `D-029 — Gmail OAuth scopes: `readonly` + `compose`, send hard-denied at the gate` to `D-023 — OpenSpine is the substrate; Lyra is a product built on it`, `D-026 — Shell containment via a `SandboxDriver` trait (Process dev-only / Docker)`, `config.rs`, `policy.rs`, `Reflection & product surface`, `get_task`, `content.d.ts`, `Proposal: Define core runtime schemas`, `ADDED Requirements`, `opsx-apply.md`, `MockServer`, `ApprovalRecord`, `ActionId`, `Tasks: Implement selected-thread email preview slice`, `Proposal: Implement selected-thread email preview slice`, `Tasks: Define core runtime schemas`?**
  _High betweenness centrality (0.016) - this node is a cross-community bridge._
- **Are the 71 inferred relationships involving `handle_owner_update()` (e.g. with `draft_command_composes_email_preview_grant_whose_pending_ref_is_derived_message()` and `draft_command_for_a_missing_thread_mints_no_grant()`) actually correct?**
  _`handle_owner_update()` has 71 INFERRED edges - model-reasoned connections that need verification._
- **Are the 67 inferred relationships involving `owner_update()` (e.g. with `draft_command_composes_email_preview_grant_whose_pending_ref_is_derived_message()` and `draft_command_for_a_missing_thread_mints_no_grant()`) actually correct?**
  _`owner_update()` has 67 INFERRED edges - model-reasoned connections that need verification._
- **Are the 56 inferred relationships involving `test_state_with_telegram()` (e.g. with `lyra_ui_preview_sends_telegram_reply_to_grant_bound_chat()` and `lyra_ui_preview_truncates_long_body_to_utf16_limit()`) actually correct?**
  _`test_state_with_telegram()` has 56 INFERRED edges - model-reasoned connections that need verification._
- **Are the 48 inferred relationships involving `gate()` (e.g. with `handle_draft_approval_callback()` and `handle_plan_approval_callback()`) actually correct?**
  _`gate()` has 48 INFERRED edges - model-reasoned connections that need verification._