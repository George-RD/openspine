...
## Corpus Check
- 751 files · ~609,521 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 10810 nodes · 15924 edges · 1990 communities (756 shown, 1234 thin omitted)
- Extraction: 89% EXTRACTED · 11% INFERRED · 0% AMBIGUOUS · INFERRED: 1714 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `7754cdb5`
...
## Communities (1990 total, 1234 thin omitted)
...
### Community 4 - ".default"
Cohesion: 0.08
Nodes (16): Result, run_benchmarks(), action_id_qualifier_is_part_of_identity(), action_id_serializes_as_bare_string(), ActionRequest, DenialReason, EffectPathClass, GateDecision (+8 more)
...
Cohesion: 0.11
Nodes (9): connector_failure_batches_and_is_audited(), counter_persistence_failure_is_durably_batched_as_resource(), immediate_failure_cannot_enter_digest_lane(), encrypt_digest_detail(), FailureClass, notify_immediate_failure(), record_callback_ack(), record_connector_outcome() (+1 more)

### Community 7 - "config.rs"
Cohesion: 0.07
Nodes (48): audit_payload_refs(), malformed_secret_command_is_reserved_from_normal_pipeline(), non_owner_update_is_ignored_and_audited_without_a_grant(), owner_update_composes_authority_and_persists_a_grant_bound_to_the_chat(), owner_update_grant_pins_original_message_raw_ref_through_to_audit(), Store, String, Vec (+40 more)
...
Cohesion: 0.08
Nodes (35): build_state_at(), full_export_restore_e2e_regression_under_newer_base(), mint_composed_owner_grant(), mode(), Path, PathBuf, Vec, write_learned_route() (+27 more)
...
Cohesion: 0.06
Nodes (76): PromotionDenial, SkillRow, recent_promotion_decisions_for_test(), dispatch_skill_context(), skill_context_rejects_unknown_grant_purpose(), skill_context_selects_only_grant_scoped_installed_matches(), task_class_from_grant_purpose(), CeremonyError (+68 more)
...
Cohesion: 0.09
Nodes (74): handle_owner_update(), owner_update(), seed_owner_history(), test_state_with_telegram(), draft_command_composes_email_preview_grant_whose_pending_ref_is_derived_message(), draft_command_for_a_missing_thread_mints_no_grant(), draft_command_for_a_real_thread_composes_a_bound_selection_grant(), draft_command_is_refused_without_the_unsafe_flag_under_process_driver() (+66 more)
...
Cohesion: 0.13
Nodes (27): build_state(), build_state_with_store(), repo_lyra_dir(), GmailConnector, Option, PathBuf, Store, TelegramConnector (+19 more)
...
Cohesion: 0.12
Nodes (17): Connector, InteractionMode, legacy_without_thread_id_defaults_to_none(), round_trips_through_serde(), ArtifactRef, Ulid, sample_envelope(), sample_ref() (+9 more)
...
Cohesion: 0.11
Nodes (21): delegation_descriptors(), egress_declarations(), id(), Vec, Vec, ActionCatalog, ActionEgressDeclaration, ActionId (+13 more)
...
Cohesion: 0.11
Nodes (17): Consequences, Consequences, Consequences, D-013 — Dynamic behavior easy; dynamic authority hard, D-056 — Eval-store groundwork defers AD-111 evaluator policy: only the verdict-landing surface is settled, D-083 — Task dispatch is atomic with receipt-keyed fail-closed recovery, Decision, Decision (+9 more)
...
### Community 334 - "Ulid"
Cohesion: 0.10
Nodes (30): Formatter, Result, ActionDescriptor, ActionImplementationDescriptor, ActionImplementationId, ActionSemantics, BudgetWindowBounds, DarkWindowPolicy (+22 more)
...
Cohesion: 0.33
Nodes (6): Change Log, Consequences, D-152 — Capability author is a contained proposer, separate from the evidence miner, Decision, Rationale, Would change if
...
### Community 442 - "Error"
Cohesion: 0.29
Nodes (14): AgentLimits, AgentManifest, main_assistant_agent(), main_assistant_denies_broad_email_access(), MemoryScope, ModelPolicy, OutputChannels, Persistence (+6 more)
...
Cohesion: 0.36
Nodes (6): router(), commissioned_worker_reports_and_master_relays_through_real_shell(), parent_briefcase(), parent_grant_for_commission(), root_pipeline_shell_does_not_report_worker_result(), worker_shell_binary()
...
Cohesion: 0.07
Nodes (21): AccountRole, ActorHint, DataClassification, Option, String, TargetRef, TargetRefKind, ResolvedActionContext (+13 more)
...
### Community 475 - "Self"
Cohesion: 0.27
Nodes (10): PostApprovalFuture, finalize_nomination(), NominationAssertion, handle_activate_artifact(), handle_create_approved_draft(), handle_headless_approved(), handle_nominate_upstream(), handle_reconfirm_artifact() (+2 more)
...
Cohesion: 0.25
Nodes (8): empty_session_policy(), Constraints, Policy, ArtifactId, Option, String, Vec, SessionPolicy
...
Cohesion: 0.50
Nodes (7): AuthorityInput, AuthorityOutcome, classification_rank(), compose_authority(), mint_task_token(), narrow(), unknown_candidate()
...
### Community 720 - "S"
Cohesion: 0.43
Nodes (7): briefcase(), get_task(), get_task_emits_empty_output_channels_for_worker_grant(), TaskLimitsBody, TaskViewBody, worker_grant(), record_worker_commissioned()
...
## Knowledge Gaps
- **3209 isolated node(s):** `set.sh script`, `autoresearch.sh script`, `TelegramReplyPayload`, `ReadThreadPayload`, `ArtifactNominatePayload` (+3204 more)
...
- **Why does `AppState` connect `ProposedArtifact` to `.default`, `config.rs`, `mod.rs`, `Reflection & product surface`, `action.rs`, `StoreError`, `AppState`, `D-047 — Task tokens are hashed at rest; expired grants are swept`, `Lifecycle`, `D-011 — Approval must be digest-bound`, `Requirements`, `D-014 — Bootstrap/setup secrets bypass shell/model context`, `Requirements`, `D-032 — Kernel↔shell transport is HTTP/JSON with a per-task bearer token`, `decisions.md`, `autoresearch.sh`, `Proposal: Backfill implemented capability specs`, `SocketAddr`, `Telegram owner-control setup (Phase 1)`, `mod.rs`, `D-001 — Lyra is a runtime/substrate, not a single agent`, `parse_thread`?**
...
- **Why does `test_state()` connect `config.rs` to `decisions.md`, `Requirements`, `SocketAddr`, `editUrl`, `D-016 — Capability packs are candidate profiles, not live authority`, `ProposedArtifact`, `compose_authority`, `tests.rs`, `policy.rs`, `content.d.ts`, `Proposal: Define core runtime schemas`, `Proposal: Refactor kernel registries`, `opsx-propose.md`, `Proposal: Backfill implemented capability specs`, `Lifecycle`, `mod.rs`?**
