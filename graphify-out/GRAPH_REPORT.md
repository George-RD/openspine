# Graph Report - implement-disclosure-policy  (2026-07-19)

## Corpus Check
- 561 files · ~443,457 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 7795 nodes · 10486 edges · 1726 communities (571 shown, 1155 thin omitted)
- Extraction: 87% EXTRACTED · 13% INFERRED · 0% AMBIGUOUS · INFERRED: 1381 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `a335e314`
...
1. `handle_owner_update()` - 99 edges
2. `test_state()` - 99 edges
3. `owner_update()` - 86 edges
4. `test_state_with_telegram()` - 76 edges
5. `gate()` - 55 edges
6. `StoreError` - 52 edges
7. `ActionId` - 46 edges
8. `digest_of_bytes()` - 46 edges
9. `AppState` - 41 edges
10. `request_for()` - 38 edges
...
## Communities (1726 total, 1155 thin omitted)
...
Cohesion: 0.10
Nodes (11): action_id_qualifier_is_part_of_identity(), action_id_serializes_as_bare_string(), ActionCatalog, ActionEgressDeclaration, ActionId, ActionRequest, EffectPath, EffectPathClass (+3 more)
...
### Community 4 - ".default"
Cohesion: 0.17
Nodes (25): approval_required_is_ok_not_err(), counterparty_denial_preserves_only_canonical_deferral(), deny_decision_is_ok_not_err(), generate_sends_bearer_auth(), generate_sends_untrusted_context_in_body(), get_task_deserializes_selection_tokens(), get_task_sends_bearer_auth(), submit_action_sends_bearer_auth() (+17 more)

### Community 5 - "GmailConnector"
Cohesion: 0.12
Nodes (36): converge_owner_accepted_dependencies(), accepted_dependency_fingerprint_migration_is_idempotent(), accepted_dependency_fingerprint_round_trips(), base_namespace_cannot_be_recorded_as_learned(), learned_artifact_round_trip_preserves_typed_provenance(), owner_accepted_is_durable_across_round_trip(), sample(), accepted_digest_tamper_invalidates() (+28 more)
...
### Community 10 - ".put"
Cohesion: 0.05
Nodes (62): add_column_if_missing(), apply_ad_hoc_migrations(), apply_single_migration_for_test(), apply_single_migration_inner(), apply_versioned_migrations(), latest_user_version(), read_user_version(), revert_versioned_migrations_for_test() (+54 more)
...
Cohesion: 0.10
Nodes (41): canonical_json(), adding_thread_binding_without_resealing_invalidates_mac(), caveat_reorder_or_remove_invalidates_mac(), child_derives_from_parent_tip_without_root_key(), effective_egress_class_intersects_every_caveat(), egress_class_tamper_invalidates_root_mac(), gate_specific_caveats_are_explicitly_supported_only_by_upgraded_verifier(), id_parent_and_action_list_tamper_fail() (+33 more)
...
Cohesion: 0.10
Nodes (37): artifact_key_bytes(), artifact_key_round_trips_bytes(), Config, ConfigError, default_kernel_bind(), example_configs_parse_against_the_real_schema(), gmail_client_secret(), gmail_refresh_token() (+29 more)

### Community 23 - "ActionId"
Cohesion: 0.06
Nodes (60): Briefcase, BriefcaseError, BriefcaseSection, CounterpartyRef, depth(), LearnedSource, Briefcase, BTreeMap (+52 more)
...
Cohesion: 0.16
Nodes (16): empty_session_policy(), emit_preflight_failure(), email_preview_lane(), owner_control_lane(), email_lane_executed_stage_trace_matches_sync_prefix(), email_lane_marker_is_not_owner_control_screened(), email_lane_preflight_resolves_counterparty_into_persisted_briefcase(), injected_briefcase_persist_failure_leaves_no_spawn_or_orphans() (+8 more)
...
Cohesion: 0.14
Nodes (20): disclosure_slug(), carve_out_extends_covered_egress_without_new_policy(), check_egress(), ClassifiedBriefcaseItem, coverage_uses_provenance_even_when_generalized_text_is_public(), DisclosureClass, DisclosureGateDecision, DisclosurePolicyKey (+12 more)

### Community 31 - "ADDED Requirements"
Cohesion: 0.12
Nodes (26): injected_activation_tx_failure_keeps_approved_old_state(), connector_failure_batches_and_is_audited(), counter_persistence_failure_is_durably_batched_as_resource(), immediate_failure_cannot_enter_digest_lane(), digest_detail_corrupt_blob_surfaces_resource_without_leak(), digest_detail_immediate_success_outcome_audit_failure_is_delivery_unknown_and_no_retry(), digest_detail_missing_blob_surfaces_resource_without_leak(), digest_detail_not_found_is_truthful() (+18 more)
...
Cohesion: 0.18
Nodes (19): allow_and_suppressed_do_not_surface(), approval_required_surfaces_as_deferral_with_approval_missing(), denial_reason_code(), deny_surfaces_canonical_deferral_and_escalation(), escalation_notice_round_trips(), EscalationEvent, EscalationNotice, EscalationPayload (+11 more)
...
Cohesion: 0.17
Nodes (22): action_for_egress(), action_for_scope(), cancel_reservations(), collect_nested_strings(), nested_json_sensitive_term_extraction_redacts_all_strings(), prepared_query_mints_consumes_once_and_verifies_digest(), DisclosureError, egress_slug() (+14 more)
...
Cohesion: 0.10
Nodes (19): ActionHandler, AppState, bounded_preview(), mint_reconfirm_grant(), Arc, Digest, Duration, HashMap (+11 more)
...
Cohesion: 0.17
Nodes (5): ArtifactNamespace, ArtifactRef, can_transition(), Lifecycle, activate_approved_artifact()
...
Cohesion: 0.19
Nodes (12): relationship_slug(), disclosure_policy_round_trips_through_store(), DisclosurePendingQuestion, ensure_schema(), Connection, Digest, Option, Result (+4 more)
...
Cohesion: 0.06
Nodes (51): authenticate(), bearer_token(), internal_error(), unauthorized(), briefcase(), get_task(), get_task_emits_empty_output_channels_for_worker_grant(), TaskLimitsBody (+43 more)
...
Cohesion: 0.06
Nodes (40): counterparty_resolves_identity_but_no_principal(), handle_owner_bind(), IdentityResolver, IdentityResolver<'a>, owner_verified_path_resolves_owner_principal_and_relationship(), unknown_resolves_to_relationship_unknown_confidence_0_and_no_write(), deny_unknown_fields_rejects_capability_pack_id(), EntityType (+32 more)
...
Cohesion: 0.19
Nodes (13): binding_matches_rejects_a_different_provenance_set(), binding_matches_rejects_a_different_requesting_grant(), DisclosureCarveOut, DisclosurePolicy, DisclosureProvenance, prepared_query(), PreparedQuery, PreparedQueryRef (+5 more)
...
Cohesion: 0.13
Nodes (8): truncate_for_telegram(), truncate_with_notice(), dispatch_plan_preview(), handle_artifact_nominate(), handle_artifact_propose(), handle_lyra_preview(), handle_plan_propose(), handle_telegram_reply()
...
Cohesion: 0.15
Nodes (12): Consequences, Consequences, D-012 — Audit stores private payloads by encrypted/hash reference, D-055 — Gate trusted paths are hardened: carve-outs are enumerated catalog data; KernelOrigin is approval-exempt, audit-never-exempt; selection-token validation lives in pure gate() with dispatch-side consumption; digests are kernel-re-derived at approval-effect time, Decision, Decision, Decision Index, Lyra PRD Companion — Decisions Log (+4 more)
...
Cohesion: 0.23
Nodes (10): MutexGuard, handle_worker_commission(), commissioned_grant_for_receipt(), CommissionReceipt, mark_worker_stranded_notified(), stranded_worker_timeouts(), stranded_worker_timeout_detects_expired(), worker_dispatch_state() (+2 more)
...
## Knowledge Gaps
- **2305 isolated node(s):** `autoresearch.sh script`, `ArtifactNominatePayload`, `ArtifactProposePayload`, `ReadThreadPayload`, `ConversationClaimGuard<'a>` (+2300 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **1155 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `gate()` connect `D-050 — `max_model_calls` is enforced with an atomic upsert, not a count-then-compare` to `OpenSpine`, `autoresearch.sh`, `D-048 — `artifact.activate` is the single canonical activation action id; every runtime proposal requires uniform owner approval; prompt templates are excluded from proposable kinds`, `content.d.ts`, `Proposal: Implement selected-thread email preview slice`, `AppState`, `Implementation plan`, `Tasks: Implement artifact lifecycle slice`, `D-042 — Reply recipient is kernel-derived, never shell-supplied: newest non-owner sender, matched against a configured mailbox address`?**
  _High betweenness centrality (0.022) - this node is a cross-community bridge._
- **Why does `AppState` connect `Requirements` to `event.rs`, `Design: Authority composition`, `config.rs`, `Requirements`, `create_approved_draft`, `Design: Gate action API`, `content.d.ts`, `mod.rs`, `ApprovalRecord`, `Requirements`, `AppState`, `Tasks: Implement artifact lifecycle slice`, `D-049 — Capability specs are backfilled for subsystems implemented inside earlier slices`, `architecture.md`?**
  _High betweenness centrality (0.020) - this node is a cross-community bridge._
- **Why does `handle_owner_update()` connect `config.rs` to `decisions.md`, `D-005 — Private-data shell must be contained`, `Tasks: Define OpenSpine development process`, `banner`, `D-028 — Canonical artifact format is YAML+serde; digests are `sha256:` over canonical JSON`, `ProposedArtifact`, `client.rs`, `properties`, `policy.rs`, `Proposal: Define core runtime schemas`, `Approach`, `action.rs`, `head`, `Lifecycle`, `ADDED Requirements`, `D-042 — Reply recipient is kernel-derived, never shell-supplied: newest non-owner sender, matched against a configured mailbox address`, `Requirements`?**
  _High betweenness centrality (0.019) - this node is a cross-community bridge._
- **Are the 90 inferred relationships involving `handle_owner_update()` (e.g. with `email_preview_lane()` and `owner_control_lane()`) actually correct?**
  _`handle_owner_update()` has 90 INFERRED edges - model-reasoned connections that need verification._
- **Are the 97 inferred relationships involving `test_state()` (e.g. with `caller_omitted_class_is_still_enforced()` and `disclosure_envelope_budget_exhaustion_blocks()`) actually correct?**
...
- **Are the 74 inferred relationships involving `test_state_with_telegram()` (e.g. with `blocked_disclosure_releases_reserved_envelope_budget()` and `enforcement_allows_after_owner_answer()`) actually correct?**
  _`test_state_with_telegram()` has 74 INFERRED edges - model-reasoned connections that need verification._