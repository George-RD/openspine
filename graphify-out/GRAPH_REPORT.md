...
## Corpus Check
- 334 files · ~236,917 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4236 nodes · 5486 edges · 789 communities (355 shown, 434 thin omitted)
- Extraction: 89% EXTRACTED · 11% INFERRED · 0% AMBIGUOUS · INFERRED: 630 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `9aec3419`
...
1. `gate()` - 47 edges
2. `handle_owner_update()` - 44 edges
3. `owner_update()` - 41 edges
4. `request_for()` - 33 edges
5. `grant_with()` - 32 edges
6. `test_catalog()` - 30 edges
7. `StoreError` - 30 edges
8. `test_state()` - 29 edges
9. `Digest` - 25 edges
10. `test_state_with_telegram()` - 23 edges
...
## Communities (789 total, 434 thin omitted)
...
Cohesion: 0.09
Nodes (15): Update, build_owner_envelope(), CallbackQueryUpdate, project_update(), TelegramUpdate, configured_owner_text_message_is_verified(), invalid_candidate_bot_token_is_rejected_without_replacing_live_bot(), missing_sender_is_ignored() (+7 more)
...
### Community 8 - "mod.rs"
Cohesion: 0.14
Nodes (23): RouteActorWhen, RouteWhen, Connector, Option, AccountRole, ActorHint, ChannelTrust, Connector (+15 more)
...
Cohesion: 0.21
Nodes (26): http_model_swap_propose_persists_allow_before_provider_effect(), approval_required_action_stops_before_dispatch(), counterparty_denial_returns_deferral_routes_and_audits(), counterparty_escalation_failure_is_not_reported_as_success(), email_read_inbox_is_denied_for_owner_control_grant(), host_filesystem_read_and_write_are_denied_for_owner_control_grant(), model_swap_propose_reaches_http_gate_and_audits_failure(), network_raw_egress_is_denied_for_owner_control_grant() (+18 more)
...
Cohesion: 0.16
Nodes (9): JudgeDenial, GateDenial, GateEvidence, JudgePassed, ReplayPassed, run_gate(), run_model_swap_gate(), evaluate() (+1 more)
...
Cohesion: 0.15
Nodes (17): BuildEnvelopeFn, email_preview_lane(), emit_preflight_failure(), LaneSpec, owner_control_lane(), PipelineStage, Result, Timestamp (+9 more)
...
Cohesion: 0.15
Nodes (14): Lifecycle, effect_defaults_to_allow_when_omitted(), owner_route(), round_trips_through_serde(), Route, route_can_be_a_deny_route(), RouteEffect, RouteResolution (+6 more)
...
Cohesion: 0.23
Nodes (15): activation_with_mutated_payload_is_denied(), approve_callback_update(), approved_artifact_activates_into_registry_and_overlay(), model_swap_ceremony_switches_real_generate_provider(), mount_send_message_ok(), telegram_stub(), dispatch_artifact_propose(), injected_activation_tx_failure_keeps_approved_old_state() (+7 more)
...
Cohesion: 0.25
Nodes (14): RwLock, AppState, dispatch_polled_updates(), kernel_notify_grant(), notify_owner_best_effort(), notify_owner_required(), poll_telegram_once_for_test(), run_telegram_poll_loop() (+6 more)
...
Cohesion: 0.19
Nodes (14): parse_audit_event(), Value, shadow_grant_effect_suppressed_skips_effect_handler(), test_path_1_notify_owner_gated_and_audited(), test_path_2_create_draft_payload_mutated_audited(), test_path_3_activate_artifact_failure_audited(), test_path_4_read_selected_thread_gated_and_audited(), test_path_5_preview_gated_and_audited() (+6 more)

### Community 63 - "Requirements"
Cohesion: 0.22
Nodes (17): ArtifactRef, deny_unknown_fields_rejects_capability_pack_id(), EntityType, Identifier, IdentifierKind, IdentifierVerificationMethod, Identity, identity_json_has_no_authority_field() (+9 more)
...
### Community 80 - "Ok"
Cohesion: 0.26
Nodes (7): HasherWriter<'a>, Error, Formatter, Result, S, Ok, Write
...
Cohesion: 0.13
Nodes (15): canonical_json(), CanonicalValue, digest_from_hash(), digest_matches_hash(), digest_of(), digest_of_bytes(), digest_of_bytes_hashes_raw_content_directly(), digest_of_is_a_pinned_golden_value() (+7 more)
...
Cohesion: 0.25
Nodes (7): Consequences, D-018 — Routes are declarative artifacts, not kernel code, Decision, Decision Index, Lyra PRD Companion — Decisions Log, Rationale, Would change if
...
Cohesion: 0.21
Nodes (11): lyra_ui_preview_sends_telegram_reply_to_grant_bound_chat(), lyra_ui_preview_truncates_long_body_to_utf16_limit(), truncated_preview_carries_no_approval_button_and_persists_no_action_request(), owner_notify_routes_through_gate_and_audits(), plan_proposal_budget_exhaustion_persists_no_request(), plan_propose_approve_rederives_gate_and_resolves(), proposed_plan_fixture(), tampered_plan_artifact_is_refused_at_approval_callback() (+3 more)
...
Cohesion: 0.19
Nodes (13): Constraints, Policy, ArtifactId, Option, String, Vec, SessionPolicy, allow_and_deny_lists_never_overlap_in_the_fixture() (+5 more)
...
## Knowledge Gaps
- **1549 isolated node(s):** `set.sh script`, `autoresearch.sh script`, `TelegramReplyPayload`, `ReadThreadPayload`, `ArtifactProposePayload` (+1544 more)
...
- **Why does `handle_owner_update()` connect `policy.rs` to `Requirements`, `SKILL.md`, `action.rs`, `Requirements`, `D-054 — Pipeline stages are a typed compiled-in sequence the driver executes; lanes are compiled-in data records`, `Runtime schema groups`?**
  _High betweenness centrality (0.017) - this node is a cross-community bridge._
- **Why does `AppState` connect `Runtime schema groups` to `properties`, `.default`, `Requirements`, `email_grant_binding`, `Tasks: Harden approval and budgets`, `policy.rs`, `ADDED Requirements`, `mod.rs`, `Tasks: Implement selected-thread email preview slice`, `Proposal: Implement Telegram owner control slice`?**
  _High betweenness centrality (0.016) - this node is a cross-community bridge._
- **Why does `AuditKind` connect `OpenSpine Agent-OS Design Log` to `Requirements`?**
  _High betweenness centrality (0.010) - this node is a cross-community bridge._
- **Are the 41 inferred relationships involving `gate()` (e.g. with `bound_parameter_conflict_is_caveat_widening()` and `explicit_shell_deny_precedes_uncovered_egress_class()`) actually correct?**
  _`gate()` has 41 INFERRED edges - model-reasoned connections that need verification._
- **Are the 40 inferred relationships involving `handle_owner_update()` (e.g. with `activation_with_mutated_payload_is_denied()` and `approved_artifact_activates_into_registry_and_overlay()`) actually correct?**
  _`handle_owner_update()` has 40 INFERRED edges - model-reasoned connections that need verification._
- **Are the 40 inferred relationships involving `owner_update()` (e.g. with `approve_callback_update()` and `activation_with_mutated_payload_is_denied()`) actually correct?**
