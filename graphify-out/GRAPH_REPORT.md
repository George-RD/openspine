...
## Corpus Check
- 559 files · ~436,672 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 7732 nodes · 9961 edges · 1738 communities (566 shown, 1172 thin omitted)
- Extraction: 87% EXTRACTED · 13% INFERRED · 0% AMBIGUOUS · INFERRED: 1331 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `2bac9aa1`
...
1. `handle_owner_update()` - 95 edges
2. `test_state()` - 87 edges
3. `owner_update()` - 86 edges
4. `test_state_with_telegram()` - 70 edges
5. `gate()` - 55 edges
...
## Communities (1738 total, 1172 thin omitted)
...
Cohesion: 0.08
Nodes (58): draft_command_composes_email_preview_grant_whose_pending_ref_is_derived_message(), draft_command_for_a_missing_thread_mints_no_grant(), draft_command_for_a_real_thread_composes_a_bound_selection_grant(), draft_command_is_refused_without_the_unsafe_flag_under_process_driver(), draft_command_with_gmail_api_error_audits_no_event_received(), draft_command_without_gmail_configured_is_a_no_op(), plan_preview_records_telegram_failure_counter_on_send_error(), concurrent_owner_callbacks_are_serialized_by_handler() (+50 more)
...
Cohesion: 0.06
Nodes (50): canonical_json(), Briefcase, adding_thread_binding_without_resealing_invalidates_mac(), caveat_reorder_or_remove_invalidates_mac(), child_derives_from_parent_tip_without_root_key(), effective_egress_class_intersects_every_caveat(), egress_class_tamper_invalidates_root_mac(), gate_specific_caveats_are_explicitly_supported_only_by_upgraded_verifier() (+42 more)
...
Cohesion: 0.18
Nodes (26): counterparty_cannot_select_owner_route_persona(), owner_bound_number_selects_route_persona(), persona(), resolve_persona(), route(), route_context_mismatch_cannot_front_persona(), artifact_ref(), email_route() (+18 more)
...
Cohesion: 0.06
Nodes (62): BTreeSet, cross_connection_topup_race_resolved_by_transaction(), denied_topup_replay_is_rejected_before_evaluating(), deterministic_pack_bytes(), email_pack_fails_without_counterparty_address(), grant(), grant_with(), independent_grants_with_identical_semantics_pack_byte_identical() (+54 more)
...
Cohesion: 0.15
Nodes (23): PromptTemplate, load_keyed(), load_registry_into(), artifact_identity_pairs(), artifact_version(), ArtifactKindSpec, ArtifactLoadError, ArtifactRegistry (+15 more)
...
Cohesion: 0.12
Nodes (21): MutexGuard, ConversationClaimGuard, ConversationClaimGuard<'a>, handle_worker_commission(), worker_dispatch_state(), WorkerDispatchState, claim_conversation_in_flight(), connector_restart_cap_available() (+13 more)
...
Cohesion: 0.18
Nodes (13): Constraints, Policy, ArtifactId, Option, String, Vec, SessionPolicy, DataClassification (+5 more)
...
Cohesion: 0.27
Nodes (12): canonical_catalog_covers_all_fixture_action_ids(), load_registry(), parse_proposal(), generic_overlay_loader_excludes_persona_and_base_loader_rejects_fixture(), kind_table_excludes_personas(), kind_table_excludes_templates(), kind_table_round_trips_all_seven_kinds(), loads_every_real_fixture_without_error() (+4 more)
...
Cohesion: 0.15
Nodes (12): Consequences, Consequences, D-043 — `lyra.ui.preview` is extended (not duplicated) to propose the exact reviewed draft and attach the approval button, D-046 — Grant budgets are enforced kernel-dispatch-side; the artifact budget counts only shell-initiated puts, Decision, Decision, Decision Index, Lyra PRD Companion — Decisions Log (+4 more)
...
Cohesion: 0.32
Nodes (6): commissioned_grant_for_receipt(), CommissionReceipt, mark_worker_stranded_notified(), stranded_worker_timeouts(), stranded_worker_timeout_detects_expired(), worker_dispatch_failed()
...
Cohesion: 0.15
Nodes (21): gated_step_persisted_pending_recovers_without_redispatch(), a_double_tap_on_approve_creates_only_one_gmail_draft(), activate_approved_artifact_audits_failure_when_no_row(), approval_audit_never_contains_the_plaintext_draft_body(), approval_fixture_grant(), approval_fixture_request(), approve_callback_update(), draft_write_timeout_is_delivery_unknown_and_leaves_pending_rows() (+13 more)
...
## Knowledge Gaps
- **2334 isolated node(s):** `set.sh script`, `autoresearch.sh script`, `TelegramReplyPayload`, `ReadThreadPayload`, `ArtifactNominatePayload` (+2329 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **1172 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `handle_owner_update()` connect `config.rs` to `decisions.md`, `Tasks: Define OpenSpine development process`, `banner`, `openspine-decision-log.md`, `ProposedArtifact`, `client.rs`, `policy.rs`, `Proposal: Define core runtime schemas`, `Approach`, `action.rs`, `Lifecycle`, `D-042 — Reply recipient is kernel-derived, never shell-supplied: newest non-owner sender, matched against a configured mailbox address`, `ADDED Requirements`?**
  _High betweenness centrality (0.028) - this node is a cross-community bridge._
- **Why does `test_state()` connect `config.rs` to `lib.rs`, `Tasks: Define OpenSpine development process`, `D-024 — OpenSpec is the development/change-management layer, not the runtime`, `banner`, `Option`, `ProposedArtifact`, `README.md`, `policy.rs`, `Proposal: Define core runtime schemas`, `action.rs`, `Proposal: Backfill implemented capability specs`, `Proposal: Harden approval and budgets`, `Lifecycle`, `D-042 — Reply recipient is kernel-derived, never shell-supplied: newest non-owner sender, matched against a configured mailbox address`?**
  _High betweenness centrality (0.026) - this node is a cross-community bridge._
- **Why does `digest_of()` connect `Design: Backfill implemented capability specs` to `D-005 — Private-data shell must be contained`, `Option`, `ProposedArtifact`, `client.rs`, `actions.rs`, `Reflection & product surface`, `Requirements`, `Proposal: Define core runtime schemas`, `Proposal: Implement gate action API`, `attrs`, `ApprovalRecord`, `ActionId`, `Requirements`, `AppState`?**
...
- **Are the 85 inferred relationships involving `test_state()` (e.g. with `draft_command_without_gmail_configured_is_a_no_op()` and `artifact_propose_rejects_non_proposed_lifecycle()`) actually correct?**
...
- **Are the 68 inferred relationships involving `test_state_with_telegram()` (e.g. with `lyra_ui_preview_sends_telegram_reply_to_grant_bound_chat()` and `lyra_ui_preview_truncates_long_body_to_utf16_limit()`) actually correct?**
  _`test_state_with_telegram()` has 68 INFERRED edges - model-reasoned connections that need verification._