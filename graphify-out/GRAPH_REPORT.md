...
## Corpus Check
- 481 files · ~354,340 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 6527 nodes · 8403 edges · 1376 communities (494 shown, 882 thin omitted)
- Extraction: 88% EXTRACTED · 12% INFERRED · 0% AMBIGUOUS · INFERRED: 1018 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `17c8fb62`
...
## Communities (1376 total, 882 thin omitted)
...
Cohesion: 0.07
Nodes (64): connector_registry_enumerates_configured_connectors(), EmptyGateContext, gmail(), gmail_failure_records_breaker_failure_and_opens_breaker(), no_egress_classifier_skips_check_for_unrated_actions(), registry_rate_buckets_are_isolated_per_connector(), search_class_pack_cannot_submit_web_form(), bound_parameter_conflict_is_caveat_widening() (+56 more)
...
Cohesion: 0.17
Nodes (26): digest_of(), activation_with_mutated_payload_is_denied(), approve_callback_update(), approved_artifact_activates_into_registry_and_overlay(), model_swap_ceremony_switches_real_generate_provider(), mount_send_message_ok(), telegram_stub(), ArtifactProposePayload (+18 more)
...
Cohesion: 0.12
Nodes (24): BuildEnvelopeFn, GrantBindingFn, PreflightFn, RouteGuardFn, gmail_state_with_real_thread(), complete_task_and_wake(), dispatch_task_wake(), reattempt_handed_off() (+16 more)
...
Cohesion: 0.11
Nodes (27): injected_activation_tx_failure_keeps_approved_old_state(), ProposalError, batch_failure(), encrypt_digest_detail(), FailureClass, notify_immediate_failure(), record_callback_ack(), record_connector_outcome() (+19 more)
...
Cohesion: 0.17
Nodes (17): String, truncate_for_telegram(), truncate_with_notice(), ActionRequestBody, ActionResponseBody, dispatch_lyra_preview(), dispatch_read_selected_thread(), DispatchError (+9 more)
...
Cohesion: 0.18
Nodes (18): a_double_tap_on_approve_creates_only_one_gmail_draft(), approval_audit_never_contains_the_plaintext_draft_body(), approval_fixture_grant(), approval_fixture_request(), approve_callback_update(), activate_approved_artifact_audits_failure_when_no_row(), owner_notify_routes_through_gate_and_audits(), payload_mutated_since_approval_is_denied_and_creates_no_draft() (+10 more)
...
Cohesion: 0.09
Nodes (23): admit_spend(), breach_message(), counted_model_generate(), guard_connector(), guard_connector_for(), prior_day_recovery_message_is_truthful(), recover_pending_breach_alerts(), SpendLane (+15 more)
...
Cohesion: 0.18
Nodes (8): ActionHandler, Result, run_benchmarks(), Instant, OwnedMutexGuard, RwLock, ActionHandlerRegistry, AppState
...
Cohesion: 0.20
Nodes (11): PostApprovalFuture, activate_approved_artifact(), finalize_nomination(), NominationAssertion, handle_activate_artifact(), handle_create_approved_draft(), handle_nominate_upstream(), handle_reconfirm_artifact() (+3 more)
...
Cohesion: 0.17
Nodes (14): PostApprovalHandler, handle_draft_approval_callback(), kernel_notify_grant(), notify_owner_best_effort(), notify_owner_required_outcome(), notify_owner_with_digest(), NotifyOutcome, record_notify_skipped() (+6 more)
...
Cohesion: 0.15
Nodes (3): ConsumerCheckpoint, EventSubscriptionFilter, filter_defaults_match_all()

### Community 160 - "D-012 — Audit stores private payloads by encrypted/hash reference"
Cohesion: 0.15
Nodes (12): Consequences, Consequences, D-082 — Task-board timer consumption is transactionally idempotent, D-097 — Persona overlay loading is admission-gated, Decision, Decision, Decision Index, Lyra PRD Companion — Decisions Log (+4 more)
...
## Knowledge Gaps
- **2073 isolated node(s):** `set.sh script`, `autoresearch.sh script`, `TelegramReplyPayload`, `ReadThreadPayload`, `ArtifactNominatePayload` (+2068 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **882 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `digest_of_bytes()` connect `Requirements` to `Tasks: Implement Telegram owner control slice`, `GmailConnector`, `MODIFIED Requirements`, `telegram.rs`, `client.rs`, `D-033 — Action identifiers are exact-match dotted strings; unverified senders are audited and ignored`, `Proposal: Implement gate action API`, `ApprovalRecord`, `SKILL.md`, `mod.rs`?**
...
- **Why does `Task` connect `archive.md` to `ConnectorRegistry`, `.put`, `Gmail selected-thread email preview setup (Phase 2)`, `Delegation & containment`, `apply.md`?**
  _High betweenness centrality (0.017) - this node is a cross-community bridge._
- **Why does `handle_owner_update()` connect `config.rs` to `decisions.md`, `D-005 — Private-data shell must be contained`, `banner`, `telegram.rs`, `Gmail selected-thread email preview setup (Phase 2)`, `digest.rs`, `SKILL.md`, `action.rs`, `Value`, `Lifecycle`, `Agent-OS change sequence (2026-07-07, AD canon)`?**
