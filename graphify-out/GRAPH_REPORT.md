...
## Corpus Check
- 633 files · ~506,871 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 9008 nodes · 11863 edges · 2063 communities (639 shown, 1424 thin omitted)
- Extraction: 86% EXTRACTED · 14% INFERRED · 0% AMBIGUOUS · INFERRED: 1656 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `ef23a29d`
...
## Communities (2063 total, 1424 thin omitted)
...
### Community 10 - ".put"
Cohesion: 0.09
Nodes (26): PostApprovalFuture, PostApprovalHandler, emit_preflight_failure(), activate_approved_artifact(), finalize_nomination(), NominationAssertion, ArtifactNamespace, handle_draft_approval_callback() (+18 more)

### Community 11 - "ProposedArtifact"
Cohesion: 0.17
Nodes (24): artifact_propose_persists_and_sends_approval_button(), artifact_propose_rejects_duplicate_id_version(), artifact_propose_rejects_malformed_yaml(), artifact_propose_rejects_persona_kind(), injected_activation_tx_failure_keeps_approved_old_state(), seed_owner_history(), approve_callback_update(), mount_send_message_ok() (+16 more)
...
Cohesion: 0.09
Nodes (39): epoch_nanos_to_timestamp(), PendingScheduleCtx, StandingRule, StandingRulePendingAction, Store, timestamp_to_epoch_nanos(), CounterpartyEraseError, erase_counterparty() (+31 more)
...
Cohesion: 0.05
Nodes (96): AgentLimits, AgentManifest, main_assistant_agent(), main_assistant_denies_broad_email_access(), MemoryScope, ModelPolicy, OutputChannels, Persistence (+88 more)
...
Cohesion: 0.19
Nodes (15): counterparty_cannot_select_owner_route_persona(), owner_bound_number_selects_route_persona(), persona(), resolve_persona(), route(), route_context_mismatch_cannot_front_persona(), PipelineStage, run_pipeline_with_envelope() (+7 more)
...
Cohesion: 0.17
Nodes (37): allowed_action_returns_allow(), allowed_plus_approval_required_returns_approval_required(), allowed_plus_denied_returns_deny(), approval_required_action_does_not_execute(), approval_required_action_returns_approval_required(), approved_but_payload_changed_since_is_denied_not_reasked(), audit_metadata_records_action_grant_and_refs_not_plaintext(), denial_audit_metadata_still_carries_refs() (+29 more)
...
Cohesion: 0.16
Nodes (18): BuildEnvelopeFn, GrantBindingFn, PreflightFn, RouteGuardFn, email_build_envelope(), email_grant_binding(), email_preflight(), EventInputs (+10 more)
...
Cohesion: 0.08
Nodes (60): draft_command_composes_email_preview_grant_whose_pending_ref_is_derived_message(), draft_command_for_a_missing_thread_mints_no_grant(), draft_command_for_a_real_thread_composes_a_bound_selection_grant(), draft_command_is_refused_without_the_unsafe_flag_under_process_driver(), draft_command_with_gmail_api_error_audits_no_event_received(), draft_command_without_gmail_configured_is_a_no_op(), artifact_propose_rejects_non_proposed_lifecycle(), artifact_propose_rejects_template_kind() (+52 more)
...
Cohesion: 0.31
Nodes (7): stranded_worker_timeout_detects_expired(), commissioned_grant_for_receipt(), CommissionReceipt, mark_worker_stranded_notified(), stranded_worker_timeouts(), worker_dispatch_state(), WorkerDispatchState
...
## Knowledge Gaps
- **2574 isolated node(s):** `set.sh script`, `autoresearch.sh script`, `TelegramReplyPayload`, `ReadThreadPayload`, `ArtifactNominatePayload` (+2569 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **1424 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `test_state()` connect `Tasks: Implement identity store and principal` to `artifact_loader.rs`, `config.rs`, `banner`, `ProposedArtifact`, `Skills & workflows`, `action.rs`, `D-043 — `lyra.ui.preview` is extended (not duplicated) to propose the exact reviewed draft and attach the approval button`, `AppState`, `Lifecycle`, `Proposal: Refactor kernel registries`, `D-013 — Dynamic behavior easy; dynamic authority hard`, `Delegation & containment`, `D-035 — Kernel advertises a separate `advertise_endpoint` from its `bind_addr`; no Unix-domain-socket transport for `ProcessDriver``, `Reference hunt`, `standing_rule_timer.rs`, `Proposal: Define core runtime schemas`, `Proposal: Backfill implemented capability specs`, `Tasks: Define OpenSpine development process`, `Path`, `mod.rs`, `SKILL.md`?**
  _High betweenness centrality (0.029) - this node is a cross-community bridge._
- **Why does `handle_owner_update()` connect `Tasks: Implement identity store and principal` to `decisions.md`, `D-005 — Private-data shell must be contained`, `.default`, `D-022 — Agent-owned inbox is distinct from owner mailbox access`, `artifact_loader.rs`, `D-016 — Capability packs are candidate profiles, not live authority`, `standing_rule_timer.rs`, `.put`, `ProposedArtifact`, `Proposal: Define core runtime schemas`, `action.rs`, `Lifecycle`, `mod.rs`, `SKILL.md`?**
  _High betweenness centrality (0.019) - this node is a cross-community bridge._
- **Why does `digest_of_bytes()` connect `Failure surfacing & operations` to `Option`, `.default`, `GmailConnector`, `D-016 — Capability packs are candidate profiles, not live authority`, `openspine-decision-log.md`, `standing_rule_timer.rs`, `ProposedArtifact`, `digest.rs`, `D-034 — `email.create_draft` is the one canonical action id; the qualified PRD §10.2 spelling is dropped`, `README.md`, `content.d.ts`, `D-035 — Kernel advertises a separate `advertise_endpoint` from its `bind_addr`; no Unix-domain-socket transport for `ProcessDriver``, `Proposal: Implement selected-thread email preview slice`, `opsx-propose.md`, `ArtifactRef`, `AppState`, `AuditEvent`, `D-006 — Identity is not authority`?**
  _High betweenness centrality (0.019) - this node is a cross-community bridge._
- **Are the 109 inferred relationships involving `test_state()` (e.g. with `draft_command_without_gmail_configured_is_a_no_op()` and `artifact_propose_rejects_non_proposed_lifecycle()`) actually correct?**
...
- **Are the 77 inferred relationships involving `test_state_with_telegram()` (e.g. with `lyra_ui_preview_sends_telegram_reply_to_grant_bound_chat()` and `lyra_ui_preview_truncates_long_body_to_utf16_limit()`) actually correct?**
