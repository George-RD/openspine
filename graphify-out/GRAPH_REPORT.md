...
## Corpus Check
- 218 files · ~153,158 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2684 nodes · 4490 edges · 297 communities (265 shown, 32 thin omitted)
- Extraction: 87% EXTRACTED · 13% INFERRED · 0% AMBIGUOUS · INFERRED: 593 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `a4055743`
...
6. `Digest` - 33 edges
7. `gate()` - 28 edges
8. `owner_update()` - 28 edges
9. `ArtifactRef` - 28 edges
10. `EventEnvelope` - 28 edges
...
- `a_deny_route_is_never_composed()` --calls--> `owner_event()`  [INFERRED]
  crates/openspine-authority/tests/compose.rs → crates/openspine-authority/src/route.rs
...
### Community 1 - ".new"
Cohesion: 0.06
Nodes (78): a_failed_token_refresh_surfaces_as_an_error(), a_non_404_api_error_is_not_treated_as_missing(), connector(), fetch_thread_extracts_text_and_skips_attachments(), mount_token_endpoint(), GmailConnector, MockServer, Value (+70 more)

### Community 2 - "event.rs"
Cohesion: 0.11
Nodes (30): classification_rank(), refuses_external_communication_without_containment(), AccountRole, ActorHint, Connector, DataClassification, EventEnvelope, EventType (+22 more)
...
### Community 4 - ".default"
Cohesion: 0.13
Nodes (40): AuditMeta, gate(), GateContext, GateOutcome, resolve(), resolve_approval_required(), Option, Timestamp (+32 more)
...
Cohesion: 0.10
Nodes (44): GateDecision, denied_read_thread_stops_without_drafting(), Draft, draft_reply(), empty_draft_skips_preview_without_error(), format_thread_for_model(), format_thread_for_model_includes_all_fields(), full_flow_reads_drafts_and_previews() (+36 more)
...
Cohesion: 0.28
Nodes (37): compose_authority(), Timestamp, artifact_ref(), email_event(), email_reply_drafter_agent(), email_route(), empty_session_policy(), global_policy() (+29 more)
...
Cohesion: 0.25
Nodes (9): Connection, Error, Option, Result, String, Ulid, Vec, Store (+1 more)
...
Cohesion: 0.20
Nodes (12): canonical_json(), CanonicalValue, digest_from_hash(), digest_of(), digest_of_bytes(), digest_of_bytes_hashes_raw_content_directly(), digest_of_is_a_pinned_golden_value(), digest_round_trips_through_serde() (+4 more)
...
### Community 50 - "mod.rs"
Cohesion: 0.09
Nodes (19): canonical_catalog(), id(), Result, run_benchmarks(), action_id_qualifier_is_part_of_identity(), action_id_serializes_as_bare_string(), ActionCatalog, ActionId (+11 more)
...
Cohesion: 0.12
Nodes (20): empty_session_policy(), handle_thread_selection(), Option, Result, Self, actor_hint_defaults_to_all_none(), allow_and_deny_lists_never_overlap_in_the_fixture(), CapabilityPack (+12 more)
...
Cohesion: 0.23
Nodes (13): Box, AuthorityInput, AuthorityOutcome, mint_task_token(), narrow(), AgentManifest, CapabilityPack, HashSet (+5 more)
...
Cohesion: 0.19
Nodes (19): resolve_owner_identity(), ChannelTrust, deny_unknown_fields_rejects_capability_pack_id(), EntityType, Identifier, IdentifierKind, IdentifierVerificationMethod, Identity (+11 more)
...
Cohesion: 0.11
Nodes (21): genesis_digest(), Mutex, Path, ArtifactRef, AuditEvent, genesis_hash(), round_trips_through_serde(), Option (+13 more)
...
Cohesion: 0.23
Nodes (8): effect_defaults_to_allow_when_omitted(), owner_route(), round_trips_through_serde(), route_can_be_a_deny_route(), RouteEffect, RouteResolution, ArtifactId, String
...
- **Why does `AppState` connect `AppState` to `.new`, `ConnectorRegistry`, `handle_owner_update`, `sandbox.rs`, `.with_api_url`, `artifact_loader.rs`, `artifact_propose.rs`, `ActionRequest`, `.put`, `mod.rs`, `actions.rs`, `Digest`, `mod.rs`, `post_action`, `AppState`, `.default`, `AuditMeta`, `AuditEvent`?**
  _High betweenness centrality (0.020) - this node is a cross-community bridge._
- **Why does `TaskGrant` connect `TaskGrant` to `compose_authority`, `.new`, `handle_owner_update`, `.default`, `.with_api_url`, `artifact_activation_tests.rs`, `artifact_propose.rs`, `ProposedArtifact`, `actions.rs`, `mod.rs`, `post_action`, `AppState`, `AppState`, `route.rs`, `AuditMeta`, `ArtifactRef`, `.default`?**
  _High betweenness centrality (0.018) - this node is a cross-community bridge._
- **Why does `ActionId` connect `mod.rs` to `compose_authority`, `.new`, `artifact_activation_tests.rs`, `.default`, `ProposedArtifact`, `actions.rs`, `post_action`, `AppState`, `TaskGrant`, `ArtifactRef`, `.default`?**
  _High betweenness centrality (0.017) - this node is a cross-community bridge._
- **Are the 32 inferred relationships involving `handle_owner_update()` (e.g. with `activation_with_mutated_payload_is_denied()` and `approved_artifact_activates_into_registry_and_overlay()`) actually correct?**
  _`handle_owner_update()` has 32 INFERRED edges - model-reasoned connections that need verification._
