...
## Corpus Check
- 278 files · ~195,647 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3520 nodes · 4934 edges · 537 communities (300 shown, 237 thin omitted)
- Extraction: 91% EXTRACTED · 9% INFERRED · 0% AMBIGUOUS · INFERRED: 426 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f299c814`
...
1. `AppState` - 47 edges
2. `gate()` - 44 edges
3. `StoreError` - 37 edges
4. `handle_owner_update()` - 31 edges
5. `request_for()` - 30 edges
6. `test_catalog()` - 30 edges
7. `TaskGrant` - 30 edges
8. `grant_with()` - 29 edges
9. `EventEnvelope` - 27 edges
10. `owner_update()` - 26 edges
...
## Communities (537 total, 237 thin omitted)

### Community 0 - "README.md"
Cohesion: 0.06
Nodes (59): artifact_ref(), email_route(), exact_deny_route_wins_over_allow_route(), gmail_connector_authenticated_alone_does_not_match_the_selected_thread_route(), higher_priority_route_wins_over_lower_priority(), matches(), no_matching_route_is_denied_not_ambiguous(), no_relationship_match_denies_the_route() (+51 more)

### Community 2 - "event.rs"
Cohesion: 0.10
Nodes (54): ActionOrigin, AuditMeta, chain_valid(), EgressClassifier, gate(), GateContext, GateOutcome, NoEgress (+46 more)

### Community 3 - "handle_owner_update"
Cohesion: 0.08
Nodes (47): String, truncate_for_telegram(), truncate_with_notice(), canonical_json(), ActionRequestBody, ActionResponseBody, dispatch_allowed_action(), dispatch_lyra_preview() (+39 more)
...
Cohesion: 0.07
Nodes (45): authenticate(), bearer_token(), get_status(), internal_error(), router(), Arc, Display, HeaderMap (+37 more)

### Community 6 - "artifact_loader.rs"
Cohesion: 0.06
Nodes (45): AgentManifest, ArtifactKindSpec, ArtifactLoadError, ArtifactRegistry, CapabilityPack, collide_keyed(), collide_route(), find_kind_spec() (+37 more)
...
Cohesion: 0.13
Nodes (19): Bot, build_owner_envelope(), CallbackQueryUpdate, parse_approve_callback(), parse_bind_command(), parse_draft_command(), project_update(), Option (+11 more)
...
Cohesion: 0.07
Nodes (67): denied_read_thread_stops_without_drafting(), Draft, draft_reply(), empty_draft_skips_preview_without_error(), format_thread_for_model(), format_thread_for_model_includes_all_fields(), full_flow_reads_drafts_and_previews(), no_selection_tokens_is_an_error() (+59 more)
...
Cohesion: 0.14
Nodes (23): AccountRole, ActorHint, Connector, DataClassification, EventEnvelope, EventType, InteractionMode, round_trips_through_serde() (+15 more)
...
Cohesion: 0.12
Nodes (51): can_transition(), Lifecycle, Constraints, Policy, ArtifactId, Option, String, Vec (+43 more)
...
Cohesion: 0.08
Nodes (21): Result, run_benchmarks(), I, Instant, Iterator, built_in_web_egress_endpoints(), conflicting_egress_rating_is_rejected_without_downgrade(), Connector (+13 more)

### Community 25 - "AppState"
Cohesion: 0.27
Nodes (16): ActionHandler, ActionHandlerRegistry, handle_artifact_propose(), handle_lyra_preview(), handle_read_selected_thread(), handle_setup_workflow_start(), handle_status_read(), handle_telegram_reply() (+8 more)
...
Cohesion: 0.15
Nodes (27): artifact_propose_persists_and_sends_approval_button(), artifact_propose_rejects_duplicate_id_version(), artifact_propose_rejects_malformed_yaml(), artifact_propose_rejects_non_proposed_lifecycle(), artifact_propose_rejects_template_kind(), artifact_propose_rejects_unknown_kind(), route_yaml(), String (+19 more)
...
Cohesion: 0.06
Nodes (34): AgentLimits, AgentManifest, main_assistant_agent(), main_assistant_denies_broad_email_access(), MemoryScope, ModelPolicy, OutputChannels, Persistence (+26 more)
...
Cohesion: 0.32
Nodes (6): create_approved_draft(), handle_draft_approval_callback(), empty_session_policy(), kernel_notify_grant(), notify_owner_best_effort(), run_telegram_poll_loop()
...
## Knowledge Gaps
- **1296 isolated node(s):** `set.sh script`, `autoresearch.sh script`, `TelegramReplyPayload`, `ReadThreadPayload`, `ArtifactProposePayload` (+1291 more)
...
- **Why does `AppState` connect `AppState` to `scripts`, `sandbox.rs`, `.default`, `GmailConnector`, `ADDED Requirements`, `D-017 — Personas grant no authority`, `artifact_loader.rs`, `mod.rs`, `Delegation & containment`, `Requirements`, `ActionId`, `AppState`, `ADDED Requirements`?**
  _High betweenness centrality (0.023) - this node is a cross-community bridge._
- **Why does `DenialReason` connect `client.rs` to `event.rs`, `ADDED Requirements`, `OpenSpine Agent-OS Design Log`?**
...
- **Are the 12 inferred relationships involving `request_for()` (e.g. with `bound_parameter_conflict_is_caveat_widening()` and `explicit_shell_deny_precedes_uncovered_egress_class()`) actually correct?**
  _`request_for()` has 12 INFERRED edges - model-reasoned connections that need verification._
- **What connects `set.sh script`, `autoresearch.sh script`, `TelegramReplyPayload` to the rest of the system?**
