...
## Corpus Check
- 408 files · ~300,064 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 5402 nodes · 7005 edges · 1064 communities (428 shown, 636 thin omitted)
- Extraction: 88% EXTRACTED · 12% INFERRED · 0% AMBIGUOUS · INFERRED: 874 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `c94138c7`
...
## Communities (1064 total, 636 thin omitted)
...
Cohesion: 0.22
Nodes (15): Future, Output, Pin, Send, emit_preflight_failure(), EventInputs, PreflightFailure, email_build_envelope() (+7 more)
...
Cohesion: 0.13
Nodes (22): RouteActorWhen, RouteWhen, Connector, Option, AccountRole, ActorHint, ChannelTrust, Connector (+14 more)
...
Cohesion: 0.22
Nodes (18): OwnedMutexGuard, RwLock, AppState, complete_task_and_wake(), dispatch_polled_updates(), dispatch_task_wake(), kernel_notify_grant(), notify_owner_required() (+10 more)
...
Cohesion: 0.06
Nodes (53): canonical_json(), CanonicalValue, CanonicalValue<'a>, Digest, digest_from_hash(), digest_matches_hash(), digest_of(), digest_of_bytes() (+45 more)
...
Cohesion: 0.16
Nodes (19): String, truncate_for_telegram(), truncate_with_notice(), ActionRequestBody, ActionResponseBody, dispatch_allowed_action(), dispatch_lyra_preview(), dispatch_read_selected_thread() (+11 more)
...
Cohesion: 0.15
Nodes (15): BuildEnvelopeFn, GrantBindingFn, PreflightFn, RouteGuardFn, email_preview_lane(), LaneSpec, owner_control_lane(), run_pipeline() (+7 more)
...
Cohesion: 0.12
Nodes (9): ArtifactId, String, Vec, WorkflowManifest, activate_approved_artifact(), ArtifactNamespace, ArtifactRef, can_transition() (+1 more)
...
Cohesion: 0.16
Nodes (16): PostApprovalFuture, PostApprovalHandler, create_approved_draft(), handle_draft_approval_callback(), finalize_nomination(), NominationAssertion, handle_activate_artifact(), handle_create_approved_draft() (+8 more)
...
## Knowledge Gaps
- **1822 isolated node(s):** `set.sh script`, `autoresearch.sh script`, `TelegramReplyPayload`, `ReadThreadPayload`, `ArtifactNominatePayload` (+1817 more)
...
- **Why does `digest_of_bytes()` connect `sandbox.rs` to `GmailConnector`, `OpenSpine conventions`, `mod.rs`, `Ok`, `Requirements`, `Q: Trace the full runtime call path for a single incoming Telegram message from an authorized owner, all the way through to a TaskGrant being persisted in the kernel's SQLite store. List the modules/files involved, in call order, and identify the 2-3 modules that are most structurally central to this flow.`, `Tasks: Implement selected-thread email preview slice`?**
  _High betweenness centrality (0.025) - this node is a cross-community bridge._
- **Why does `mediate_and_dispatch_action()` connect `digest.rs` to `event.rs`, `sandbox.rs`, `handle_owner_update`, `artifact_loader.rs`, `config.rs`, `ProposedArtifact`, `owner_event`, `Tasks: Define core runtime schemas`?**
  _High betweenness centrality (0.017) - this node is a cross-community bridge._
- **Why does `canonical_json()` connect `sandbox.rs` to `README.md`, `handle_owner_update`, `Design: Authority composition`, `digest.rs`?**
  _High betweenness centrality (0.017) - this node is a cross-community bridge._
- **Are the 67 inferred relationships involving `handle_owner_update()` (e.g. with `concurrent_owner_callbacks_are_serialized_by_handler()` and `audit_append_failure_fails_notification_before_connector_effect()`) actually correct?**
  _`handle_owner_update()` has 67 INFERRED edges - model-reasoned connections that need verification._
- **Are the 65 inferred relationships involving `owner_update()` (e.g. with `audit_append_failure_fails_notification_before_connector_effect()` and `audit_readonly_failure_fails_notification_before_connector_effect()`) actually correct?**
