...
## Corpus Check
- 444 files · ~323,678 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 5926 nodes · 7730 edges · 1200 communities (443 shown, 757 thin omitted)
- Extraction: 88% EXTRACTED · 12% INFERRED · 0% AMBIGUOUS · INFERRED: 951 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `62d101b5`
...
## Communities (1200 total, 757 thin omitted)
...
Cohesion: 0.11
Nodes (37): apply_compatibility(), dangling_for_parsed(), ensure_reconfirm_request(), exclude_orphans(), find_orphans(), missing_provenance(), OrphanedArtifact, owner_accepted_newly_dangling() (+29 more)
...
Cohesion: 0.16
Nodes (17): AccountRole, ActorHint, Connector, DataClassification, EventEnvelope, EventType, InteractionMode, legacy_without_thread_id_defaults_to_none() (+9 more)
...
Cohesion: 0.05
Nodes (59): canonical_json(), bindings_valid(), Caveat, caveat_bytes(), chain_structurally_valid(), ChainStep, compute_mac_hex(), compute_tip() (+51 more)
...
Cohesion: 0.17
Nodes (15): effect_defaults_to_allow_when_omitted(), owner_route(), round_trips_through_serde(), Route, route_can_be_a_deny_route(), RouteActorWhen, RouteEffect, RouteResolution (+7 more)
...
Cohesion: 0.18
Nodes (8): ActionHandler, Fn, ActionHandlerRegistry, Cli, commit_post_bind_clock(), main(), shutdown_signal(), empty_session_policy()
...
Cohesion: 0.06
Nodes (36): PostApprovalFuture, PostApprovalHandler, kernel_notify_grant(), notify_owner_best_effort(), notify_owner_required_outcome(), notify_owner_with_digest(), NotifyOutcome, record_notify_skipped() (+28 more)
...
Cohesion: 0.15
Nodes (12): Consequences, Consequences, D-034 — `email.create_draft` is the one canonical action id; the qualified PRD §10.2 spelling is dropped, D-038 — `resolve_owner_identity`'s `channel_trust` is caller-supplied, not hardcoded, Decision, Decision, Decision Index, Lyra PRD Companion — Decisions Log (+4 more)
...
## Knowledge Gaps
- **1920 isolated node(s):** `set.sh script`, `autoresearch.sh script`, `TelegramReplyPayload`, `ReadThreadPayload`, `ArtifactNominatePayload` (+1915 more)
...
- **Why does `handle_owner_update()` connect `config.rs` to `Tasks: Implement Telegram owner control slice`, `Design: Authority composition`, `digest.rs`, `policy.rs`, `actions.rs`, `action.rs`, `Requirements`, `Q: Trace the full runtime call path for a single incoming Telegram message from an authorized owner, all the way through to a TaskGrant being persisted in the kernel's SQLite store. List the modules/files involved, in call order, and identify the 2-3 modules that are most structurally central to this flow.`, `HeaderMap`, `SKILL.md`?**
  _High betweenness centrality (0.019) - this node is a cross-community bridge._
- **Why does `ActionId` connect `ADDED Requirements` to `SKILL.md`, `Design: OpenSpine development process`?**
  _High betweenness centrality (0.015) - this node is a cross-community bridge._
- **Why does `digest_of_bytes()` connect `Requirements` to `Design: Authority composition`, `GmailConnector`, `MODIFIED Requirements`, `telegram.rs`, `add_column_if_missing`, `ADDED Requirements`, `Requirements`, `Q: Trace the full runtime call path for a single incoming Telegram message from an authorized owner, all the way through to a TaskGrant being persisted in the kernel's SQLite store. List the modules/files involved, in call order, and identify the 2-3 modules that are most structurally central to this flow.`, `Tasks: Implement selected-thread email preview slice`, `Agent-OS change sequence (2026-07-07, AD canon)`?**
