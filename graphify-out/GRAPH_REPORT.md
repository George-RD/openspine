...
## Corpus Check
- 627 files · ~500,803 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 8907 nodes · 11742 edges · 2044 communities (634 shown, 1410 thin omitted)
- Extraction: 86% EXTRACTED · 14% INFERRED · 0% AMBIGUOUS · INFERRED: 1637 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `272ba1e9`
...
1. `test_state()` - 108 edges
2. `handle_owner_update()` - 96 edges
...
- `handle_plan_propose()` --calls--> `dispatch_plan_preview()`  [INFERRED]
  crates/openspine-kernel/src/api/handler_registry.rs → crates/openspine-kernel/src/api/plan.rs

## Import Cycles
- None detected.

## Communities (2044 total, 1410 thin omitted)
...
Cohesion: 0.08
Nodes (48): dispatch_task_timer_event(), mediate_and_dispatch_action(), test_state(), bind_email_identity(), email_counterparty_resolves_to_bound_identity_when_address_is_bound(), email_counterparty_stays_unresolved_when_address_is_unbound(), minimal_grant(), mint_topup_grant() (+40 more)
...
Cohesion: 0.09
Nodes (58): draft_command_composes_email_preview_grant_whose_pending_ref_is_derived_message(), draft_command_for_a_missing_thread_mints_no_grant(), draft_command_for_a_real_thread_composes_a_bound_selection_grant(), draft_command_is_refused_without_the_unsafe_flag_under_process_driver(), draft_command_with_gmail_api_error_audits_no_event_received(), draft_command_without_gmail_configured_is_a_no_op(), plan_preview_records_telegram_failure_counter_on_send_error(), plan_preview_records_telegram_success_counter() (+50 more)
...
Cohesion: 0.09
Nodes (32): ActionHandler, Fn, TcpListener, ActionHandlerRegistry, append_overlay_finalization_audits(), bind_clock_and_finalize_overlay(), bind_kernel_listener(), build_provider_pool() (+24 more)
...
Cohesion: 0.09
Nodes (72): AgentLimits, AgentManifest, main_assistant_agent(), main_assistant_denies_broad_email_access(), MemoryScope, ModelPolicy, OutputChannels, Persistence (+64 more)
...
Cohesion: 0.11
Nodes (34): empty_session_policy(), counterparty_cannot_select_owner_route_persona(), owner_bound_number_selects_route_persona(), persona(), resolve_persona(), route(), route_context_mismatch_cannot_front_persona(), artifact_ref() (+26 more)
...
Cohesion: 0.08
Nodes (32): PostApprovalHandler, emit_preflight_failure(), handle_draft_approval_callback(), kernel_notify_grant(), notify_owner_best_effort(), notify_owner_required(), notify_owner_required_outcome(), notify_owner_with_digest() (+24 more)
...
Cohesion: 0.05
Nodes (22): connector_failure_batches_and_is_audited(), counter_persistence_failure_is_durably_batched_as_resource(), immediate_failure_cannot_enter_digest_lane(), ConsumerError, IdempotentConsumer, LedgerEntry, PersistedConsumerState, Store (+14 more)
...
Cohesion: 0.15
Nodes (10): Update, owner_envelope_is_verified_with_owner_id_match_method(), build_owner_envelope(), CallbackQueryUpdate, DisclosureAnswer, parse_digest_detail_command(), parse_digest_namespace(), parse_disclosure_command() (+2 more)
...
Cohesion: 0.15
Nodes (12): Consequences, Consequences, D-088 — A workflow transition writes exactly one advancing durable step, D-108 — Commissioned workers must be effectively able to report, Decision, Decision, Decision Index, Lyra PRD Companion — Decisions Log (+4 more)
...
Cohesion: 0.31
Nodes (8): String, truncate_for_telegram(), truncate_with_notice(), dispatch_lyra_preview(), PreviewPayload, handle_lyra_preview(), dispatch_plan_preview(), propose_draft_creation()
...
Cohesion: 0.20
Nodes (4): handle_artifact_nominate(), handle_artifact_propose(), handle_plan_propose(), handle_telegram_reply()
...
## Knowledge Gaps
- **2520 isolated node(s):** `set.sh script`, `autoresearch.sh script`, `TelegramReplyPayload`, `ReadThreadPayload`, `ArtifactNominatePayload` (+2515 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **1410 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `digest_of_bytes()` connect `Failure surfacing & operations` to `.default`, `GmailConnector`, `SocketAddr`, `config.rs`, `.put`, `ProposedArtifact`, `content.d.ts`, `ApprovalRecord`, `D-013 — Dynamic behavior easy; dynamic authority hard`, `D-029 — Gmail OAuth scopes: `readonly` + `compose`, send hard-denied at the gate`, `digest.rs`, `Change quiz`, `ConnectorRegistry`, `README.md`, `Store`, `Proposal: Implement selected-thread email preview slice`, `AuditEvent`, `Option`, `openspine-decision-log.md`, `opsx-apply.md`?**
  _High betweenness centrality (0.027) - this node is a cross-community bridge._
- **Why does `test_state()` connect `config.rs` to `.default`, `ProposedArtifact`, `client.rs`, `D-037 — Gmail OAuth via a plain refresh-token POST (no `oauth2` crate); `base64` promoted from transitive to direct dependency`, `action.rs`, `D-044 — Approved draft creation dispatches kernel-side; no new shell spawn`, `ADDED Requirements`, `Proposal: Refactor kernel registries`, `Lifecycle`, `D-018 — Routes are declarative artifacts, not kernel code`, `Change quiz`, `Reference hunt`, `ConnectorRegistry`, `Proposal: Define core runtime schemas`, `Store`, `provenance`, `Proposal: Backfill implemented capability specs`, `Tasks: Define OpenSpine development process`, `Tasks: Implement digest-bound draft approval`, `mod.rs`?**
  _High betweenness centrality (0.024) - this node is a cross-community bridge._
- **Why does `handle_owner_update()` connect `ProposedArtifact` to `decisions.md`, `Tasks: Implement digest-bound draft approval`, `ConnectorRegistry`, `.default`, `D-022 — Agent-owned inbox is distinct from owner mailbox access`, `config.rs`, `client.rs`, `Proposal: Define core runtime schemas`, `action.rs`, `Proposal: Backfill implemented capability specs`, `Lifecycle`, `mod.rs`?**
  _High betweenness centrality (0.018) - this node is a cross-community bridge._
- **Are the 106 inferred relationships involving `test_state()` (e.g. with `draft_command_without_gmail_configured_is_a_no_op()` and `artifact_propose_rejects_non_proposed_lifecycle()`) actually correct?**
  _`test_state()` has 106 INFERRED edges - model-reasoned connections that need verification._
- **Are the 91 inferred relationships involving `handle_owner_update()` (e.g. with `draft_command_composes_email_preview_grant_whose_pending_ref_is_derived_message()` and `draft_command_for_a_missing_thread_mints_no_grant()`) actually correct?**
  _`handle_owner_update()` has 91 INFERRED edges - model-reasoned connections that need verification._
- **Are the 86 inferred relationships involving `owner_update()` (e.g. with `draft_command_composes_email_preview_grant_whose_pending_ref_is_derived_message()` and `draft_command_for_a_missing_thread_mints_no_grant()`) actually correct?**
