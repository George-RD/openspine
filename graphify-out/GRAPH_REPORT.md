# Graph Report - implement-task-board  (2026-07-17)

## Corpus Check
- 400 files · ~296,912 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 5257 nodes · 7127 edges · 1029 communities (408 shown, 621 thin omitted)
- Extraction: 88% EXTRACTED · 12% INFERRED · 0% AMBIGUOUS · INFERRED: 873 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `d3b6596a`
...
1. `StoreError` - 81 edges
2. `handle_owner_update()` - 76 edges
3. `owner_update()` - 66 edges
4. `AppState` - 55 edges
5. `test_state_with_telegram()` - 49 edges
6. `gate()` - 46 edges
7. `test_state()` - 42 edges
8. `Timestamp` - 34 edges
...
- `telegram_offset_reads_namespaced_consumed_offset_for_same_bot()` --calls--> `test_state()`  [INFERRED]
  crates/openspine-kernel/src/pipeline/tests/offset.rs → crates/openspine-kernel/src/test_support.rs
- `different_bot_rotation_starts_fresh_namespace()` --calls--> `test_state()`  [INFERRED]
  crates/openspine-kernel/src/pipeline/tests/offset.rs → crates/openspine-kernel/src/test_support.rs
...
## Communities (1029 total, 621 thin omitted)
...
Cohesion: 0.06
Nodes (28): DeserializeOwned, complete_u32(), divergent_inputs_fail_closed(), ledger_corruption_fails_closed(), ModelOutput, (), ApprovalStepInputs, ArtifactRef (+20 more)
...
Cohesion: 0.13
Nodes (42): handle_owner_update(), audit_payload_refs(), draft_command_composes_email_preview_grant_whose_pending_ref_is_derived_message(), draft_command_for_a_missing_thread_mints_no_grant(), draft_command_for_a_real_thread_composes_a_bound_selection_grant(), draft_command_is_refused_without_the_unsafe_flag_under_process_driver(), draft_command_with_gmail_api_error_audits_no_event_received(), draft_command_without_gmail_configured_is_a_no_op() (+34 more)
...
Cohesion: 0.06
Nodes (50): dependency_wake_requires_all_dependencies_and_unblocks_task(), ref_of(), ArtifactRef, all_slice_categories_are_owner_scoped(), anchored_slice_includes_not_yet_due_focal_task_and_honors_limit_one(), insert_task_rejects_prepopulated_timer_ids(), master_slice_is_bounded_and_excludes_task_detail(), master_slice_is_owner_scoped() (+42 more)
...
Cohesion: 0.21
Nodes (27): EventInputs, PreflightFailure, String, Ulid, email_build_envelope(), email_grant_binding(), email_preflight(), email_route_guard() (+19 more)
...
Cohesion: 0.15
Nodes (31): AppState, complete_task_and_wake(), dispatch_polled_updates(), dispatch_task_wake(), kernel_notify_grant(), mint_reconfirm_grant(), notify_owner_required(), notify_owner_required_outcome() (+23 more)

### Community 24 - "AppState"
Cohesion: 0.12
Nodes (27): AccountRole, ActorHint, ChannelTrust, Connector, DataClassification, EventEnvelope, EventType, InteractionMode (+19 more)

### Community 25 - "AppState"
Cohesion: 0.24
Nodes (3): From, ActionCatalog, ActionId

### Community 26 - ".sweep_expired_grants"
Cohesion: 0.18
Nodes (16): ensure_schema(), epoch_nanos_to_timestamp(), map_slice_row(), ArtifactRef, Connection, Option, Result, Self (+8 more)

### Community 27 - "Lifecycle"
Cohesion: 0.07
Nodes (42): HandlerFuture, truncate_for_telegram(), truncate_with_notice(), handle_artifact_nominate(), handle_artifact_propose(), handle_lyra_preview(), handle_plan_propose(), handle_read_selected_thread() (+34 more)
...
Cohesion: 0.16
Nodes (5): AuditEvent, AuditKind, AuditKindError, genesis_hash(), round_trips_through_serde()
...
### Community 50 - "mod.rs"
Cohesion: 0.23
Nodes (8): is_unique_constraint_store_error(), is_unique_constraint_violation(), Error, Option, Result, Ulid, Store, Principal
...
Cohesion: 0.23
Nodes (8): DependencyWake, parse_ulid(), ArtifactRef, Option, Result, Ulid, Vec, Store

### Community 53 - "Requirements"
Cohesion: 0.22
Nodes (22): digest_of(), artifact_propose_persists_and_sends_approval_button(), artifact_propose_rejects_duplicate_id_version(), injected_activation_tx_failure_keeps_approved_old_state(), activation_with_mutated_payload_is_denied(), approve_callback_update(), approved_artifact_activates_into_registry_and_overlay(), model_swap_ceremony_switches_real_generate_provider() (+14 more)
...
### Community 74 - ".put"
Cohesion: 0.25
Nodes (17): dispatch_task_timer_event(), authority_granted_audit_is_atomic_with_grant_persist(), deadline_timer_reaches_routed_granted_and_worker_gate(), fires_task_timer_and_reaches_worker_gate(), linked_fired_event_after_task_cancelled_ack_skips_without_grant(), linked_fired_event_after_task_done_ack_skips_without_grant(), recovery_refuses_receiptless_handed_off_redispatch(), ref_of() (+9 more)
...
Cohesion: 0.16
Nodes (26): Source, effect_defaults_to_allow_when_omitted(), owner_route(), round_trips_through_serde(), Route, route_can_be_a_deny_route(), RouteActorWhen, RouteEffect (+18 more)
...
Cohesion: 0.17
Nodes (12): BuildEnvelopeFn, emit_preflight_failure(), LaneSpec, Option, Result, Vec, run_pipeline(), scheduled_internal_lane() (+4 more)
...
Cohesion: 0.19
Nodes (7): action_id_qualifier_is_part_of_identity(), action_id_serializes_as_bare_string(), ActionRequest, DenialReason, EffectPath, EffectPathClass, GateDecision
...
Cohesion: 0.22
Nodes (8): email_preview_lane(), owner_control_lane(), email_lane_executed_stage_trace_matches_sync_prefix(), non_immediate_lane_breach_blocks_composition_and_notifies_owner(), owner_lane_executed_stage_trace_matches_sync_prefix(), owner_lane_without_verified_context_fails_closed_before_grant(), gmail_state_with_real_thread(), MockServer
...
Cohesion: 0.05
Nodes (42): notify_owner_best_effort(), PostApprovalFuture, PostApprovalHandler, create_approved_draft(), handle_draft_approval_callback(), activate_approved_artifact(), finalize_nomination(), NominationAssertion (+34 more)
...
Cohesion: 0.23
Nodes (8): allow_and_deny_lists_never_overlap_in_the_fixture(), CapabilityPack, owner_control_basic_pack(), round_trips_through_serde(), ArtifactId, Vec, EgressClass, Lifecycle
...
## Knowledge Gaps
- **1736 isolated node(s):** `autoresearch.sh script`, `TelegramReplyPayload`, `ReadThreadPayload`, `ArtifactNominatePayload`, `ArtifactProposePayload` (+1731 more)
...
- **Why does `AppState` connect `ActionId` to `D-004 — Every effectful action goes through `gate()``, `properties`, `config.rs`, `mod.rs`, `openspine-decision-log.md`, `.put`, `.put`, `D-026 — Shell containment via a `SandboxDriver` trait (Process dev-only / Docker)`, `actions.rs`, `Ok`, `Tasks: Implement gate action API`, `content.d.ts`, `Tasks: Implement selected-thread email preview slice`, `AppState`, `HeaderMap`, `D-006 — Identity is not authority`?**
  _High betweenness centrality (0.029) - this node is a cross-community bridge._
- **Why does `Timestamp` connect `actions.rs` to `handle_owner_update`, `telegram.rs`, `Tasks: Implement gate action API`, `AppState`, `.sweep_expired_grants`?**
  _High betweenness centrality (0.020) - this node is a cross-community bridge._
- **Why does `digest_of_bytes()` connect `sandbox.rs` to `GmailConnector`, `OpenSpine conventions`, `openspine-decision-log.md`, `mod.rs`, `Ok`, `Requirements`, `Q: Trace the full runtime call path for a single incoming Telegram message from an authorized owner, all the way through to a TaskGrant being persisted in the kernel's SQLite store. List the modules/files involved, in call order, and identify the 2-3 modules that are most structurally central to this flow.`, `Tasks: Implement selected-thread email preview slice`, `D-006 — Identity is not authority`?**
  _High betweenness centrality (0.019) - this node is a cross-community bridge._
- **Are the 67 inferred relationships involving `handle_owner_update()` (e.g. with `email_preview_lane()` and `owner_control_lane()`) actually correct?**
