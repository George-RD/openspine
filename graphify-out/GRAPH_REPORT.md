...
## Corpus Check
- 396 files · ~288,946 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 5172 nodes · 6734 edges · 998 communities (405 shown, 593 thin omitted)
- Extraction: 87% EXTRACTED · 13% INFERRED · 0% AMBIGUOUS · INFERRED: 844 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `0d102dad`
...
## Communities (998 total, 593 thin omitted)

### Community 0 - "README.md"
Cohesion: 0.08
Nodes (45): canonical_json(), CanonicalValue, Value, bindings_valid(), Caveat, caveat_bytes(), chain_structurally_valid(), ChainStep (+37 more)
...
Cohesion: 0.14
Nodes (22): BuildEnvelopeFn, Future, GrantBindingFn, Output, Pin, PreflightFn, RouteGuardFn, Send (+14 more)
...
Cohesion: 0.13
Nodes (24): refuses_external_communication_without_containment(), RouteActorWhen, RouteWhen, Connector, Option, AccountRole, ActorHint, ChannelTrust (+16 more)
...
Cohesion: 0.07
Nodes (43): String, truncate_for_telegram(), truncate_with_notice(), HandlerFuture, handle_artifact_nominate(), handle_artifact_propose(), handle_lyra_preview(), handle_plan_propose() (+35 more)
...
Cohesion: 0.16
Nodes (26): digest_of(), artifact_propose_persists_and_sends_approval_button(), artifact_propose_rejects_duplicate_id_version(), artifact_propose_rejects_malformed_yaml(), artifact_propose_rejects_non_proposed_lifecycle(), artifact_propose_rejects_template_kind(), artifact_propose_rejects_unknown_kind(), injected_activation_tx_failure_keeps_approved_old_state() (+18 more)
...
Cohesion: 0.19
Nodes (15): notify_owner_required_errors_and_audits_on_send_failure(), notify_owner_required_succeeds_when_sent(), notify_owner_with_digest_records_success_counter(), notify_send_failure_records_attempt_failure_and_dead_letter(), parse_audit_event(), required_send_failure_keeps_dlq_when_counter_persistence_breaks(), shadow_grant_effect_suppressed_skips_effect_handler(), test_path_1_notify_owner_gated_and_audited() (+7 more)
...
Cohesion: 0.15
Nodes (12): Consequences, Consequences, D-027 — Multi-provider model gateway with per-provider auth mode, D-051 — The agent-OS canon (AD-001..153) is decomposed into a dependency-edged change sequence; the stale later-changes placeholders are superseded or subsumed, Decision, Decision, Decision Index, Lyra PRD Companion — Decisions Log (+4 more)
...
Cohesion: 0.16
Nodes (16): PostApprovalFuture, PostApprovalHandler, create_approved_draft(), handle_draft_approval_callback(), finalize_nomination(), NominationAssertion, notify_owner_best_effort(), handle_activate_artifact() (+8 more)
...
Cohesion: 0.24
Nodes (4): canonical_catalog_covers_all_fixture_action_ids(), canonical_catalog(), counterparty_classification_is_kernel_owned_and_fails_closed(), test_catalog_effect_paths_are_fully_enumerated_and_classified()
...
Cohesion: 0.18
Nodes (18): Sandbox, OwnedMutexGuard, RwLock, AppState, dispatch_polled_updates(), kernel_notify_grant(), notify_owner_required_outcome(), notify_owner_with_digest() (+10 more)
...
## Knowledge Gaps
- **1768 isolated node(s):** `set.sh script`, `autoresearch.sh script`, `TelegramReplyPayload`, `ReadThreadPayload`, `ArtifactNominatePayload` (+1763 more)
...
- **Why does `digest_of_bytes()` connect `sandbox.rs` to `GmailConnector`, `tests.rs`, `OpenSpine conventions`, `mod.rs`, `Ok`, `Requirements`, `Q: Trace the full runtime call path for a single incoming Telegram message from an authorized owner, all the way through to a TaskGrant being persisted in the kernel's SQLite store. List the modules/files involved, in call order, and identify the 2-3 modules that are most structurally central to this flow.`, `Tasks: Implement selected-thread email preview slice`?**
  _High betweenness centrality (0.025) - this node is a cross-community bridge._
- **Why does `Timestamp` connect `Proposal: Define OpenSpine development process` to `handle_owner_update`, `properties`, `.put`, `SKILL.md`, `Kernel foundation`, `HeaderMap`?**
  _High betweenness centrality (0.021) - this node is a cross-community bridge._
- **Why does `Digest` connect `tests.rs` to `sandbox.rs`, `MODIFIED Requirements`, `openspine-decision-log.md`, `mod.rs`, `Requirements`, `Kernel foundation`?**
