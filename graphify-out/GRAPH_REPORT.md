...
## Corpus Check
- 431 files · ~312,051 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 5673 nodes · 7333 edges · 1155 communities (436 shown, 719 thin omitted)
- Extraction: 87% EXTRACTED · 13% INFERRED · 0% AMBIGUOUS · INFERRED: 944 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `18d47ede`
...
## Communities (1155 total, 719 thin omitted)
...
Cohesion: 0.15
Nodes (32): handle_owner_update(), draft_command_composes_email_preview_grant_whose_pending_ref_is_derived_message(), draft_command_for_a_missing_thread_mints_no_grant(), draft_command_for_a_real_thread_composes_a_bound_selection_grant(), draft_command_is_refused_without_the_unsafe_flag_under_process_driver(), draft_command_with_gmail_api_error_audits_no_event_received(), draft_command_without_gmail_configured_is_a_no_op(), audit_payload_refs() (+24 more)
...
Cohesion: 0.14
Nodes (17): bounded_json_response(), CachedToken, extract_email_address(), GmailConnector, GmailError, GmailMessage, GmailThread, build_raw_reply_message() (+9 more)
...
Cohesion: 0.17
Nodes (16): AccountRole, ActorHint, ChannelTrust, Connector, EventEnvelope, InteractionMode, legacy_without_thread_id_defaults_to_none(), round_trips_through_serde() (+8 more)
...
Cohesion: 0.07
Nodes (45): BTreeSet, Briefcase, BriefcaseError, BriefcaseSection, CounterpartyRef, depth(), LearnedSource, Briefcase (+37 more)
...
Cohesion: 0.05
Nodes (20): AuditKind, AuditKindError, genesis_hash(), round_trips_through_serde(), append(), consumer_id_is_bound_to_filter(), failed_handler_does_not_advance_checkpoint(), filtered_replay_is_idempotent() (+12 more)
...
Cohesion: 0.16
Nodes (25): artifact_propose_persists_and_sends_approval_button(), artifact_propose_rejects_duplicate_id_version(), artifact_propose_rejects_malformed_yaml(), artifact_propose_rejects_non_proposed_lifecycle(), artifact_propose_rejects_template_kind(), artifact_propose_rejects_unknown_kind(), injected_activation_tx_failure_keeps_approved_old_state(), activation_with_mutated_payload_is_denied() (+17 more)
...
Cohesion: 0.17
Nodes (18): String, truncate_for_telegram(), truncate_with_notice(), ActionRequestBody, ActionResponseBody, dispatch_allowed_action(), dispatch_lyra_preview(), dispatch_read_selected_thread() (+10 more)
...
Cohesion: 0.12
Nodes (8): ArtifactId, String, Vec, WorkflowManifest, ArtifactNamespace, ArtifactRef, can_transition(), Lifecycle
...
Cohesion: 0.20
Nodes (17): DeserializeOwned, (), ApprovalStepInputs, ArtifactRef, bool, GatedStepDigest, Outcome, run_timer_driver() (+9 more)
...
Cohesion: 0.07
Nodes (32): deny_unknown_fields_rejects_capability_pack_id(), EntityType, Identifier, IdentifierKind, IdentifierVerificationMethod, Identity, identity_json_has_no_authority_field(), IdentityResolution (+24 more)
...
Cohesion: 0.10
Nodes (37): client_error(), get_briefcase(), post_topup(), authenticate(), bearer_token(), internal_error(), router(), unauthorized() (+29 more)
...
Cohesion: 0.20
Nodes (20): dispatch_task_timer_event(), concurrent_owner_callbacks_are_serialized_by_handler(), test_state(), consumed_update_is_dropped_before_pipeline_dispatch(), different_bot_rotation_starts_fresh_namespace(), telegram_offset_reads_namespaced_consumed_offset_for_same_bot(), authority_granted_audit_is_atomic_with_grant_persist(), deadline_timer_reaches_routed_granted_and_worker_gate() (+12 more)
...
Cohesion: 0.15
Nodes (17): BuildEnvelopeFn, GrantBindingFn, PreflightFn, RouteGuardFn, run_pipeline(), email_preview_lane(), LaneSpec, owner_control_lane() (+9 more)
...
Cohesion: 0.16
Nodes (9): empty_session_policy(), grant(), GrantLimits, GrantMode, legacy_missing_chain_defaults_but_fails_closed(), legacy_without_thread_id_defaults_to_none(), round_trip_and_mac(), TaskGrant (+1 more)
...
Cohesion: 0.15
Nodes (12): Consequences, Consequences, D-025 — Rust/Tokio substrate: storage, audit chain, and secrets handling, D-071 — External owner delivery may be delivery-unknown after a crash, Decision, Decision, Decision Index, Lyra PRD Companion — Decisions Log (+4 more)
...
Cohesion: 0.22
Nodes (8): Result, run_benchmarks(), Fn, Instant, Cli, commit_post_bind_clock(), main(), shutdown_signal()
...
Cohesion: 0.07
Nodes (38): PostApprovalFuture, PostApprovalHandler, kernel_notify_grant(), notify_owner_best_effort(), notify_owner_required(), notify_owner_required_outcome(), notify_owner_with_digest(), NotifyOutcome (+30 more)
...
Cohesion: 0.19
Nodes (10): ActionHandler, build_selection_token(), format_pending_message(), String, Ulid, OwnedMutexGuard, RwLock, email_grant_binding() (+2 more)
...
## Knowledge Gaps
- **1855 isolated node(s):** `set.sh script`, `autoresearch.sh script`, `TelegramReplyPayload`, `ReadThreadPayload`, `ArtifactNominatePayload` (+1850 more)
...
- **Why does `handle_owner_update()` connect `config.rs` to `digest.rs`, `policy.rs`, `Tasks: Implement gate action API`, `action.rs`, `Requirements`, `Q: Trace the full runtime call path for a single incoming Telegram message from an authorized owner, all the way through to a TaskGrant being persisted in the kernel's SQLite store. List the modules/files involved, in call order, and identify the 2-3 modules that are most structurally central to this flow.`, `Tasks: Define core runtime schemas`, `HeaderMap`, `SKILL.md`, `D-006 — Identity is not authority`, `D-003 — Gmail is a guarded workflow, not the whole product`?**
  _High betweenness centrality (0.023) - this node is a cross-community bridge._
- **Why does `digest_of()` connect `Requirements` to `properties`, `Proposal: Define core runtime schemas`, `content.d.ts`, `Q: Trace the full runtime call path for a single incoming Telegram message from an authorized owner, all the way through to a TaskGrant being persisted in the kernel's SQLite store. List the modules/files involved, in call order, and identify the 2-3 modules that are most structurally central to this flow.`, `Requirements`, `ActionId`, `Requirements`, `.default`, `Implementation plan`, `D-006 — Identity is not authority`?**
  _High betweenness centrality (0.022) - this node is a cross-community bridge._
- **Why does `Task` connect `archive.md` to `ConnectorRegistry`, `.put`, `Delegation & containment`, `apply.md`, `Tasks: Define core runtime schemas`?**
