# Graph Report - implement-counterparty-key-model  (2026-07-23)

## Corpus Check
- 572 files · ~466,245 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 8070 nodes · 10815 edges · 1758 communities (579 shown, 1179 thin omitted)
- Extraction: 87% EXTRACTED · 13% INFERRED · 0% AMBIGUOUS · INFERRED: 1417 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `c703e73c`
...
1. `test_state()` - 99 edges
2. `handle_owner_update()` - 95 edges
3. `owner_update()` - 86 edges
4. `test_state_with_telegram()` - 78 edges
5. `digest_of_bytes()` - 64 edges
6. `gate()` - 55 edges
7. `ArtifactStore` - 45 edges
8. `StoreError` - 45 edges
9. `request_for()` - 38 edges
10. `LearnedArtifact` - 35 edges
...
## Communities (1758 total, 1179 thin omitted)
...
Cohesion: 0.10
Nodes (12): action_id_qualifier_is_part_of_identity(), action_id_serializes_as_bare_string(), ActionCatalog, ActionEgressDeclaration, ActionId, DenialReason, EffectPath, EffectPathClass (+4 more)
...
Cohesion: 0.17
Nodes (27): digest_of(), artifact_propose_persists_and_sends_approval_button(), artifact_propose_rejects_duplicate_id_version(), artifact_propose_rejects_malformed_yaml(), artifact_propose_rejects_non_proposed_lifecycle(), artifact_propose_rejects_persona_kind(), artifact_propose_rejects_template_kind(), artifact_propose_rejects_unknown_kind() (+19 more)

### Community 12 - "client.rs"
Cohesion: 0.28
Nodes (16): a_double_tap_on_approve_creates_only_one_gmail_draft(), activate_approved_artifact_audits_failure_when_no_row(), approval_audit_never_contains_the_plaintext_draft_body(), approval_fixture_grant(), approval_fixture_request(), approve_callback_update(), draft_write_timeout_is_delivery_unknown_and_leaves_pending_rows(), gmail_with_token_mock() (+8 more)
...
Cohesion: 0.17
Nodes (27): persona_overlay_dir(), repair_persona_file(), ArtifactRef, Digest, Path, PathBuf, PersonaElement, Result (+19 more)
...
Cohesion: 0.09
Nodes (11): resolve_text(), retry_backoff(), retry_due_notifications(), run_retry_loop(), Store, DeadLetterState, detail_insert_columns(), DetailReceipt (+3 more)
...
Cohesion: 0.19
Nodes (28): active_v2_survives_stale_approved_v1_recovery(), approved_dangling_recovery_quarantines_and_requests_reconfirm(), commit_before_publish_crash_republishes_exact_active_ref(), fixture(), insert_active_proposal(), insert_approved_proposal(), insert_proposal(), learned_row() (+20 more)
...
Cohesion: 0.16
Nodes (23): apply_compatibility(), compatibility_epoch(), dangling_for_parsed(), ensure_reconfirm_request(), exclude_erased(), exclude_orphans(), find_orphans(), missing_provenance() (+15 more)
...
Cohesion: 0.15
Nodes (24): CompatibilityStatus, dependency_fingerprint_allows(), ensure_closure_triggers(), ensure_schema(), LearnedArtifact, LearnedArtifactErasure, LearnedArtifactIdentity, migrate_provenance_column() (+16 more)
...
Cohesion: 0.16
Nodes (22): PromptTemplate, load_keyed(), load_registry_into(), artifact_identity_pairs(), artifact_version(), ArtifactKindSpec, ArtifactLoadError, ArtifactRegistry (+14 more)
...
Cohesion: 0.06
Nodes (73): PromotionDenial, SkillRow, dispatch_skill_context(), skill_context_rejects_unknown_grant_purpose(), skill_context_selects_only_grant_scoped_installed_matches(), task_class_from_grant_purpose(), CeremonyError, CeremonyToken (+65 more)
...
Cohesion: 0.14
Nodes (28): alias_survival_prevented_and_temp_cleaned_on_publication_and_startup(), already_locked_legacy_lookup_migrates_without_re_locking(), associated_data_binds_counterparty_id_and_rejects_key_substitution(), closed_scope_in_memory_blocks_get_and_create(), concurrent_first_use_is_race_free(), creates_and_persists_a_key(), different_scopes_do_not_contend(), distinct_counterparties_get_distinct_keys() (+20 more)
...
Cohesion: 0.19
Nodes (14): CounterpartyKeyRing, ArtifactStore, ArtifactStoreError, fsync_dir(), AtomicBool, E, Error, FnOnce (+6 more)

### Community 108 - "Gmail selected-thread email preview setup (Phase 2)"
Cohesion: 0.06
Nodes (33): counterparty_resolves_identity_but_no_principal(), handle_owner_bind(), IdentityResolver, IdentityResolver<'a>, owner_verified_path_resolves_owner_principal_and_relationship(), unknown_resolves_to_relationship_unknown_confidence_0_and_no_write(), deny_unknown_fields_rejects_capability_pack_id(), EntityType (+25 more)
...
Cohesion: 0.13
Nodes (31): aead_associated_data_scope_tamper_fails_decrypt(), bit_flipped_ciphertext_fails_aead(), clear_upgrade_pending_sync_failure_retains_marker_for_retry(), concurrent_marker_recovery_and_missing_marker_clear_are_idempotent(), corrupt_scoped_target_with_valid_flat_source_fails_migration(), cross_counterparty_dedup_does_not_defeat_erasure(), different_content_is_different_ref(), erased_counterparty_payload_is_unrecoverable() (+23 more)
...
Cohesion: 0.20
Nodes (25): CounterpartyEraseError, erase_counterparty(), ErasureReport, Result, Store, Ulid, Vec, activated_standing_rule_becomes_unusable_after_source_scope_erasure() (+17 more)
...
Cohesion: 0.15
Nodes (13): activate_approved_artifact(), Result, reinstate_artifact(), Result, grant(), GrantLimits, GrantMode, legacy_missing_chain_defaults_but_fails_closed() (+5 more)
...
Cohesion: 0.05
Nodes (35): Debug, ReplayKey, BreakerState, CircuitBreaker, CircuitBreakerConfig, breaker_transitions_closed_open_half_open_closed(), dropped_permit_reopens_breaker_instead_of_wedging_half_open(), failures_outside_window_expire() (+27 more)
...
Cohesion: 0.18
Nodes (19): call_with_connector(), ActionRequestBody, ActionResponseBody, cleanup_pre_effect_reservations(), dispatch_lyra_preview(), dispatch_read_selected_thread(), DispatchError, FailureSurface (+11 more)

### Community 160 - "D-012 — Audit stores private payloads by encrypted/hash reference"
Cohesion: 0.16
Nodes (14): PostApprovalHandler, emit_preflight_failure(), handle_draft_approval_callback(), kernel_notify_grant(), notify_owner_best_effort(), notify_owner_required_outcome(), notify_owner_with_digest(), NotifyOutcome (+6 more)
...
Cohesion: 0.13
Nodes (26): admits_more_than_three_tiny_items(), canonical_unavailable_marker_view_does_not_grow_items_or_audits(), detail_pages(), detail_pages_reconstruct_every_byte_on_utf8_boundaries(), drains_aggregate_across_pages_without_duplicates(), handle_command(), handle_detail_command(), item() (+18 more)

### Community 175 - "D-033 — Action identifiers are exact-match dotted strings; unverified senders are audited and ignored"
Cohesion: 0.24
Nodes (8): Constraints, Policy, SessionPolicy, allow_and_deny_lists_never_overlap_in_the_fixture(), AppliesTo, CapabilityPack, owner_control_basic_pack(), round_trips_through_serde()
...
Cohesion: 0.15
Nodes (12): Consequences, Consequences, D-015 — Phase 1 should avoid final email send, D-059 — Dormant thread bindings are MAC-authenticated before activation, Decision, Decision, Decision Index, Lyra PRD Companion — Decisions Log (+4 more)
...
Cohesion: 0.29
Nodes (3): ActionHandler, bounded_preview(), ActionHandlerRegistry
...
Cohesion: 0.14
Nodes (7): truncate_for_telegram(), truncate_with_notice(), dispatch_plan_preview(), handle_artifact_nominate(), handle_artifact_propose(), handle_lyra_preview(), handle_plan_propose()
...
Cohesion: 0.25
Nodes (9): handle_worker_commission(), commissioned_grant_for_receipt(), CommissionReceipt, mark_worker_stranded_notified(), stranded_worker_timeouts(), stranded_worker_timeout_detects_expired(), worker_dispatch_failed(), worker_dispatch_state() (+1 more)
...
Cohesion: 0.30
Nodes (10): approve_callback(), learned_route(), nomination_owner_tap_persists_nominated_status_and_audit(), nomination_rejects_persona_kind_before_authority_path(), nomination_requires_explicit_depersonalized_assertion(), MockServer, Ulid, telegram_ok() (+2 more)
...
Cohesion: 0.16
Nodes (11): gated_step_persisted_pending_recovers_without_redispatch(), router(), commissioned_worker_reports_and_master_relays_through_real_shell(), parent_briefcase(), parent_grant_for_commission(), root_pipeline_shell_does_not_report_worker_result(), worker_shell_binary(), build_state() (+3 more)
...
Cohesion: 0.25
Nodes (8): Change Log, Consequences, D-121 — Fail-closed disclosure budget accounting with policy-free worker outcomes, Decision, Open Decision Questions — CLOSED (see linked decisions), Rationale, Research / Reference Backlog, Would change if
...
### Community 440 - "Vec"
Cohesion: 0.60
Nodes (4): admit(), Path, Result, Store
...
## Knowledge Gaps
- **2362 isolated node(s):** `autoresearch.sh script`, `TelegramReplyPayload`, `ReadThreadPayload`, `ArtifactNominatePayload`, `ArtifactProposePayload` (+2357 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **1179 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `digest_of_bytes()` connect `openspine-decision-log.md` to `identity.rs`, `D-022 — Agent-owned inbox is distinct from owner mailbox access`, `GmailConnector`, `sandbox.rs`, `.default`, `standing_rule_timer.rs`, `.put`, `OpenSpine`, `ProposedArtifact`, `digest.rs`, `Tasks: Implement authority composition`, `check-claims.sh`, `Requirements`, `content.d.ts`, `opsx-apply.md`, `Proposal: Implement selected-thread email preview slice`, `AuditEvent`, `Requirements`?**
  _High betweenness centrality (0.038) - this node is a cross-community bridge._
- **Why does `test_state()` connect `config.rs` to `D-004 — Every effectful action goes through `gate()``, `standing_rule_timer.rs`, `ProposedArtifact`, `Display`, `client.rs`, `action.rs`, `AppState`, `Lifecycle`, `D-032 — Kernel↔shell transport is HTTP/JSON with a per-task bearer token`, `Approach`, `artifact_propose.rs`, `approval.rs`, `Requirements`, `README.md`, `Proposal: Define core runtime schemas`, `Proposal: Backfill implemented capability specs`, `Proposal: Implement artifact lifecycle slice`, `Tasks: Define OpenSpine development process`, `Gmail selected-thread email preview setup (Phase 2)`?**
  _High betweenness centrality (0.023) - this node is a cross-community bridge._
- **Why does `handle_owner_update()` connect `config.rs` to `decisions.md`, `D-005 — Private-data shell must be contained`, `D-012 — Audit stores private payloads by encrypted/hash reference`, `standing_rule_timer.rs`, `.default`, `ProposedArtifact`, `Display`, `D-032 — Kernel↔shell transport is HTTP/JSON with a per-task bearer token`, `Proposal: Define core runtime schemas`, `Approach`, `action.rs`, `opsx-apply.md`, `Lifecycle`, `ADDED Requirements`, `architecture.md`?**
  _High betweenness centrality (0.023) - this node is a cross-community bridge._
- **Are the 97 inferred relationships involving `test_state()` (e.g. with `long_detail_is_bounded_in_summary_and_full_via_ref()` and `draft_command_without_gmail_configured_is_a_no_op()`) actually correct?**
  _`test_state()` has 97 INFERRED edges - model-reasoned connections that need verification._
- **Are the 90 inferred relationships involving `handle_owner_update()` (e.g. with `nomination_owner_tap_persists_nominated_status_and_audit()` and `nomination_rejects_persona_kind_before_authority_path()`) actually correct?**
  _`handle_owner_update()` has 90 INFERRED edges - model-reasoned connections that need verification._
- **Are the 85 inferred relationships involving `owner_update()` (e.g. with `approve_callback()` and `nomination_owner_tap_persists_nominated_status_and_audit()`) actually correct?**
  _`owner_update()` has 85 INFERRED edges - model-reasoned connections that need verification._
- **Are the 76 inferred relationships involving `test_state_with_telegram()` (e.g. with `nomination_owner_tap_persists_nominated_status_and_audit()` and `nomination_rejects_persona_kind_before_authority_path()`) actually correct?**
