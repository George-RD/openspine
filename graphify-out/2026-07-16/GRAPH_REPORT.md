# Graph Report - implement-plan-digest-approval  (2026-07-16)

## Corpus Check
- 280 files · ~197,650 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3481 nodes · 4427 edges · 680 communities (285 shown, 395 thin omitted)
- Extraction: 90% EXTRACTED · 10% INFERRED · 0% AMBIGUOUS · INFERRED: 450 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `2d68fa73`
...
1. `gate()` - 49 edges
2. `AppState` - 43 edges
3. `TaskGrant` - 40 edges
4. `handle_owner_update()` - 37 edges
5. `request_for()` - 34 edges
6. `grant_with()` - 33 edges
7. `test_catalog()` - 30 edges
8. `owner_update()` - 27 edges
9. `StoreError` - 23 edges
10. `test_state()` - 23 edges
...
## Communities (680 total, 395 thin omitted)
...
Cohesion: 0.06
Nodes (73): ActionOrigin, AuditMeta, chain_valid(), EgressClassifier, gate(), GateContext, GateOutcome, NoEgress (+65 more)

### Community 3 - "handle_owner_update"
Cohesion: 0.13
Nodes (33): canonical_json(), bindings_valid(), Caveat, caveat_bytes(), chain_structurally_valid(), ChainStep, compute_mac_hex(), compute_tip() (+25 more)

### Community 4 - ".default"
Cohesion: 0.06
Nodes (36): Value, step(), digest_is_order_argument_and_schema_sensitive(), digest_matches_serialized_plan_payload(), Plan, PlanApprovalQuestion, PlanStep, question_renders_every_digest_bound_field() (+28 more)
...
Cohesion: 0.07
Nodes (31): Bot, build_owner_envelope(), CallbackQueryUpdate, parse_approve_callback(), parse_approve_plan_callback(), parse_bind_command(), parse_draft_command(), project_update() (+23 more)
...
Cohesion: 0.05
Nodes (69): ActionHandler, ActionHandlerRegistry, handle_artifact_propose(), handle_lyra_preview(), handle_plan_propose(), handle_read_selected_thread(), handle_setup_workflow_start(), handle_status_read() (+61 more)
...
Cohesion: 0.05
Nodes (46): BuildEnvelopeFn, Command, empty_session_policy(), Future, GrantBindingFn, Output, Pin, PreflightFn (+38 more)
...
### Community 50 - "mod.rs"
Cohesion: 0.12
Nodes (15): ADDED Requirements, MODIFIED Requirements, Requirement: A mutated approved plan MUST be refused at the gate, Requirement: Plan approval MUST bind the complete ordered step-list digest, Requirement: Plan steps MUST bind exact execution identity, Requirement: The kernel MUST re-derive digests from artifact-store bytes at approval-effect time, Scenario: Arguments change while summary remains unchanged, Scenario: Data-handling step participates in the digest (+7 more)
...
Cohesion: 0.06
Nodes (80): handle_owner_update(), audit_payload_refs(), draft_command_composes_email_preview_grant_whose_pending_ref_is_derived_message(), draft_command_for_a_missing_thread_mints_no_grant(), draft_command_for_a_real_thread_composes_a_bound_selection_grant(), draft_command_is_refused_without_the_unsafe_flag_under_process_driver(), draft_command_with_gmail_api_error_audits_no_event_received(), draft_command_without_gmail_configured_is_a_no_op() (+72 more)
...
## Knowledge Gaps
- **1250 isolated node(s):** `autoresearch.sh script`, `TelegramReplyPayload`, `ReadThreadPayload`, `ArtifactProposePayload`, `TokenResponse` (+1245 more)
...
- **Why does `AppState` connect `ADDED Requirements` to `event.rs`, `sandbox.rs`, `GmailConnector`, `artifact_loader.rs`, `.put`, `digest.rs`, `Requirements`, `AppState`?**
  _High betweenness centrality (0.027) - this node is a cross-community bridge._
- **Why does `handle_owner_update()` connect `Requirements` to `telegram.rs`, `sandbox.rs`, `ADDED Requirements`?**
  _High betweenness centrality (0.019) - this node is a cross-community bridge._
- **Why does `TaskGrant` connect `ADDED Requirements` to `handle_owner_update`, `event.rs`, `Requirements`, `GmailConnector`?**
  _High betweenness centrality (0.018) - this node is a cross-community bridge._
- **Are the 39 inferred relationships involving `gate()` (e.g. with `allowed_action_returns_allow()` and `allowed_plus_approval_required_returns_approval_required()`) actually correct?**
  _`gate()` has 39 INFERRED edges - model-reasoned connections that need verification._
- **Are the 29 inferred relationships involving `handle_owner_update()` (e.g. with `handle_draft_approval_callback()` and `handle_plan_approval_callback()`) actually correct?**
  _`handle_owner_update()` has 29 INFERRED edges - model-reasoned connections that need verification._
- **Are the 15 inferred relationships involving `request_for()` (e.g. with `approved_plan_digest_allows_through_gate()` and `argument_mutation_denied_when_summary_is_unchanged()`) actually correct?**
