...
## Corpus Check
- 271 files · ~191,600 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3437 nodes · 5238 edges · 419 communities (305 shown, 114 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 401 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `94eb040f`
...
1. `AppState` - 73 edges
2. `TaskGrant` - 72 edges
3. `gate()` - 43 edges
4. `ActionId` - 42 edges
5. `StoreError` - 37 edges
6. `handle_owner_update()` - 35 edges
7. `test_catalog()` - 32 edges
8. `EventEnvelope` - 31 edges
9. `request_for()` - 29 edges
10. `grant_with()` - 28 edges
...
- `token_requiring_action_allowed_with_valid_token()` --calls--> `gate()`  [INFERRED]
  crates/openspine-gate/src/gate/token_tests.rs → crates/openspine-gate/src/gate.rs
- `token_requiring_action_denied_for_foreign_grant()` --calls--> `gate()`  [INFERRED]
  crates/openspine-gate/src/gate/token_tests.rs → crates/openspine-gate/src/gate.rs
- `token_requiring_action_denied_when_expired()` --calls--> `gate()`  [INFERRED]
  crates/openspine-gate/src/gate/token_tests.rs → crates/openspine-gate/src/gate.rs
...
## Communities (419 total, 114 thin omitted)

### Community 0 - "README.md"
Cohesion: 0.07
Nodes (71): AuthorityInput, AuthorityOutcome, classification_rank(), compose_authority(), mint_task_token(), narrow(), AgentManifest, Box (+63 more)

### Community 1 - ".new"
Cohesion: 0.08
Nodes (21): hash_task_token(), Result, String, Timestamp, Ulid, Store, Option, Result (+13 more)

### Community 2 - "event.rs"
Cohesion: 0.09
Nodes (50): deny_limit_exceeded(), GenerateRequestBody, GenerateResponseBody, post_model_generate(), Arc, HeaderMap, Json, Option (+42 more)
...
### Community 4 - ".default"
Cohesion: 0.09
Nodes (22): canonical_catalog(), id(), test_catalog_effect_paths_are_fully_enumerated_and_classified(), action_id_qualifier_is_part_of_identity(), action_id_serializes_as_bare_string(), ActionCatalog, ActionId, EffectPath (+14 more)
...
### Community 8 - "mod.rs"
Cohesion: 0.26
Nodes (30): gate(), allowed_action_returns_allow(), allowed_plus_approval_required_returns_approval_required(), allowed_plus_denied_returns_deny(), approval_for(), approval_required_action_does_not_execute(), approval_required_action_returns_approval_required(), approved_but_payload_changed_since_is_denied_not_reasked() (+22 more)

### Community 9 - "telegram.rs"
Cohesion: 0.08
Nodes (28): Bot, build_owner_envelope(), CallbackQueryUpdate, parse_approve_callback(), parse_bind_command(), parse_draft_command(), project_update(), Option (+20 more)

### Community 10 - ".put"
Cohesion: 0.17
Nodes (28): canonical_json(), bindings_valid(), Caveat, caveat_bytes(), chain_structurally_valid(), ChainStep, compute_mac_hex(), compute_tip() (+20 more)

### Community 11 - "ProposedArtifact"
Cohesion: 0.15
Nodes (29): artifact_propose_persists_and_sends_approval_button(), artifact_propose_rejects_duplicate_id_version(), artifact_propose_rejects_malformed_yaml(), artifact_propose_rejects_non_proposed_lifecycle(), artifact_propose_rejects_template_kind(), artifact_propose_rejects_unknown_kind(), route_yaml(), String (+21 more)

### Community 12 - "client.rs"
Cohesion: 0.07
Nodes (65): denied_read_thread_stops_without_drafting(), Draft, draft_reply(), empty_draft_skips_preview_without_error(), format_thread_for_model(), format_thread_for_model_includes_all_fields(), full_flow_reads_drafts_and_previews(), no_selection_tokens_is_an_error() (+57 more)

### Community 13 - "policy.rs"
Cohesion: 0.10
Nodes (15): try_count_model_call_allows_exactly_one_concurrent_winner_at_max_one(), action_request_consume_is_single_use(), action_request_round_trips_by_id(), approval_round_trips_by_action_request_id(), find_task_grant_by_token_rejects_the_raw_hash_value(), most_recent_approval_wins_when_multiple_exist_for_one_request(), opening_a_pre_existing_db_without_the_used_column_is_migrated_in_place(), persisted_grant_json_contains_no_task_token() (+7 more)
...
Cohesion: 0.14
Nodes (23): AccountRole, ActorHint, ChannelTrust, Connector, EventEnvelope, EventType, InteractionMode, round_trips_through_serde() (+15 more)
...
Cohesion: 0.15
Nodes (20): create_approved_draft(), handle_draft_approval_callback(), Result, Ulid, activate_approved_artifact(), Result, AppState, kernel_notify_grant() (+12 more)
...
Cohesion: 0.16
Nodes (17): matches(), Option, Source, RelationshipKind, effect_defaults_to_allow_when_omitted(), owner_route(), round_trips_through_serde(), Route (+9 more)
...
Cohesion: 0.21
Nodes (17): ActionOrigin, AuditMeta, chain_valid(), GateContext, GateOutcome, resolve(), resolve_approval_required(), Option (+9 more)
...
### Community 46 - "Digest"
Cohesion: 0.23
Nodes (15): grant_with_token(), make_token(), MockContext, request_for_token(), HashMap, Option, Timestamp, Ulid (+7 more)
...
Cohesion: 0.20
Nodes (12): grant(), GrantLimits, GrantMode, legacy_missing_chain_defaults_but_fails_closed(), round_trip_and_mac(), ArtifactId, Option, String (+4 more)
...
Cohesion: 0.43
Nodes (15): artifact_ref(), email_route(), exact_deny_route_wins_over_allow_route(), gmail_connector_authenticated_alone_does_not_match_the_selected_thread_route(), higher_priority_route_wins_over_lower_priority(), no_matching_route_is_denied_not_ambiguous(), no_relationship_match_denies_the_route(), owner_event() (+7 more)

### Community 59 - "ADDED Requirements"
Cohesion: 0.27
Nodes (13): counterparty_resolves_identity_but_no_principal(), handle_owner_bind(), IdentityResolver, IdentityResolver<'a>, owner_verified_path_resolves_owner_principal_and_relationship(), Option, Result, Self (+5 more)
...
Cohesion: 0.27
Nodes (14): authenticate(), bearer_token(), get_status(), internal_error(), router(), Arc, Display, HeaderMap (+6 more)
...
Cohesion: 0.23
Nodes (11): lyra_ui_preview_sends_telegram_reply_to_grant_bound_chat(), lyra_ui_preview_truncates_long_body_to_utf16_limit(), truncated_preview_carries_no_approval_button_and_persists_no_action_request(), owner_notify_routes_through_gate_and_audits(), build_state(), repo_lyra_dir(), GmailConnector, Option (+3 more)
...
### Community 119 - "attrs"
Cohesion: 0.10
Nodes (21): Result, run_benchmarks(), Connector, connector_registry_enumerates_configured_connectors(), ConnectorRegistry, gmail(), GmailConnector, Item (+13 more)
...
Cohesion: 0.05
Nodes (48): Aes256Gcm, ArtifactStore, ArtifactStoreError, different_content_is_different_ref(), get_is_idempotent(), hex_encode(), key(), round_trips_plaintext() (+40 more)
...
## Knowledge Gaps
- **1271 isolated node(s):** `set.sh script`, `autoresearch.sh script`, `ArtifactProposePayload`, `name`, `private` (+1266 more)
...
- **Why does `AppState` connect `AppState` to `event.rs`, `.default`, `artifact_loader.rs`, `ProposedArtifact`, `Requirements`, `AppState`, `sandbox.rs`, `ADDED Requirements`, `add_column_if_missing`, `D-025 — Rust/Tokio substrate: storage, audit chain, and secrets handling`, `Requirements`, `ADDED Requirements`, `ADDED Requirements`, `Requirements`, `editUrl`, `Proposal: Define OpenSpine development process`, `explore.md`, `opsx-explore.md`, `attrs`?**
  _High betweenness centrality (0.030) - this node is a cross-community bridge._
- **Why does `AuditKind` connect `OpenSpine Agent-OS Design Log` to `Requirements`?**
  _High betweenness centrality (0.020) - this node is a cross-community bridge._
- **Why does `Digest` connect `add_column_if_missing` to `Requirements`, `.new`, `README.md`, `ADDED Requirements`, `Requirements`, `Requirements`?**
...
- **What connects `set.sh script`, `autoresearch.sh script`, `ArtifactProposePayload` to the rest of the system?**
  _1271 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `README.md` be split into smaller, more focused modules?**
  _Cohesion score 0.07314524555903866 - nodes in this community are weakly interconnected._
