# Graph Report - implement-overlay-eval-gate  (2026-07-16)

## Corpus Check
- 296 files · ~209,106 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3696 nodes · 4579 edges · 737 communities (311 shown, 426 thin omitted)
- Extraction: 90% EXTRACTED · 10% INFERRED · 0% AMBIGUOUS · INFERRED: 465 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `ca69ef6a`
...
1. `gate()` - 46 edges
2. `StoreError` - 36 edges
3. `handle_owner_update()` - 35 edges
4. `request_for()` - 33 edges
5. `grant_with()` - 32 edges
6. `test_catalog()` - 30 edges
7. `owner_update()` - 30 edges
8. `test_state()` - 24 edges
9. `test_state_with_telegram()` - 22 edges
10. `OpenSpine Agent-OS Design Log` - 22 edges
...
## Communities (737 total, 426 thin omitted)
...
### Community 4 - ".default"
Cohesion: 0.07
Nodes (30): Aes256Gcm, genesis_digest(), Ok, Serialize, Sha256, ArtifactStore, ArtifactStoreError, different_content_is_different_ref() (+22 more)
...
Cohesion: 0.31
Nodes (11): PromptTemplate, T, ArtifactKindSpec, ArtifactLoadError, ArtifactRegistry, collide_keyed(), collide_route(), find_kind_spec() (+3 more)
...
### Community 10 - ".put"
Cohesion: 0.12
Nodes (17): evaluate(), JudgeDenial, Result, String, GateDenial, GateEvidence, JudgePassed, ReplayPassed (+9 more)
...
Cohesion: 0.21
Nodes (17): ensure_schema(), epoch_nanos_to_timestamp(), EvalVerdict, insert_eval_verdict_conn(), map_row(), read_eval_row(), Connection, Option (+9 more)
...
Cohesion: 0.17
Nodes (25): dispatch_artifact_propose(), Option, Result, Value, artifact_propose_persists_and_sends_approval_button(), artifact_propose_rejects_duplicate_id_version(), artifact_propose_rejects_malformed_yaml(), artifact_propose_rejects_non_proposed_lifecycle() (+17 more)

### Community 54 - "Requirements"
Cohesion: 0.29
Nodes (15): test_state_with_telegram(), lyra_ui_preview_sends_telegram_reply_to_grant_bound_chat(), lyra_ui_preview_truncates_long_body_to_utf16_limit(), truncated_preview_carries_no_approval_button_and_persists_no_action_request(), approval_required_action_stops_before_dispatch(), counterparty_denial_returns_deferral_routes_and_audits(), counterparty_escalation_failure_is_not_reported_as_success(), email_read_inbox_is_denied_for_owner_control_grant() (+7 more)
...
Cohesion: 0.10
Nodes (22): ActionHandler, ArtifactProposePayload, String, build_state(), repo_lyra_dir(), GmailConnector, Option, PathBuf (+14 more)
...
Cohesion: 0.08
Nodes (48): get_task(), TaskLimitsBody, TaskViewBody, a_spoofed_closing_marker_in_the_content_does_not_escape_the_boundary(), build_prompt(), build_prompt_carries_system_preamble_and_conversation_through(), build_prompt_with_untrusted_context(), PromptMessage (+40 more)
...
Cohesion: 0.32
Nodes (9): test_state(), parse_audit_event(), shadow_grant_effect_suppressed_skips_effect_handler(), test_path_1_notify_owner_gated_and_audited(), test_path_2_create_draft_payload_mutated_audited(), test_path_3_activate_artifact_failure_audited(), test_path_4_read_selected_thread_gated_and_audited(), test_path_5_preview_gated_and_audited() (+1 more)

### Community 82 - "Proposal: Implement authority composition"
Cohesion: 0.27
Nodes (10): load_registry(), parse_proposal(), kind_table_excludes_templates(), kind_table_round_trips_all_five_kinds(), loads_every_real_fixture_without_error(), malformed_fixture_fails_to_load(), missing_directory_is_not_an_error(), non_yaml_files_are_ignored() (+2 more)
...
Cohesion: 0.08
Nodes (34): BuildEnvelopeFn, Future, GrantBindingFn, Output, Pin, PostApprovalFuture, PreflightFn, RouteGuardFn (+26 more)
...
Cohesion: 0.40
Nodes (9): activation_with_mutated_payload_is_denied(), approve_callback_update(), approved_artifact_activates_into_registry_and_overlay(), mount_send_message_ok(), MockServer, TelegramConnector, Ulid, telegram_stub() (+1 more)
...
## Knowledge Gaps
- **1337 isolated node(s):** `autoresearch.sh script`, `TelegramReplyPayload`, `ReadThreadPayload`, `TokenResponse`, `IdentityResolver` (+1332 more)
...
- **Why does `AppState` connect `compose_authority` to `properties`, `.default`, `artifact_loader.rs`, `SKILL.md`, `Gmail selected-thread email preview setup (Phase 2)`, `Tasks: Harden approval and budgets`, `ADDED Requirements`, `Proposal: Define core runtime schemas`, `Requirements`, `Requirements`?**
  _High betweenness centrality (0.022) - this node is a cross-community bridge._
- **Why does `dispatch_artifact_propose()` connect `Requirements` to `README.md`, `compose_authority`, `.default`, `artifact_loader.rs`, `.put`, `Gmail selected-thread email preview setup (Phase 2)`, `Digest`, `Proposal: Implement authority composition`?**
  _High betweenness centrality (0.020) - this node is a cross-community bridge._
- **Why does `gate()` connect `event.rs` to `compose_authority`, `SKILL.md`, `properties`, `Digest`?**
