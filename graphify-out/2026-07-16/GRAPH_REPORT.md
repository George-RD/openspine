# Graph Report - implement-egress-classes  (2026-07-16)

## Corpus Check
- 270 files · ~193,093 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3389 nodes · 4500 edges · 606 communities (281 shown, 325 thin omitted)
- Extraction: 91% EXTRACTED · 9% INFERRED · 0% AMBIGUOUS · INFERRED: 427 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `7baef130`
...
1. `TaskGrant` - 53 edges
2. `gate()` - 48 edges
3. `ActionId` - 44 edges
4. `AppState` - 38 edges
5. `handle_owner_update()` - 35 edges
6. `request_for()` - 31 edges
7. `test_catalog()` - 31 edges
8. `grant_with()` - 30 edges
9. `owner_update()` - 27 edges
10. `owner_control_input()` - 23 edges

## Surprising Connections (you probably didn't know these)
- `post_model_generate()` --calls--> `internal_error()`  [INFERRED]
  crates/openspine-kernel/src/api/generate.rs → crates/openspine-kernel/src/api/mod.rs
...
## Communities (606 total, 325 thin omitted)

### Community 0 - "README.md"
Cohesion: 0.13
Nodes (56): AuthorityInput, AuthorityOutcome, classification_rank(), compose_authority(), mint_task_token(), narrow(), AgentManifest, Box (+48 more)

### Community 1 - ".new"
Cohesion: 0.05
Nodes (32): Aes256Gcm, Ok, Serialize, Sha256, ArtifactStore, ArtifactStoreError, different_content_is_different_ref(), get_is_idempotent() (+24 more)
...
Cohesion: 0.08
Nodes (47): artifact_key_bytes(), artifact_key_round_trips_bytes(), Config, ConfigError, default_kernel_bind(), example_configs_parse_against_the_real_schema(), gmail_client_secret(), gmail_refresh_token() (+39 more)

### Community 4 - ".default"
Cohesion: 0.13
Nodes (15): canonical_catalog(), id(), test_catalog_effect_paths_are_fully_enumerated_and_classified(), action_id_qualifier_is_part_of_identity(), action_id_serializes_as_bare_string(), ActionCatalog, EffectPath, EffectPathClass (+7 more)

### Community 5 - "GmailConnector"
Cohesion: 0.08
Nodes (22): counterparty_resolves_identity_but_no_principal(), handle_owner_bind(), IdentityResolver, IdentityResolver<'a>, owner_verified_path_resolves_owner_principal_and_relationship(), unknown_resolves_to_relationship_unknown_confidence_0_and_no_write(), is_unique_constraint_store_error(), is_unique_constraint_violation() (+14 more)

### Community 6 - "artifact_loader.rs"
Cohesion: 0.08
Nodes (28): PromptTemplate, T, AgentManifest, ArtifactKindSpec, ArtifactLoadError, ArtifactRegistry, CapabilityPack, collide_keyed() (+20 more)
...
### Community 8 - "mod.rs"
Cohesion: 0.06
Nodes (81): ActionOrigin, AuditMeta, chain_valid(), EgressClassifier, gate(), GateContext, GateOutcome, NoEgress (+73 more)
...
### Community 10 - ".put"
Cohesion: 0.07
Nodes (67): email_reply_drafter_grant(), email_reply_drafter_template_wraps_untrusted_context_on_the_wire(), grant_with_limits(), max_artifacts_of_one_denies_the_second_call_with_a_single_provider_hit(), max_model_calls_of_one_denies_the_second_call_with_a_single_provider_hit(), post_model_generate(), JoinHandle, Option (+59 more)

### Community 11 - "ProposedArtifact"
Cohesion: 0.17
Nodes (24): artifact_ref(), email_route(), exact_deny_route_wins_over_allow_route(), gmail_connector_authenticated_alone_does_not_match_the_selected_thread_route(), higher_priority_route_wins_over_lower_priority(), matches(), no_matching_route_is_denied_not_ambiguous(), no_relationship_match_denies_the_route() (+16 more)
...
Cohesion: 0.17
Nodes (24): ActionRequestBody, ActionResponseBody, dispatch_allowed_action(), dispatch_lyra_preview(), dispatch_read_selected_thread(), DispatchError, post_actions(), PreviewPayload (+16 more)
...
Cohesion: 0.08
Nodes (30): allow_and_deny_lists_never_overlap_in_the_fixture(), AppliesTo, CapabilityPack, owner_control_basic_pack(), round_trips_through_serde(), ArtifactId, Connector, Option (+22 more)
...
Cohesion: 0.20
Nodes (15): create_approved_draft(), handle_draft_approval_callback(), Result, Ulid, AppState, kernel_notify_grant(), notify_owner_best_effort(), Option (+7 more)
...
Cohesion: 0.19
Nodes (12): built_in_web_egress_endpoints(), ConnectorRegistry, EgressRegistrationError, Display, Error, Formatter, HashMap, Option (+4 more)

### Community 38 - "ADDED Requirements"
Cohesion: 0.05
Nodes (43): BuildEnvelopeFn, Command, Future, GrantBindingFn, Output, Pin, PostApprovalFuture, PostApprovalHandler (+35 more)
...
Cohesion: 0.16
Nodes (7): ActionId, Display, Formatter, Into, Result, String, From
...
Cohesion: 0.25
Nodes (8): authenticate(), bearer_token(), internal_error(), router(), unauthorized(), get_task(), TaskLimitsBody, TaskViewBody
...
Cohesion: 0.39
Nodes (8): HandlerFuture, handle_artifact_propose(), handle_lyra_preview(), handle_read_selected_thread(), handle_setup_workflow_start(), handle_status_read(), handle_telegram_reply(), handle_workflow_invoke()
...
### Community 119 - "attrs"
Cohesion: 0.22
Nodes (10): conflicting_egress_rating_is_rejected_without_downgrade(), Connector, connector_registry_enumerates_configured_connectors(), gmail(), GmailConnector, no_egress_classifier_skips_check_for_unrated_actions(), registry_rates_built_in_web_egress_endpoints(), Self (+2 more)
...
## Knowledge Gaps
- **1221 isolated node(s):** `autoresearch.sh script`, `ArtifactProposePayload`, `TokenResponse`, `IdentityResolver`, `WorkflowManifest` (+1216 more)
...
- **Why does `AppState` connect `AppState` to `.new`, `event.rs`, `sandbox.rs`, `.default`, `handle_owner_update`, `artifact_loader.rs`, `ADDED Requirements`, `.put`, `Requirements`, `Requirements`, `AppState`?**
  _High betweenness centrality (0.021) - this node is a cross-community bridge._
- **Why does `handle_owner_update()` connect `Requirements` to `AppState`, `.put`, `ADDED Requirements`, `telegram.rs`?**
  _High betweenness centrality (0.016) - this node is a cross-community bridge._
- **Why does `ProviderClient` connect `handle_owner_update` to `AppState`, `.put`, `Requirements`?**
  _High betweenness centrality (0.016) - this node is a cross-community bridge._
- **Are the 38 inferred relationships involving `gate()` (e.g. with `allowed_action_returns_allow()` and `allowed_plus_approval_required_returns_approval_required()`) actually correct?**
  _`gate()` has 38 INFERRED edges - model-reasoned connections that need verification._
- **Are the 27 inferred relationships involving `handle_owner_update()` (e.g. with `handle_draft_approval_callback()` and `email_preview_lane()`) actually correct?**
...
- **Should `README.md` be split into smaller, more focused modules?**
  _Cohesion score 0.1337916446324696 - nodes in this community are weakly interconnected._