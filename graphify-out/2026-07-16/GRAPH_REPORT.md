...
## Corpus Check
- 261 files · ~183,238 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3274 nodes · 5107 edges · 378 communities (302 shown, 76 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 400 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `9b1b7549`
...
3. `ActionId` - 45 edges
4. `gate()` - 43 edges
5. `StoreError` - 41 edges
6. `handle_owner_update()` - 35 edges
7. `Digest` - 33 edges
8. `test_catalog()` - 32 edges
9. `ArtifactRef` - 31 edges
10. `EventEnvelope` - 31 edges
...
Cohesion: 0.16
Nodes (50): AuthorityInput, AuthorityOutcome, compose_authority(), mint_task_token(), narrow(), AgentManifest, Box, CapabilityPack (+42 more)

### Community 1 - ".new"
Cohesion: 0.07
Nodes (45): deny_limit_exceeded(), GenerateRequestBody, GenerateResponseBody, post_model_generate(), Arc, HeaderMap, Json, Option (+37 more)
...
Cohesion: 0.06
Nodes (29): try_count_model_call_allows_exactly_one_concurrent_winner_at_max_one(), action_request_consume_is_single_use(), action_request_round_trips_by_id(), approval_round_trips_by_action_request_id(), find_task_grant_by_token_rejects_the_raw_hash_value(), most_recent_approval_wins_when_multiple_exist_for_one_request(), opening_a_pre_existing_db_without_the_used_column_is_migrated_in_place(), persisted_grant_json_contains_no_task_token() (+21 more)

### Community 4 - ".default"
Cohesion: 0.09
Nodes (23): canonical_catalog(), id(), test_catalog_effect_paths_are_fully_enumerated_and_classified(), action_id_qualifier_is_part_of_identity(), action_id_serializes_as_bare_string(), ActionCatalog, ActionId, EffectPath (+15 more)
...
Cohesion: 0.06
Nodes (41): Bot, counterparty_resolves_identity_but_no_principal(), handle_owner_bind(), IdentityResolver, IdentityResolver<'a>, owner_verified_path_resolves_owner_principal_and_relationship(), Option, Result (+33 more)
...
Cohesion: 0.16
Nodes (6): Option, Result, Ulid, Store, Store, StoreError
...
Cohesion: 0.11
Nodes (26): Result, run_benchmarks(), create_approved_draft(), handle_draft_approval_callback(), Result, Ulid, activate_approved_artifact(), Result (+18 more)
...
Cohesion: 0.14
Nodes (15): Connection, Option, Result, Ulid, Store, AuditEvent, genesis_hash(), round_trips_through_serde() (+7 more)
...
Cohesion: 0.18
Nodes (15): Source, RelationshipKind, effect_defaults_to_allow_when_omitted(), owner_route(), round_trips_through_serde(), Route, route_can_be_a_deny_route(), RouteActorWhen (+7 more)
...
Cohesion: 0.19
Nodes (13): Lifecycle, allow_and_deny_lists_never_overlap_in_the_fixture(), CapabilityPack, owner_control_basic_pack(), round_trips_through_serde(), ArtifactId, Vec, Constraints (+5 more)
...
Cohesion: 0.28
Nodes (14): ActionOrigin, AuditMeta, chain_valid(), GateContext, GateOutcome, resolve(), resolve_approval_required(), Option (+6 more)
...
Cohesion: 0.22
Nodes (12): classification_rank(), DataClassification, InstructionSources, ModelRequest, OutputPolicy, RedactionRequirement, RetentionMode, Option (+4 more)
...
Cohesion: 0.26
Nodes (11): Deserialize, ArtifactProposePayload, derived_lineage_round_trips_on_artifact_row(), inconsistent_generation_zero_with_parents_is_rejected(), inconsistent_positive_generation_without_parents_is_rejected(), lineage_is_distinct_from_version(), root_lineage_round_trips_on_artifact_row(), row_with() (+3 more)
...
Cohesion: 0.33
Nodes (4): ArtifactId, String, Vec, WorkflowManifest
...
Cohesion: 0.13
Nodes (17): CanonicalValue<'a>, Digest, digest_round_trips_through_serde(), HasherWriter<'a>, InvalidDigest, Display, Error, Formatter (+9 more)

### Community 301 - "ActionRequest"
Cohesion: 0.12
Nodes (18): Connector, connector_registry_enumerates_configured_connectors(), ConnectorRegistry, gmail(), GmailConnector, Item, Option, Self (+10 more)

## Knowledge Gaps
- **1231 isolated node(s):** `set.sh script`, `autoresearch.sh script`, `ArtifactProposePayload`, `name`, `private` (+1226 more)
...
- **Why does `AppState` connect `AppState` to `.new`, `sandbox.rs`, `.default`, `editUrl`, `Proposal: Define OpenSpine development process`, `head`, `artifact_loader.rs`, `config.rs`, `D-015 — Phase 1 should avoid final email send`, `ProposedArtifact`, `ActionRequest`, `Requirements`, `opsx-explore.md`, `ADDED Requirements`, `AppState`, `Lifecycle`, `ADDED Requirements`, `ADDED Requirements`?**
  _High betweenness centrality (0.034) - this node is a cross-community bridge._
- **Why does `TaskGrant` connect `Requirements` to `README.md`, `.new`, `event.rs`, `handle_owner_update`, `.default`, `editUrl`, `head`, `D-023 — OpenSpine is the substrate; Lyra is a product built on it`, `ProposedArtifact`, `policy.rs`, `Requirements`, `ADDED Requirements`, `ADDED Requirements`, `AppState`, `.default`, `ADDED Requirements`, `ADDED Requirements`, `AppState`?**
  _High betweenness centrality (0.021) - this node is a cross-community bridge._
- **Why does `Digest` connect `add_column_if_missing` to `Requirements`, `head`, `ADDED Requirements`, `Proposal: Implement authority composition`, `Proposal: Implement gate action API`, `Requirements`, `ADDED Requirements`, `.default`, `Lifecycle`, `OpenSpine Agent-OS Design Log`, `AppState`?**
...
- **Should `handle_owner_update` be split into smaller, more focused modules?**
