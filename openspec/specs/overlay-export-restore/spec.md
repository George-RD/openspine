# overlay-export-restore Specification

## Purpose
Define the governed, crash-recoverable export, restore, erasure-continuity, and rollback contract for the owner's durable overlay state.
...
Startup MUST normalize relative lexical data-root forms against one fixed process working directory, reject parent traversal and symlink/non-directory roots, use the resulting expected absolute data-root identity for every store, derive one adjacent protected control/snapshot root from that identity, and acquire one exclusive operating-system lock before pending-operation inspection, missing-root creation, or store open. The lock MUST be held for the process lifetime. A signed marker with invalid HMAC MUST fail startup before operation/store I/O.

#### Scenario: Alias cannot bypass process exclusion
- **WHEN** two configurations use equivalent relative lexical forms in one working directory, or one attempts a parent/symlink alias
- **THEN** equivalent forms select the same lock identity while parent/symlink aliases are rejected before data mutation
