...
Compose verified event context, identity resolution, route policy, global policy, agent manifest, workflow, capability pack, and session policy into a single, deny-by-default task grant — the only live authority object a running agent or workflow receives.
