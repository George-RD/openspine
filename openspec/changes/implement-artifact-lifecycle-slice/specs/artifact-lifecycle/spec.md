...
Define the runtime path by which a declarative artifact (route, agent, workflow, capability pack, or policy) proposed by an agent can become live authority: schema validation, digest-bound owner approval, and activation into the composed registry — never a silent self-activation.
