...
Per-scope payload encryption with terminal crypto-erasure (AD-140, resolves
OQ-7). Explicit counterparty scopes use distinct wrapped keys; existing
`ArtifactStore::put/get` callers and legacy/internal provenance remain on the
reserved `SYSTEM_SCOPE`. Erasure makes scoped plaintext unrecoverable,
preserves audit-chain evidence, and invalidates derived artifacts through their
recorded D-077 `source_scope`.
## ADDED Requirements
