...
- [x] Only after archive, verify runtime ids are archived non-integer ids, regenerate the deferred map end state (`progressive-delegation: proof_in_progress`; selected `recurring-gmail-draft: shipped`) with `node scripts/capability-map.mjs --write`, and never hand-edit roadmap markers. Do not claim `wired_into_lyra` until a later product-surface change adds Lyra artifacts and capability-level positive/fallback evidence. — DONE in the archive PR: `progressive-delegation` is `proof_in_progress` with `blocking_issues: []`, `lyra_artifacts: []` and `owner_path_tests: []`; the selected `recurring-gmail-draft` proof is `shipped` with its three named owner-path tests; the roadmap block was regenerated by the script.
