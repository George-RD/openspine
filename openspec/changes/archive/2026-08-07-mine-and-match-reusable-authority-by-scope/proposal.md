...
- Gmail `To`/`Cc` extraction. The reviewed scope binds the thread participant set the kernel can observe, which `parse_thread` derives from each message's `From` header; a counterparty CC'd onto a reviewed thread who never posts stays invisible to the scope. `email.send` remains denied, so the admitted effect is an unsent draft in the owner's own mailbox. See design.md §"Known limitation: silent thread participants".
