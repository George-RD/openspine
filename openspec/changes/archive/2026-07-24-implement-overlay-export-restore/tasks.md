...
- [x] 4.3 Add thin shell `/export <bundle-name>` and `/restore <bundle-name>` submissions plus root-owner success, foreign-principal, owner-derived-worker, malformed name, equivalent-relative lock contention, symlink-alias rejection, and conflicting request tests.
