...
Startup resolves relative `data_dir` forms against one fixed process working directory, rejects parent traversal plus symlink/non-directory roots, and derives the expected absolute data-root identity without creating a missing root. It derives one adjacent `0700` control directory from the canonical parent plus a collision-resistant digest of those absolute data-root bytes. The process opens the control directory and lock without following symlinks, acquires the exclusive lock, and retains the guard through shutdown. Only after locking may first boot create the data root or inspect pending operations. Equivalent lexical forms such as `data` and `./data` in one working directory select one lock; parent/symlink aliases fail before mutation.
