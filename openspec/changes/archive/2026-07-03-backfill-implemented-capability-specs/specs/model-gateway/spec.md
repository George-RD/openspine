...
Mediate every model-provider call that touches private context: the shell
never talks to a model provider directly and never sees a provider API
key. The kernel owns the prompt template, the untrusted-content wrapping,
and the final provider call (D-010, D-027).
