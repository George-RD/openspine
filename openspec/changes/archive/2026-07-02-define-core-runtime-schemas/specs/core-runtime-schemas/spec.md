...
Then it MUST include `source`, `connector`, `event_type`, `verified_source`, `verification_method`, `lane`, and `trust_context`
And `verified_source` MUST reflect configured owner Telegram ID verification.

#### Scenario: Email thread selection is normalized

Given an owner selects an email thread
When the event envelope is created
Then it MUST include a selected target ref
And the selected target ref MUST be traceable to a trusted user selection path.
